...
本稿は、“Attention Is All You Need”\cite{Vaswani2017} に着想を得て、動的知識グラフを単一ゲージで統御できるかを問う。

本稿では、動的知識グラフにおいて、\textbf{構造コスト}（正規化編集経路コスト：$\Delta$EPC）と\textbf{情報利得}（エントロピー変化と経路短縮：$\Delta$IG）を統合した\,\textbf{制御ゲージ} geDIG（graph edit Distance and Information Gain）を定義する。単一指標
\[
\F\;=\;\gednorm\; -\; \lambda\,\ignorm
\]
を与え、この指標に基づき\textbf{二相設計}（Phase~1: クエリ中心の局所評価、Phase~2: 大域再配線）で\,NP困難性を回避しつつ実時間性を保つ（注: 本稿では Phase~2 は設計提示のみで実証対象外）。さらに\,\textbf{二段ゲート}（AG: 曖昧検知=0‑hop、DG: 洞察確認=multi‑hop）により、\textbf{イベント駆動}で\,受容/棄却/探索/バックトラック/エビクションを横断制御する。

実験では、\textbf{部分観測迷路}で\,\emph{ステップ25–30\%削減}・\emph{冗長エッジ94.8\%圧縮}を確認し、\textbf{知識検索（RAG, 500クエリ/50ドメイン）}で\,\emph{PSZ準拠（受容$\ge$95\%, FMR$\le$2\%, 追加P50$\le$\SI{200}{ms}）}に到達した構成を示す。さらに、DG が選別した\,\emph{30ノード級サブグラフ}から導出したベクトルが LLM 応答方向と\,\emph{方向整合}（$\Delta s{=}+0.23$）を示し、\textbf{小規模構造が推論補助に寄与し得る}という\,\emph{予備的証拠（preliminary evidence）}を得た。

geDIG は、\textbf{学習（知識キュレーション）と推論（検索・利用）}の\,\emph{同時制御}を\,\textbf{One‑Gauge} の原理で実装する道筋を与える。これは\textbf{自律的な知識システム}や\textbf{Transformer 内部構造への適用}に向けた基盤となる。要するに、本稿は\,\textbf{動的知識グラフの制御}を単一スカラー $\F$ で再定式化し、\,\textbf{二段ゲート}によるイベント駆動の統一制御と\,FEP–MDL の\,\emph{操作的橋渡し}を与える。\\
\noindent 併せて、\textbf{近接的な産業的有用性と長期的学術インパクト}を明確化する。すなわち、\textbf{Phase~1} は RAG システムにおける\,\emph{クエリ中心の評価と制御}（LLM 幻覚・検索精度の実務課題）に対応し、\textbf{制約付きの実運用（PoC/パイロット）に直ちに適用可能}である。一方、\textbf{Phase~2} は FEP–MDL ブリッジに基づく\,\emph{大域再配線}という理論課題を開き、\textbf{形式的証明と大規模検証に向けた共同的な検討}を広くお願いしたい。\\
\noindent 本稿は GraphRAG, DyG‑RAG, KEDKG などの\,\emph{既存系統を補完}する立場であり、\,\textbf{性能改善（How）}ではなく\,\textbf{制御（When）}の原理に焦点を当てる。\\
\noindent \textit{Code}: \url{https://github.com/miyauchikazuyoshi/InsightSpike-AI}
\end{abstract}
...
Phase~1 は、RAG における\,\textbf{クエリ中心の評価と制御}を通じて LLM 幻覚や検索精度といった実務課題に取り組み、\,\textbf{制約付きの実運用（PoC/パイロット）に直ちに適用可能}な運用設計を与える。Phase~2 は、FEP–MDL ブリッジに基づく\,\textbf{大域再配線}という理論課題を開き、\,\textbf{形式的証明と大規模検証に向けた共同検討}を広くお願いしたい（本稿では設計スケッチを提示）。


\section{貢献 (Contributions)}
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{One‑Gauge統合制御}: 動的知識グラフにおける、構造コストの抑制（$\Delta$EPC 小）と情報整理（$\Delta$IG 増加）を単一スカラー $\F$ で測定し、AG/DG（Attention Gate / Decision Gate）の二段ゲート（$g_0>\theta_{\mathrm{AG}}$,\; $b(t){=}\min\{g_0, g_{\min}\}\le\theta_{\mathrm{DG}}$）によって探索・配線・バックトラック・メモリエビクションをイベント駆動で統一制御する枠組みを提示した。\footnote{DG は本文中で\,\textbf{D‑Gate} とも表記し、情報利得の $\Delta$IG と混同しないようにする。}
  \item \textbf{オンライン同時稼働}: 二段ゲートと二段階アーキテクチャにより、\textbf{推論（探索・検索）と学習（記憶構造化）}を単一ゲージでイベント駆動に統合し、\textbf{オンライン同時運用}への\,\emph{道筋}を示した。
  \item \textbf{FEP–MDLブリッジの形式化}: \S\ref{sec:gauge_theory} において $\F=\gednorm-\lambda\,\ignorm$ を正規化・上界・定数吸収の仮定 (B1)–(B4) の下で命題として定式化し、残差 $O(1/N)$ で $\Delta\mathrm{MDL}$ に比例することを証明スケッチと共に示した。これにより、FEP 型の誤差検出（0\,\mbox{-}hop）と MDL 型の圧縮（multi\,\mbox{-}hop）を単一ゲージで扱える条件を明確化した。
  \item \textbf{Phase 1横断実証と評価枠組の提示}: 部分観測迷路（15×15, 25×25, 50×50, 150話以上）と中規模RAG（500アイテム・50ドメイン）で DG 発火に基づく知識選別が機能することを確認し、\textbf{PSZ／クエリ品質／KG品質}の三軸評価フレームを提案。25クエリ・500クエリの実測値を当てはめて、AG/DG発火率・$g_0/g_{\min}$ 分布・遅延のトレードオフを他手法と比較可能にした。
  \item \textbf{予備的証拠（minimal structures）}: DG 発火で選別した\,\textbf{30ノード級サブグラフ}から導出したベクトルが LLM 応答方向と整合する傾向（$\Delta s{=}+\,0.23$）を観測。\,\textbf{小さな構造的橋渡し}でも推論補助が可能であることを示唆する\,\emph{予備的証拠}であり、\,\S\ref{sec:transformer_outlook} に述べる\textbf{Transformer内部の注意パターン（暗黙KG）}への適用の道筋（\S\ref{tab:insight_vector_preliminary}, \S\ref{sec:rag_limited_reasoning}）を補強する。さらに、\textbf{記憶・学習・推論を単一ゲージで統一制御する原理を外部KGで検証}した。
\end{itemize}


本研究では、以上の課題に対し動的知識グラフという外部メモリ上で洞察生成に不可欠な「構造の統合」と「情報の秩序化（エントロピー低下・集中化）」の同時発生を可観測にし、これを単一の計器（ゲージ） $\F$ で扱う One\,\mbox{-}Gauge 制御の枠組みを提案する。提案手法 geDIG では、知識グラフの構造編集（ノード・エッジ追加に伴う\,\emph{編集経路コスト（EPC）の増大というコスト（抑制が望ましい）}）と情報価値の更新（ノード情報エントロピーの低減）をそれぞれ定量化し、差分を統合した単一スカラー $\F$ によって学習（記憶の整理）と推論（探索・検索）の動的な同時制御を可能にする。これにより洞察の瞬間をイベント検出し、即座に探索方針や記憶管理へ反映できる新しいオンライン知識運用パラダイムを実現する。

既存のオンライン知識更新システム群と比較すると、geDIG は、構造変化 $\Delta$EPC を一次シグナルとして $\Delta$IG と明示的に融合し、AG/DG 二段ゲートで受容・保留・棄却を動的に分岐させ、さらにオフライン統合（Phase~2）を計画する点で明確に差別化される（\Cref{tab:related_comparison} 参照）。補足評価では、DG 発火で選別した\textbf{30ノード級サブグラフ}から導出したベクトルが LLM 応答方向と整合する傾向（\Cref{tab:insight_vector_preliminary}、$\Delta s{=}+0.23$）を観測し、「小さな構造でも推論を補助し得る」という\,\emph{予備的証拠}を得た。\Cref{tab:maze_rag_mapping} に示すように、具体的な表現は異なっても距離関数・閾値・ゲージ更新式という抽象レイヤを共有することで、迷路とRAGの両方で同じ制御原理が働く。\textbf{本稿の位置づけ}は、まず外部知識グラフで\,\emph{構造的橋渡し（短絡）の検出・選別}を One\,\mbox{-}Gauge で原理検証し（Phase~1）、その知見を手掛かりとして、\,\textbf{Transformer内部の注意パターン（暗黙KG）}への適用へと\,\emph{段階的に拡張}する道筋（§\ref{sec:rag_limited_reasoning}, §\ref{sec:transformer_outlook}）を明確に示す点にある。
...
本章ではまず、編集経路コストと情報利得の\,\textbf{正規化差分}を統合し、両者のバランスを\,\textbf{単一の評価関数}で定式化する。具体的には、評価式 $\F$ を次式のような\,\emph{仕事–エビデンス（構造編集コスト–証拠情報）の差分}形式で定義する：
\begin{equation}
\F\;=\;\gednorm\; -\; \lambda\,\big(\,\Delta H_{\mathrm{norm}}\; +\; \gamma\,\Delta\mathrm{SP}_{\mathrm{rel}}\,\big),\label{eq:F_job_evidence_intro}
\end{equation}
ここで係数 $\lambda$ は構造と情報のトレードオフ比、$\gamma$ は情報利得項内でエントロピーと構造効率利得を調整する重みである。$\F$ は小さいほど\,\emph{構造編集コストに見合う十分な情報整理が得られている}ことを示す指標となる。以後、断りのない限り\,\textbf{正規化形（$\gednorm,\,\Delta H_{\rm norm},\,\Delta \mathrm{SP}_{\rm rel}$）}を用いる。
\\
...
このとき単一ゲージ $\F$ は次式で定義する（式上の目安）。運用上の発火判定は \S\ref{sec:ag_dg_gate} の $b(t)$ に基づく：
\begin{equation}
  \boxed{\,\F := \gednorm - \lambda\,\ignorm\,},\qquad \text{（評価目安）}:\ \F\le\theta.
  \label{eq:gedig}
\end{equation}
\noindent $\F$ は\textbf{小さいほど良い}（= 構造コストが低く、情報秩序化が進むほど小さくなる）。本稿では符号規約として「負側が改善（洞察）方向」となるように定める。以後、$\F$ は\emph{評価式}を指す（\,\textbf{閾値とゲートの詳細は後述}）。
...
本選択は\,\emph{理論上の独自主張ではなく運用上の設計判断}である。ELBO/KL を含む代替定義の体系的比較やアブレーションは\,\emph{今後の課題}とし（限界と今後の課題節参照）、本稿では一貫性（FEP–MDL 命題）と運用性（分離性・解釈性）を優先している。
...
\label{sec:dual_fep_mdl_b}
\label{sec:hop_duality_b}
...
本節の主目的は、\textbf{未知迷路における探索効率の改善}を定量化することである。参照（Dijkstra／A*）は\,\emph{理想上限}として整合度（Regret／SPL や順位相関）で扱う。代表結果（25\,\mbox{$\times$}\,25, \,$N{=}100$）の要点を先にまとめる：
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{探索率}（unique/total）: geDIG は Random／DFS 比で大幅削減（\experimNote{確定値を反映}）
  \item \textbf{訪問重複率}: geDIG は 1.\,\mbox{\experimNote{xx}} 回/セル近傍まで低減（\experimNote{確定値を反映}）
  \item \textbf{平均バックトラック長}: AG\,$\to$\,DG 区間が \experimNote{xx} ステップ（最近傍分岐へ直行）
  \item \textbf{デッドエンド検出遅延}: \experimNote{xx} ステップ（$\approx$ 即時）
  \item \textbf{成功率}: \experimNote{xx}\%（25\,\mbox{$\times$}\,25, \,$N{=}100$）
...
\section{実験II: 大規模オンライン知識検索 (RAG) での評価}
...
% Operating Curves（占位図）
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[x=1.2cm,y=1.2cm]
    \draw[->] (0,0) -- (5.2,0) node[right]{Latency / Acceptance};
    \draw[->] (0,0) -- (0,3.2) node[above]{FMR};
    \fill[gray!15] (0.6,0.0) rectangle (1.8,0.4);
    \draw[thick,blue] plot[smooth] coordinates{(0.8,2.6) (1.2,1.9) (2.0,1.2) (3.0,0.8) (4.0,0.5)};
  \end{tikzpicture}
  \caption{Operating Curves（占位）: 分位掃引による FMR–Acceptance–Latency の三者同時曲線と PSZ 帯域の例示。\experimNote{実測に差し替え}}
...
\paragraph{結果サマリ（占位）}
\experimNote{equal\,\mbox{-}resources 設定での再測定完了後に、主要指標（Acc/FMR/P50/AG・DG）を反映。ベースライン内訳・埋め込み差し替え・$g_0/g_{\min}$ 分布は付録Aへ移管。}
\begin{table}[H]
\centering
\small
\caption{RAG主要指標（占位; 実測後に反映）}
\label{tab:rag_summary}
\begin{tabular}{lccccc}
\toprule
セット & PER(\%) & 受容率(\%) & FMR(\%) & P50遅延(ms) & AG/DG(\%) \\
\midrule
\multicolumn{6}{l}{\experimNote{25 / 168 / 500 の各セット結果を再測定後に挿入}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{PSZ 指標（占位; 再測定後に反映）。括弧内は目標との差分。}
\label{tab:psz_progress}
\begin{tabular}{lccc}
\toprule
クエリ数 & 受容率(\%) & FMR(\%) & P50遅延(ms) \\
\midrule
\multicolumn{4}{l}{\experimNote{25 / 168 / 500 の各セット結果を再測定後に挿入}} \\
\bottomrule
\end{tabular}
\end{table}
...
\section{最小十分性の探索: 限定的KGからの推論補助の可能性}
