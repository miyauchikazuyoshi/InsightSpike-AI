...
v2 additions: question_type-stratified aggregation.
"""
...
class EvaluationResult:
    ...
    question_type: str = ""  # v2: "comparison" or "bridge"
...
class HotpotQAEvaluator:
    ...
    def _aggregate_subset(self, results: list[EvaluationResult]) -> AggregatedResults:
        """Aggregate a subset of results."""
        if not results:
            return AggregatedResults(
                count=0,
                em=0.0, f1=0.0, precision=0.0, recall=0.0,
                sf_em=0.0, sf_f1=0.0, sf_precision=0.0, sf_recall=0.0,
        ...
        return AggregatedResults(
            ...
            sf_em=sum(r.sf_em for r in results) / n,
            sf_f1=sum(r.sf_f1 for r in results) / n,
            sf_precision=sum(r.sf_precision for r in results) / n,
            sf_recall=sum(r.sf_recall for r in results) / n,
    ...
    def aggregate(self) -> AggregatedResults:
        ...
        return self._aggregate_subset(self.results)

    def aggregate_by_type(self) -> dict[str, AggregatedResults]:
        """Aggregate results stratified by question_type (v2 addition).

        Returns:
            Dict mapping question_type to AggregatedResults.
            Always includes "all", plus "bridge" and "comparison" if present.
        """
        out: dict[str, AggregatedResults] = {"all": self.aggregate()}

        types_seen: set[str] = {r.question_type for r in self.results if r.question_type}
        for qt in sorted(types_seen):
            subset = [r for r in self.results if r.question_type == qt]
            out[qt] = self._aggregate_subset(subset)
