#!/usr/bin/env python3
"""
Pure Episodic Navigator with geDIG-aware integrated index
Combines similarity and geDIG evaluation for better navigation
"""
...
class PureEpisodicGeDIG:
    """Navigator using geDIG-aware edge selection"""
    ...
    def __init__(self, maze: np.ndarray, message_depth: int = 3):
        ...
        self.index = GeDIGAwareIntegratedIndex(
            ...
            config={
                ...
                'gedig_threshold': 0.5,
                'gedig_weight': 0.4,  # Balance similarity vs geDIG
    ...
    def _detect_loop(self) -> bool:
        ...
        return unique_positions < 4  # Stuck if visiting < 4 unique positions
    
    def get_action(self) -> Optional[str]:
        """Get next action using geDIG-aware search"""
        ...
        # Store episode with dynamic confidence
        base_confidence = 0.5
        if self.stuck_counter > 5:
            base_confidence = 0.2  # Low confidence when stuck
        ...
        # 2. geDIG-guided search (follows low geDIG paths)
        ...
        if self.stuck_counter > 5:
            # When stuck, trust geDIG-guided exploration
        ...
        # Project direction onto available actions
        for i, action in enumerate(self.actions):
            if vis.get(action) == 'path':
                ...
                # Add exploration bonus for unvisited
                next_pos = (x + dx, y + dy)
                if next_pos not in self.recent_positions[-20:]:
                    action_scores[i] += 0.3
        
        # Select action
        if np.sum(action_scores) > 0:
            probs = action_scores / np.sum(action_scores)
            valid_actions = [a for i, a in enumerate(self.actions) if action_scores[i] > 0]
            valid_probs = [p for i, p in enumerate(probs) if action_scores[i] > 0]
            
            if valid_actions:
                return np.random.choice(valid_actions, p=valid_probs)
        
        # Fallback: random valid action
        valid_actions = [a for a in self.actions if vis.get(a) == 'path']
        ...
        for step in range(max_steps):
            ...
            if step % 100 == 0 and step > 0:
                ...
                print(f"Step {step}: pos={self.position}, episodes={stats['episodes']}, "
                      f"edges={stats['edges']}, acceptance_rate={stats['edge_acceptance_rate']:.2f}")
