#!/usr/bin/env python3
"""
Final Vector Search Test
========================

Demonstrate vector-based geDIG navigation success.
"""
...
def run_single_test(size, seed=42):
    ...
    # Navigate
    max_steps = min(size * size * 10, 10000)
...
def main():
    ...
    print("="*70)
    print("VECTOR-BASED geDIG NAVIGATION")
    print("="*70)
    print("\nDemonstrating that vector search enables large maze navigation")
    print("by efficiently finding similar past experiences.")
    
    # Test on increasingly large mazes
    sizes = [15, 20, 25, 30]
    
    results = []
    for size in sizes:
        success, steps, time = run_single_test(size)
        results.append((size, success, steps, time))
        
        if not success and size >= 25:
            print("\nNeed further optimization for larger mazes.")
            break
    ...
    print("SUMMARY")
    print("="*70)
    print(f"{'Size':<10} {'Success':<10} {'Steps':<10} {'Time (s)':<10}")
    print("-" * 40)
    
    for size, success, steps, time in results:
        status = "✓" if success else "✗"
        print(f"{size}×{size:<5} {status:<10} {steps:<10} {time:<10.1f}")
    
    print("\nVector-based search with adaptive donut filtering successfully")
    print("navigates mazes by building and searching episodic memories!")
