#!/usr/bin/env python3
"""
Donut-based True geDIG Navigator
=================================

Efficient geDIG implementation using donut search from existing codebase.
"""
...
# Import donut search from InsightSpike
from insightspike.maze_experimental.query.dimension_aware_sphere_search import DimensionAwareSphereSearch

class DonutGeDIGNavigator:
    """Navigator using donut search for efficient geDIG evaluation"""
    
    def __init__(self, maze: np.ndarray, 
                 inner_volume: float = 0.001,  # 0.1% - exclude very close
                 outer_volume: float = 0.05):  # 5% - search radius
        self.maze = maze
        ...
        # Episode storage
        self.episodes = []
        self.episode_vectors = {}  # {episode_id: vector}
    ...
    def add_episode(self, pos: Tuple[int, int], action: str, 
        ...
        # Store in vector format for sphere search
        self.episode_vectors[f"ep_{episode['id']}"] = embedding
        
        # Rebuild sphere searcher with new vectors
        if len(self.episodes) > 0:
            self.sphere_searcher = DimensionAwareSphereSearch(self.episode_vectors)
            
            # Find neighbors using donut search
            neighbors = self.sphere_searcher.adaptive_donut_search(
                embedding,
                inner_volume_fraction=self.inner_volume,
                outer_volume_fraction=self.outer_volume,
                max_neighbors=50
            )
            
            # Update graph
            for neighbor in neighbors:
                neighbor_id = int(neighbor.node_id.split('_')[1])
                if neighbor_id != episode['id']:
                    # Weight based on similarity and distance
                    weight = neighbor.similarity * np.exp(-neighbor.distance * 0.5)
                    if weight > 0.1:
                        self.graph[episode['id']].append((neighbor_id, weight))
                        self.graph[neighbor_id].append((episode['id'], weight))
    ...
    def get_n_hop_neighbors_donut(self, query_pos: Tuple[int, int], n_hops: int) -> List[Dict]:
        ...
        query_embedding = np.array([
            query_pos[0] / self.width,
            query_pos[1] / self.height,
            0.0, 0.0, 0.0, 0.0  # Neutral values for non-position dimensions
        ], dtype=np.float32)
        
        # Initial donut search
        initial_neighbors = self.sphere_searcher.adaptive_donut_search(
            query_embedding,
            inner_volume_fraction=self.inner_volume,
            outer_volume_fraction=self.outer_volume,
            max_neighbors=30
        ...
        # Convert to episode IDs
        visited = set()
        current_layer = set()
        result_episodes = []
        
        for neighbor in initial_neighbors:
            ep_id = int(neighbor.node_id.split('_')[1])
            current_layer.add(ep_id)
            visited.add(ep_id)
    ...
    def navigate(self, max_steps: int = 5000) -> Dict:
        ...
        print(f"Donut search: inner={self.inner_volume*100:.1f}%, outer={self.outer_volume*100:.1f}%")
        ...
        while self.position != self.goal and steps < max_steps:
            if steps % 500 == 0 and steps > 0:
                ...
                print(f"Step {steps}: pos={self.position}, dist={dist}, "
                      ...
                      f"edges={total_edges}, search_time={avg_search_time:.1f}ms")
...
def test_donut_gedig():
    """Test donut-based geDIG navigator on 50x50"""
    from pure_episodic_navigator import create_complex_maze, visualize_maze_with_path
    import os
    
    print("="*70)
    print("DONUT-BASED geDIG NAVIGATION TEST")
    print("="*70)
    ...
    # Adaptive parameters based on maze size
    inner_volume = 0.0001  # Very small inner radius
    outer_volume = 0.01    # 1% of space
    
    nav = DonutGeDIGNavigator(maze, inner_volume=inner_volume, outer_volume=outer_volume)
    ...
    if result['success']:
        ...
        visualize_maze_with_path(
            maze, nav.path,
            f'visualizations/donut_gedig_50x50.png'
        )
    else:
        print(f"\n✗ Failed")
