#!/usr/bin/env python3
"""
Pure Episodic Navigator with modified query strategy
Query: (current_x, current_y, null, null, path, goal)
"""
...
class PureEpisodicGeDIGQuery:
    """Navigator with aspirational query strategy"""
    ...
    def _create_query_vector(self, x: int, y: int) -> np.ndarray:
        """Create query vector for SEARCH (aspirational - wants paths)"""
        vec = np.zeros(6, dtype=np.float32)
        ...
        # Aspirational features - want all paths!
        # This searches for episodes where all directions were open
        for i in range(4):
            vec[2+i] = 1.0  # "I want to find episodes with paths"
        
        # Alternative: Could also bias toward goal direction
        # goal_dx = self.goal[0] - x
        # goal_dy = self.goal[1] - y
        # if abs(goal_dx) > abs(goal_dy):
        #     vec[2 + (0 if goal_dx < 0 else 1)] = 2.0  # Extra weight
        # else:
        #     vec[2 + (3 if goal_dy < 0 else 2)] = 2.0  # Extra weight
    ...
    def _message_passing_with_gedig(self, indices: List[int], depth: int) -> np.ndarray:
        ...
        for idx, value in messages.items():
            if idx < len(self.index.metadata):
                ...
                vec = episode['vec']
                # Weight by success
                success_weight = 1.0 if episode.get('success', False) else 0.1
                direction += vec * value * success_weight
    ...
    def get_action(self) -> Optional[str]:
        """Get next action using aspirational query"""
        self._update_visual_memory()
        
        x, y = self.position
        current_vec = self._create_episode_vector(x, y)  # Actual situation
        query_vec = self._create_query_vector(x, y)      # What we want
        ...
        # Store episode with actual situation
        base_confidence = 0.5
        if self.stuck_counter > 5:
            base_confidence = 0.2
        
        episode_idx = self.index.add_episode({
            'vec': current_vec,  # Store what actually happened
        ...
        # Search with aspirational query
        start_time = time.time()
        
        # Use query vector that looks for open paths
        indices_vec, scores_vec = self.index.search(query_vec, k=15, mode='vector')
        indices_gedig, scores_gedig = self.index.search(query_vec, k=15, mode='gedig')
        indices_hybrid, scores_hybrid = self.index.search(query_vec, k=20, mode='hybrid')
