#!/usr/bin/env python3
"""
Fixed InsightSpike-AI RAG Experiment
=====================================

Fixes the issues found in the original experiment:
1. Uses correct field name (reasoning_quality instead of quality_score)
2. Adjusts insight detection threshold
3. Improves retrieval evaluation
"""
...
class FixedInsightSpikeRAGExperiment:
    """Fixed experiment using InsightSpike-AI for RAG tasks"""
    ...
    def __init__(self, backup_dir="experiment_backup"):
        ...
        self.all_responses = []  # Store all responses for analysis
    ...
    def process_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            if 'reasoning_quality' in result:
                ...
                # Adjusted threshold for CleanLLMProvider
                is_spike = quality > 0.6  # Lower threshold
                ...
                logger.debug(f"No reasoning_quality in result: {result.keys()}")
    ...
    def test_retrieval(self, queries, documents):
        ...
        for i, query in enumerate(queries):
            ...
            # More flexible evaluation - check if any relevant content is mentioned
            ...
            for topic, keywords in topic_keywords.items():
                if any(keyword in query.lower() for keyword in keywords):
                    # Check if response mentions any of these keywords
                    if any(keyword in response_text for keyword in keywords):
                        relevant_found = True
                        break
            
            # Also get reasoning quality
    ...
    def visualize_results(self, analysis):
        ...
        output_dir = Path("results_fixed_insightspike")
        ...
        ax = axes[0, 1]
        all_qualities = analysis['quality_distribution']['all_qualities']
        if all_qualities:
            ax.hist(all_qualities, bins=20, alpha=0.7, color='blue', label='All Responses')
            if self.insights:
                insight_qualities = [i['quality'] for i in self.insights]
                ax.hist(insight_qualities, bins=20, alpha=0.7, color='green', label='Insights')
            ax.axvline(analysis['quality_distribution']['mean'], color='red', linestyle='--', 
                      label=f'Mean: {analysis["quality_distribution"]["mean"]:.3f}')
            ax.set_xlabel('Reasoning Quality Score')
        ...
        metrics = [
            ...
            f"Quality Range: [{analysis['quality_distribution']['min']:.3f}, {analysis['quality_distribution']['max']:.3f}]"
        ]
        ...
        values = [
            min(1.0, 1.0 / (analysis['processing']['avg_processing_time'] + 0.001)),  # Speed (normalized)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / 'fixed_insightspike_experiment_results.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # Save analysis results
        with open(output_dir / 'fixed_experiment_analysis.json', 'w') as f:
            ...
            # Don't save all quality values to avoid large file
            analysis_to_save = analysis.copy()
            analysis_to_save['quality_distribution']['all_qualities'] = []
            
            json.dump(convert_to_serializable(analysis_to_save), f, indent=2)
...
def main():
    ...
    print("="*60)
    print("Fixed InsightSpike-AI RAG Experiment")
    print("="*60)
    
    # Create experiment instance
    experiment = FixedInsightSpikeRAGExperiment()
    ...
    try:
        ...
        retrieval_results = experiment.test_retrieval(queries, documents)
        ...
        print(f"Quality score range: [{analysis['quality_distribution']['min']:.3f}, {analysis['quality_distribution']['max']:.3f}]")
