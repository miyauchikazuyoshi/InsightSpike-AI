...
A single, configurable MainAgent that combines all features from various agent variants.
...
class UnifiedCycleResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional_fields = [
            "graph_analysis", "error_state", "query_state", 
            "transformation_history", "reasoning_paths", 
            "branch_results", "multi_hop_trace"
        ]
...
class UnifiedMainAgent:
    """
    Unified Main Agent that combines all features from different agent variants.
    
    This single class replaces:
    - MainAgent (basic)
    - EnhancedMainAgent (graph-aware memory)
    - MainAgentWithQueryTransform (query transformation)
    - MainAgentAdvanced (multi-hop, branching)
    - MainAgentOptimized (caching, async, GPU)
    - GraphCentricMainAgent (pure graph-based)
    """
    ...
    def _setup_components(self):
        ...
        if self.config.enable_async_processing:
            self.thread_pool = ThreadPoolExecutor(max_workers=self.config.parallel_branches)
            logger.info(f"Async processing enabled with {self.config.parallel_branches} workers")
        ...
        if self.config.enable_query_transform:
            try:
                ...
                self.query_transformer = QueryTransformer(
                    use_gnn=self.config.enable_gpu_acceleration
                )
                logger.info("Query transformation enabled")
            except ImportError:
                logger.warning("Query transformation requested but module not available")
    ...
    def _initialize_standard(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Enable GNN if query transform is enabled
                if self.config.enable_query_transform:
    ...
    def _initialize_graph_centric(self) -> bool:
        ...
        try:
            ...
            from insightspike.core.layers.layer3_graph_reasoner import L3GraphReasoner
            from insightspike.core.layers.layer4_narrative_generator import NarrativeGenerator
            
            self.l2_memory = GraphCentricMemoryManager()
            self.l3_graph = L3GraphReasoner(self.config.graph_config)
            self.l4_llm = NarrativeGenerator()
    ...
    def _process_standard(self, question: str, max_cycles: int, verbose: bool) -> UnifiedCycleResult:
    ...
    def _execute_standard_cycle(self, question: str, cycle_number: int) -> UnifiedCycleResult:
        ...
        try:
            ...
            retrieved_docs = memory_results if isinstance(memory_results, list) else memory_results.get("documents", [])
    ...
    def _process_with_query_transform(self, question: str, max_cycles: int, verbose: bool) -> UnifiedCycleResult:
        """Process with query transformation enabled"""
        # Initialize transformation history
        from insightspike.core.query_transformation import QueryTransformationHistory
        ...
        for cycle in range(max_cycles):
            ...
            if cycle_result.reasoning_quality > max_quality:
                ...
                best_result.transformation_history = [s.to_dict() for s in transformation_history.states]
    ...
    def _process_graph_centric(self, question: str, verbose: bool) -> UnifiedCycleResult:
...
def create_agent(mode: str = "basic", **kwargs) -> UnifiedMainAgent:
...
MainAgent = UnifiedMainAgent  # Alias for backward compatibility