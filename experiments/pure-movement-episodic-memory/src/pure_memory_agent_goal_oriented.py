#!/usr/bin/env python3
"""
Pure Memory Agent with Goal-Oriented Query
ゴール指向のクエリを使用する純粋記憶エージェント
"""
...
class PureMemoryAgentGoalOriented:
    """ゴール指向クエリを持つ純粋記憶エージェント（geDIG適応的深度選択付き）"""
    ...
    def __init__(self, 
        ...
        self.stats = {
            ...
            'gedig_evaluations': [],
            'query_types': {'goal_oriented': 0, 'exploration': 0}
        }
    ...
    def _create_goal_oriented_query(self) -> np.ndarray:
        """ゴール指向のクエリベクトル（改良版）"""
        vec = np.zeros(7, dtype=np.float32)
        ...
        # ゴール: 1.0（ゴールに関連する記憶を優先）
        vec[6] = 1.0
        
        self.stats['query_types']['goal_oriented'] += 1
        
        return vec
    
    def _create_exploration_query(self) -> np.ndarray:
        """探索用クエリ（行き詰まり時）"""
        vec = np.zeros(7, dtype=np.float32)
        ...
        # 訪問回数: 低い値（あまり訪問していない場所）
        vec[5] = 0.2
        
        # ゴール: 中立
        vec[6] = 0.5
        
        self.stats['query_types']['exploration'] += 1
    ...
    def _goal_oriented_message_passing(self, indices: List[int], depth: int) -> np.ndarray:
        """ゴール指向のメッセージパッシング"""
        if depth <= 0 or not indices:
            return np.zeros(7)
        ...
        for idx, value in messages.items():
            if idx < len(self.index.metadata):
                ...
                # ゴール関連のエピソードを優遇
                if vec[6] > 0.5:  # ゴールフラグが高い
                    weight *= 1.5
                
                # 成功エピソードを優遇
                if metadata.get('type') == 'movement' and metadata.get('success'):
                    weight *= 1.2
                
                # 未訪問エリアのエピソードを優遇
                if vec[5] < 0.1:  # 訪問回数が少ない
                    weight *= 1.1
    ...
    def get_action(self) -> str:
        ...
        # クエリ生成（状況に応じて選択）
        current_visits = self.visit_counts[self.position]
        if current_visits > 5:  # 同じ場所に何度も来ている
            query = self._create_exploration_query()
        else:
            query = self._create_goal_oriented_query()
        ...
        # 最適深度でメッセージパッシング
        insight = self._goal_oriented_message_passing(indices.tolist(), optimal_depth)
        ...
        if 0 <= direction_idx < 4:
            ...
            goal_relevance = insight[6]
            
            # ゴール関連性が高いほど、その方向を強く選択
            boost = confidence * (1.0 + goal_relevance * 0.5)
            probs[direction_idx] += 0.6 * boost
