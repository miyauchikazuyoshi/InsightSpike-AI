#!/usr/bin/env python3
"""
Pure Movement Episodic Memory Agent
完全に純粋な記憶ベースのエージェント（チート要素なし）
"""
...
class PureMemoryAgent:
    ...
    def _generate_pure_insight(self, query: np.ndarray) -> np.ndarray:
        """純粋な洞察生成（多段階統合）"""
        # 検索
        ...
        for depth in range(1, self.max_depth + 1):
            insight = self._pure_message_passing(indices, depth)
        ...
        for i, insight in enumerate(insights):
            weight = 1.0 / (i + 1) ** 0.5  # 緩やかな減衰
            final_insight += insight * weight
            total_weight += weight
        
        if total_weight > 0:
            final_insight = final_insight / total_weight
    ...
    def get_action(self) -> str:
        ...
        insight = self._generate_pure_insight(query)
        ...
        # 確率分布に変換（純粋版）
        probs = np.ones(4) * 0.15  # ベース確率
        
        if 0 <= direction_idx < 4:
            # 成功度（insight[3]）を信頼度として使用
