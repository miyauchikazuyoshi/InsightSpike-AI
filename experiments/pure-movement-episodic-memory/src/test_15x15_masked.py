...
class MaskedGedigAgent:
    """方向マスク検索を使用するgeDIGエージェント"""
    
    def __init__(self, maze: np.ndarray, use_mask: bool = True, 
                 datastore_path: str = "data/masked_gedig",
        ...
        print(f"✅ {'マスク検索' if use_mask else '通常検索'}エージェント初期化")
...
def visualize_results(maze, agent, filename):
    ...
    ax1.set_title(f"{'マスク検索' if agent.use_mask else '通常検索'} - {agent.steps}ステップ")
    ax1.legend()
    ax1.axis('off')
...
def test_15x15_maze():
    ...
    print(f"  スタート: (1, 1)")
    print(f"  ゴール: (13, 13)")
    print(f"  最短距離: {12 + 12} = 24")
    
    results = {}
    
    for use_mask in [False, True]:
        print(f"\n{'='*50}")
        print(f"{'マスク検索' if use_mask else '通常検索'}実行中...")
        print(f"{'='*50}")
        
        agent = MaskedGedigAgent(
            maze=maze,
            use_mask=use_mask,
            datastore_path=f"data/15x15_{'masked' if use_mask else 'normal'}",
            config={
        ...
        stats = agent.get_statistics()
        results[use_mask] = stats
        
        # 可視化
        visualize_results(
            maze, agent,
            f"../results/15x15_{'masked' if use_mask else 'normal'}.png"
        )
    ...
    print(f"\n通常検索:")
    print(f"  ステップ数: {normal['steps']}")
    print(f"  壁衝突率: {normal['wall_hit_rate']:.1%}")
    print(f"  ユニーク訪問: {normal['unique_visits']}")
    
    print(f"\nマスク検索:")
    print(f"  ステップ数: {masked['steps']}")
    print(f"  壁衝突率: {masked['wall_hit_rate']:.1%}")
    print(f"  ユニーク訪問: {masked['unique_visits']}")
    
    if masked['steps'] < normal['steps']:
        improvement = (normal['steps'] - masked['steps']) / normal['steps'] * 100
        print(f"\n✨ マスク検索が{improvement:.1f}%効率的")
    
    print("\n💡 分析:")
    print("- マスク検索: 視覚情報と慣性効果を活用")
    print("- 通常検索: 方向成分の影響で探索が分散")
