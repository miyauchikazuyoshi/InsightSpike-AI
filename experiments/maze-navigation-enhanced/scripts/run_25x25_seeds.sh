...
# 25x25 multi-seed runner with paper-default presets.
# Usage:
#   ./scripts/run_25x25_seeds.sh 33 57 88 101
# Optional env overrides:
#   MAX_STEPS=1500 BRAID=0.08 GOAL_FARTHEST=1 COPY_VIEWER=1 \
#   SP_SAMPLES=400 SP_BUDGET=45 SP_FORCE_SAMPLES=120 SP_FORCE_BUDGET=12 \
#   NA_TAU=-0.006 BT_TAU=-0.012 HOP_MAX=4 \
#   FORCE_L1_TAU=0.66 FORCE_L1_TOPK=2 L1_TAU=1.15 L1_TOPK=10
...
SP_SAMPLES=${SP_SAMPLES:-400}
SP_BUDGET=${SP_BUDGET:-45}
...
FORCE_L1_TAU=${FORCE_L1_TAU:-0.66}
FORCE_L1_TOPK=${FORCE_L1_TOPK:-2}
L1_TAU=${L1_TAU:-1.15}
L1_TOPK=${L1_TOPK:-10}
...
for S in "${SEEDS[@]}"; do
  OUT_DIR="$BASE_OUT/threshold_dfs_live_25_s${S}"
  mkdir -p "$OUT_DIR"
  echo "== 25x25 seed $S -> $OUT_DIR (max-steps=$MAX) =="
  ...
  python experiments/maze-navigation-enhanced/src/visualization/export_threshold_dfs_run.py \
    --size 25 --seed "$S" --max-steps "$MAX" --out-dir "$OUT_DIR" || echo "seed $S run failed or timed out"
done
