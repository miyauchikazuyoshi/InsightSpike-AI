...
ENTROPY_TOKEN_MODE="${ENTROPY_TOKEN_MODE:-auto}"
ENTROPY_TAIL_K="${ENTROPY_TAIL_K:-8}"
EPC_DISTANCE_NORM="${EPC_DISTANCE_NORM:-none}"
REQUIRE_PROBE_B="${REQUIRE_PROBE_B:-0}"
...
for model in "${MODEL_ARRAY[@]}"; do
  ...
  cmd=(
    ...
    --output "${model_output}"
    --proj-dim "${PROJ_DIM}"
    --entropy-token-mode "${ENTROPY_TOKEN_MODE}"
    --entropy-tail-k "${ENTROPY_TAIL_K}"
    --epc-distance-norm "${EPC_DISTANCE_NORM}"
...
echo "[done] inference geDIG v3 layerwise-redefine batch run completed"
