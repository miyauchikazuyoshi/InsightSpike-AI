# geDIG Transformer推論実験設計書 v3（Layerwise Redefine）

> v2 frozen snapshot: `experiments/transformer/archive/inference_gedig_v2_freeze_20260211T142929`

## 実験目的（再定義）

本実験の目的は、Transformer推論時の層遷移を次の過程として定式化し、
その過程で geDIG の `F` がどう動くかを観測・検証することである。

- 層を跨ぐたびに、文構造は一度ほぐれ（構造の破壊・確率的不確実性の増大）、
- その後、出力予測に向けて再編成される（構造の再構築・不確実性の低下）。

この設計書では上記を**中心仮定（作業仮説）**として採用する。
将来の再現実験で一貫して支持された場合は、仮説から主張へ格上げする。

迷路実験（エージェント学習過程）で導出した F 分解を推論層に写像する：

$$
F^{(l)} = \Delta EPC^{(l)} - \lambda(\Delta H^{(l)} + \gamma \Delta SP^{(l)})
$$

本実験で直接観測するのは「`F(l)` の層方向の軌跡」と「control条件との差分」である。

### Fの直感（1行）

`F` は「層遷移で進んだ構造再編の量」から「不確実性と構造複雑性のコスト」を引いた正味の進捗量である。
...
- `EPC`: hidden state 幾何の組み替え量（構造をどれだけ動かしたか）
- `H`: 語彙分布の不確実性（意味情報の拡散/収束）
- `SP`（または `β1`）: 構造の安定性/位相複雑性

本仮説では、層内で次が同時進行するとみなす。

- 構造側: 既存構造のほぐし（破壊）と再編（再構築）
- 意味側: 情報の分解 -> 固化 -> 必要に応じた再分解

### 判定基準（冒頭版）

初見の読者向けに、判定を同じ粒度で先に明記する。

- 共通定義:
  - 勾配則（本実験の操作的定義）: `F(l+1) - F(l) ≈ -k`（`k>0`）に近い層軌跡を持つこと
  - `delta_r2_struct = baseline_r2 - max(shuffle_r2, random_r2)`

| 観点 | 指標 | 判定条件（目標） | 現在値（GPT 128, token_lm） | 現状判定 |
|---|---|---|---|---|
| 勾配則の成立 | `baseline_r2` | 高い線形性（少なくとも系列内で安定） | distilgpt2 `0.0190`, gpt2 `0.0763`, gpt2-medium `0.0716` | 部分成立（強くはない） |
| control超過 | `delta_r2_struct` | `> 0` | `-0.7300`, `-0.1779`, `-0.1599` | 未達 |
| 系列整合 | 順位傾向 | 規模/品質上昇で改善方向 | `distilgpt2 < gpt2 < gpt2-medium` で単調改善 | 成立（初期シグナル） |

注: 現段階では `delta_r2_struct > 0` が未達のため、主張は「初期シグナル」に留める。
...
層別観察をやり直すにあたり、v3 では次を明示的に採用する。

- `B`:
  - claim-grade 実験では Structural Probe 学習済み `B_dist`, `B_depth` を必須化
  - fallback（恒等近似）は exploratory 限定
- `H`:
  - 語彙エントロピー定義は維持しつつ、観測トークン集合 `T_H` を明示
  - 既定: `auto`（MLM=`content`, Causal=`tail_k`）
- `EPC`:
  - `D_l = cdist(h_l B_dist^T)` の層間差を使う
  - 必要に応じて層内距離の中央値で正規化（`epc_distance_norm=median`）
...
`run_inference_gedig_v2.py` / `metrics.py` に以下を反映済み。
...
- EPC 距離行列の層内スケール正規化:
  - `--epc-distance-norm none|median`
- B 必須モード:
  - `--require-probe-b`（`--b-dist`, `--b-depth` 未指定ならエラー）
...
- 条件:
  ...
  - `entropy-token-mode=auto`
  - `epc-distance-norm=median`
  - `require-probe-b=false`（探索モード）

観測（token_lm）:

- distilgpt2:
  - `baseline_r2=0.3292`, `random_r2=0.7742`, `delta_r2_struct=-0.4450`
- gpt2:
  - `baseline_r2=0.0706`, `random_r2=0.3761`, `delta_r2_struct=-0.3055`

解釈:

- v3 定義でパイプラインは正常稼働
- ただし探索モードでは control 超過（`delta_r2_struct > 0`）は未達
- claim-grade 判定は `--require-probe-b` 付き再実行で行う

### claim-grade 試行ログ（2026-02-11）

`B_dist/B_depth` 未整備のボトルネックを解消するため、ローカル完結の軽量学習器を追加した。

- 追加スクリプト:
  - `experiments/transformer/inference_gedig_v3_layerwise_redefine/fit_probe_b_lightweight.py`
- 方式:
  - spaCy 依存木（`en_core_web_sm`）を教師として、hidden state から `B_dist/B_depth` を最小二乗学習
  - 出力は `.npy`（`run_inference_gedig_v2.py --require-probe-b` に直接投入可能）
...
#### 実測結果（`--require-probe-b`）

`delta_r2_struct_fixed = baseline_r2 - max(shuffle_r2, random_r2)` を採用。

| モデル | サンプル | 実行ディレクトリ | baseline_r2 | shuffle_r2 | random_r2 | delta_r2_struct_fixed | 判定 |
|---|---:|---|---:|---:|---:|---:|---|
| distilgpt2 | 64 | `results/claim_probeb_distilgpt2_64/` | 0.4543 | 0.3606 | 0.0318 | +0.0937 | 初期シグナルあり |
| gpt2 | 64 | `results/claim_probeb_gpt2_64/` | 0.3073 | 0.2286 | 0.0264 | +0.0787 | 初期シグナルあり |
| gpt2-medium（layer12-B） | 64 | `results/claim_probeb_gpt2-medium_layer12_64/` | 0.0932 | 0.0831 | 0.0252 | +0.0102 | 弱いシグナル |
| gpt2-medium（layer-1-B） | 128 | `results/claim_probeb_scaled_gpt2-medium_128/` | 0.0204 | 0.0450 | 0.0012 | -0.0246 | 未達 |

#### 中間解釈

- `distilgpt2` / `gpt2` では `delta_r2_struct_fixed > 0` を確認（「仮説棄却ではない」段階）。
- `gpt2-medium` は設定依存性が高く、同一手順での再現性がまだ弱い。
- 現時点の主張は「初期シグナル」に限定し、普遍性主張（モデルスケールと単調相関）は保留。
- 補足:
  - `gpt2-medium` の probe 学習自体は成立（例: `layer=-1, n=128` で `dist_pearson=0.9665`, `depth_pearson=0.9166`）。
  - それでも `F` 指標が弱いので、問題は「B学習失敗」より「F動態側の定義/係数/層選択」の可能性が高い。

#### 直近の課題（実験設計上）

1. `B` 学習条件の固定化（層選択、学習率、seed、サンプル）と再現ランの追加
2. baseline 最適 `(λ,γ)` を固定して control へ転移する厳密比較ランの追加
3. `gpt2-medium` の不安定性切り分け:
   - `layer-index` 掃引（-1 / 中間層）
   - `max-samples` 掃引（64 / 128 / 256）
   - `B` 品質指標（dist/depth の教師相関）と `baseline_r2` の相関確認

## 観測対象：Fの層ダイナミクス

評価対象は、各層での `F(l)` とその control 差分。

- baseline: 学習済み重み
- shuffle control: 入力語順シャッフル
- random control: ランダム初期化重み

ここから `baseline_r2`, `delta_r2_learn`, `delta_r2_struct` を算出し、
「構造の破壊→再構築」仮説と整合するかを判定する。

## 操作的定義（EPC -> B -> H）

### EPC定義（構造変化）

層 `l-1` と層 `l` の距離行列変化を、Frobeniusノルムで正規化して定義する。
...
`B` は Structural Probe で学習する線形射影行列で、hidden state を
「構文木距離・深さが読み出せる部分空間」へ写像する。

$$
d_B(h_i, h_j)^2 = (B(h_i-h_j))^T(B(h_i-h_j))
$$

推論時は `B` を固定し、層ごとの構造量（EPC, SP）を算出する。

### H定義（語彙エントロピー）

hidden state を unembedding で語彙空間へ射影し、softmax後の Shannon entropy を取る。
...
- `baseline R²`:
  - 学習済みモデル（通常重み）で得た平均 `F(l)` カーブに対する線形近似の決定係数。
  - 1に近いほど「層に沿って直線的に説明できる」。
- `shuffle R²`:
  - 入力語順をシャッフルした control 条件での同じ R²。
- `random R²`:
  - ランダム初期化重み（未学習）条件での同じ R²。
- `ΔR²_learn`:
  - `baseline_r2 - random_r2`
  - 正なら「学習済みの方が random-init より線形性が高い」。
- `ΔR²_struct`:
  - `baseline_r2 - max(shuffle_r2, random_r2)`
  - 正なら「最も強い control を上回る」。
  - 本実験の厳しめの主指標。
- `baseline best (λ,γ)`:
  - baseline 条件でグリッド探索したときに、平均 `F(l)` の線形性（R²）が最大となる係数ペア。
  - 「普遍定数」とは限らず、系列・正規化方法で変わりうる。
...
### ローカル深掘り追記（2026-02-11）

#### 実施内容

- 解析スクリプト追加:
  - `analyze_local_deepening.py`（z-score再推定、系列順位チェック）
  - `compute_llv_lite.py`（LLV-lite 距離と F 指標の照合）
- 実行ディレクトリ:
...
#### 128サンプル時点の観測（GPT 3モデル）

| model | baseline R² | random R² | ΔR²_struct | best (λ,γ) |
|---|---:|---:|---:|---|
| distilgpt2 | 0.0190 | 0.7490 | -0.7300 | (0.01, 0.01) |
| gpt2 | 0.0763 | 0.2542 | -0.1779 | (0.5, 10.0) |
| gpt2-medium | 0.0716 | 0.2315 | -0.1599 | (0.01, 0.01) |

系列内順位（`distilgpt2 < gpt2 < gpt2-medium`）で
`ΔR²_struct` は単調改善（非減少）を確認:

- `-0.7300 -> -0.1779 -> -0.1599`

64サンプル時点との比較では:

- `distilgpt2` は `ΔR²_struct` が改善（`-0.7773 -> -0.7300`）
- `gpt2` は `ΔR²_struct` が悪化（`-0.0864 -> -0.1779`）
- `gpt2-medium` も `ΔR²_struct` は悪化（`-0.0648 -> -0.1599`）

したがって、`max-samples` 増加だけでは一様改善は得られず、
モデル依存の揺れが残ることを確認した。

#### z-score再推定（暫定）

- `distilgpt2`: raw/z-score とも `(0.01, 0.01)` で安定
- `gpt2`: raw `(0.5, 10.0)` -> z-score `(0.1, 10.0)` に変化
- `gpt2-medium`: raw/z-score とも `(0.01, 0.01)` で安定
- 含意: 生の係数比較の前に正規化手順を標準化する必要がある

補足（`raw` と `z-score` の違い）:

- `raw_best_*`: 元スケール（`ΔEPC, ΔH, Δstruct` をそのまま）で最適化した結果
- `z_best_*`: 各項を z-score 正規化してから最適化した結果
- 目的: 項ごとのスケール差で `(λ,γ)` が見かけ上ずれる問題を分離する

#### LLV-lite（Oyama軸のローカル近似）

- 参照モデル `distilgpt2` からの LLV 距離（二乗L2）:
  - `gpt2`: `46.60`
  - `gpt2-medium`: `100.72`
- `LLV_dist2_to_ref` vs `ΔR²_struct` の相関（n=3）:
  - Pearson: `0.8582`
  - Spearman: `1.0000`
- ただし n=3 のため、傾向は「初期シグナル」に留める

補足（LLV-lite 指標）:

- `LLV_dist2_to_ref`: 参照モデルとの LLV-lite ベクトル距離（二乗L2）
  - 大きいほど、モデル全体の次トークン確率挙動が参照から離れている
- `corr(LLV_dist2_to_ref, ΔR²_struct)`:
  - 距離と主指標の共変動を見る探索相関（この段階では因果解釈しない）

#### 運用メモ（再現性）

- `run_inference_gedig_v2.sh` 実行時、`XDG_CACHE_HOME` が結果ディレクトリ配下だと
  モデル再ダウンロードが起きる場合がある
- 大きいモデル再実行時は `XDG_CACHE_HOME=$HOME/.cache` を明示して回避する
