...
## この実験の位置づけ（何の証明か / 何の証明でないか）

> **記憶グラフを構築しながら迷路を進むエージェントにおいて、「何を受け入れ・何を捨て・いつ引き返すか」という判断のすべてが、ドメイン知識なしに F の運用（単一ゲージ＋分位ゲート AG/DG）だけから創発することを示す PoC。**

- **証明しないこと**: 迷路ソルバとしての優越。Greedy DFS（迷路専用アルゴリズム）は 15×15 で 99% を達成し、geDIG（98%）はこれを上回らない（25×25 では 75% vs 95% と明確に劣る）。「解ける」は制御が壊れていないことの必要条件であって、主張ではない。
- **証明すること**:
  1. **ルールなしの創発的制御** — 「行き止まりなら戻れ」等のルールを一切持たず、F と分位ゲートだけで探索深化・バックトラック・受容が成立する
  2. **記憶の選別（write-gate）** — 候補エッジの 98–99% を棄却しながら成功率を維持＝トポロジー骨格だけを記憶する
  3. **技術的動作検証** — F の実時間計算、二段ゲートの設計どおりの発火、クエリ中心 multi-hop の計算量管理
- **stage-2（Wake-Sleep-Wake; `graph_persistent_dg/`）の追加目標**: wake で構築した記憶グラフが sleep の再編（報酬伝播・剪定）を経て**次の試行を改善する**こと。従属変数は成功率ではなく（15×15 では 98% で天井）、**平均ステップ・エッジ数・再訪時の改善**。現状の証拠（25×25: 71.9%→95.3%）は 10D＋sleep＋カリキュラムの**パッケージ効果**であり、sleep 単独の寄与分離（事前登録アブレーション）が次の課題（`docs/audits/`, `docs/prereg/` 参照）。
...
geDIG は「if文（ルールベース）」と「強化学習（RL）」の **中間的なアプローチ** です。
...
|------|------|-----------|----------|
| 学習 | なし | **なし** | あり（大量データ必要） |
| 判断基準 | 明示的ルール | **構造評価** | 学習済みポリシー |
| 解釈可能性 | 完全 | **高い**（g₀, g_min, k★で説明可能） | 低い（ブラックボックス） |
| 汎化 | 脆い | 構造依存 | データ依存 |
| 訓練データ | 不要 | **不要** | 大量に必要 |
| ゼロショット | ✓ | **✓** | ✗ |
...
geDIG の **hop 制限** は、人間の空間ナビゲーションにおける **ワーキングメモリ制約** と類似しています。

| 人間の迷路認知 | geDIG |
|---------------|-------|
| 短期記憶の限界 | max_hops の限界 |
| 「あの分岐、どっち行ったっけ？」 | 遠くの未探索ノードが見えない |
| 行き止まりから戻る時の迷い | SP利得が計算できない |
| 通った道の記憶が薄れる | hop で届かない = 存在しないのと同じ |
...
| 設定 | 成功 | 失敗 | 成功率 | 備考 |
|------|------|------|--------|------|
| max_hops=10 | 43 | 17 | 72% | ベースライン |
| max_hops=15 (失敗分再試行) | +8 | 9 | - | 17シード中8シード回復 |
| max_hops=25 (失敗分再試行) | +0 | 9 | - | 9シード中0シード回復 |
| **統合** | **51** | **9** | **85%** | Phase 1 の限界 |

**発見:**
- hop 数を増やすことで 72% → 85% に改善（+13ポイント）
- 残り15%（9シード）は hop 数だけでは解決できない
- これらの迷路は構造的に複雑（行き止まり・分岐が多い）
- 探索効率は高い（繰り返し訪問が少ない）が、経路選択が難しい

**失敗シードの特徴（例: Seed 9）:**
- 最短経路: 104ステップ（到達可能）
- 500ステップ探索しても未到達
- 探索効率: Edges/Steps = 1.28（効率的）
- 問題: 探索が進むと ΔSP → 0（ショートカット発見不能）

**hop 制限の影響:**
- 分岐から行き止まりまでの距離が hop 数を超えると、未探索分岐に「戻れない」
- hop 数を増やす = **認知地図の拡張**（より広い範囲を「覚えている」）
- 計算コストは hop 数に比例しない（必要な時だけ深く探索）

**Phase 2 への示唆:**
- 残り15%は「学習の余地」として設計に織り込み済み
- 失敗経路を負例として学習すれば、2回目で解ける可能性が高い
...
A* を知っている人には「ゴール未知でグラフ自体を発見する拡張」と伝わる。

### ヒューリスティックから内発報酬化への移行ロードマップ

geDIG は Phase 1 では完全にヒューリスティック（人間定義）で動作するが、段階的に内発報酬化へ移行する設計。
...
これは RL の「最初は何もできない、大量データで学習してやっと動く」と対照的。
人間の認知発達（生まれつきの構造バイアス + 経験による調整）とも一致する。
