...
from qhlib.models import StepRecord, QueryHubConfig, EpisodeArtifacts, MazeSummary
from qhlib.utils import (
    ...
    compute_episode_vector, compute_query_vector, weighted_distance,
    gather_node_features, build_feature_matrix, encode_observation, manhattan,
    candidate_index, edge_set, norm_edge, normalise_container, item_key, merge_unique,
)
from qhlib.sleep import (
    build_sleep_action_plan as _build_sleep_action_plan,
    build_sleep_q_table as _build_sleep_q_table,
...
def run_episode_query(
    ...
    *,
    ...
    graph = nx.Graph()
    # Optional: load affordance biases and DS baseline edges from persisted graph
    affordance_cache: Dict[str, Dict[str, Any]] = {}
    ds_edges_baseline: List[List[List[int]]] = []
    try:
        if getattr(config, "persist_sqlite_path", None):
            ...
            if bool(getattr(config, "affordance_bias", False)):
                affordance_cache = store.load_node_attributes()
            try:
                _, ds_edges_baseline = store.snapshot_graph()
            except Exception:
                ds_edges_baseline = []
    except Exception:
        affordance_cache = {}
    ...
    def register_direction_node(
        ...
        *,
        ...
        # Apply persisted affordance bias if available and not yet set
        try:
            if "affordance_bias" not in node_entry and affordance_cache:
                key = f"{int(node_id[0])},{int(node_id[1])},{int(node_id[2])}"
                cached = affordance_cache.get(key) or {}
                if "affordance_bias" in cached:
                    node_entry["affordance_bias"] = float(cached.get("affordance_bias", 0.0))
                if "affordance_updates" in cached:
                    node_entry["affordance_updates"] = int(cached.get("affordance_updates", 0))
                if "affordance_last_step" in cached:
                    node_entry["affordance_last_step"] = int(cached.get("affordance_last_step", -1))
        except Exception:
            pass
    ...
    for step in range(config.max_steps):
        ...
        try:
            sleep_edge_mode = str(getattr(config, "sleep_edge_mode", "mul") or "mul").lower()
        except Exception:
            sleep_edge_mode = "mul"
        if sleep_edge is not None and guide_mode == "prefer":
            try:
                sleep_edge_beta = float(getattr(config, "sleep_edge_beta", 0.0) or 0.0)
            except Exception:
                sleep_edge_beta = 0.0
            if math.isfinite(sleep_edge_beta) and abs(sleep_edge_beta) > 1e-12:
                try:
                    sleep_edge_by_action = dict(sleep_edge.get(current_position, {}) or {})
                except Exception:
                    sleep_edge_by_action = {}
        ...
        try:
            affordance_enabled = bool(getattr(config, "affordance_bias", False))
        except Exception:
            affordance_enabled = False
        try:
            affordance_beta = float(getattr(config, "affordance_beta", 1.0))
        except Exception:
            affordance_beta = 1.0
        try:
            affordance_lr = float(getattr(config, "affordance_lr", 0.0))
        except Exception:
            affordance_lr = 0.0
        try:
            affordance_clamp = float(getattr(config, "affordance_clamp", 0.0))
        except Exception:
            affordance_clamp = 0.0
        affordance_bias_enabled = bool(affordance_enabled) and math.isfinite(affordance_beta) and abs(affordance_beta) > 1e-12
        affordance_update_enabled = bool(affordance_enabled) and math.isfinite(affordance_lr) and abs(affordance_lr) > 1e-12
        ...
        def _affordance_bias_for_action(action_id: int) -> float:
            if not affordance_bias_enabled:
                return 0.0
            try:
                delta = SimpleMaze.ACTIONS[int(action_id)]
            except Exception:
                return 0.0
            dir_idx = direction_from_delta(delta)
            if dir_idx is None:
                return 0.0
            node_id = make_direction_node(anchor_position, int(dir_idx))
            try:
                if node_id in graph:
                    return float(graph.nodes[node_id].get("affordance_bias", 0.0) or 0.0)
            except Exception:
                return 0.0
            return 0.0

        def _sleep_edge_weight_for_action(action_id: int) -> float:
            if not sleep_edge_bias:
                return 0.0
            try:
                return float(sleep_edge_by_action.get(int(action_id), 0.0) or 0.0)
            except Exception:
                return 0.0

        def _sleep_edge_multiplier(edge_weight: float) -> float:
            if not sleep_edge_bias:
                return 1.0
            try:
                if sleep_edge_mode == "add":
                    return math.exp(float(sleep_edge_beta) * float(edge_weight))
                return max(0.0, 1.0 + math.tanh(float(sleep_edge_beta) * float(edge_weight)))
            except Exception:
                return 1.0
        ...
        def choose_observation_candidate(collections: Sequence[Sequence[Dict[str, Any]]]) -> Optional[Dict[str, Any]]:
            ...
            """
            def _select_from_items(candidate_items: Sequence[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
                items = [it for it in candidate_items if it]
                if not items:
                    return None
                # Anti-backtrack: avoid immediately reversing previous action if alternatives exist
                if getattr(config, 'anti_backtrack', True) and prev_action_delta is not None and len(items) > 1:
                        ...
                        for s, item in zip(sims, items):
                            ...
                            if event_bias_enabled and item.get("action") is not None:
                                try:
                                    ...
                                    pass
                            if affordance_bias_enabled and item.get("action") is not None:
                                try:
                                    act = int(item.get("action"))
                                    w *= math.exp(float(affordance_beta) * _affordance_bias_for_action(act))
                                except Exception:
                                    pass
                            if sleep_edge_bias and item.get("action") is not None:
                                try:
                                    act = int(item.get("action"))
                                    edge_weight = _sleep_edge_weight_for_action(act)
                ...
                def _score(entry: Dict[str, Any]) -> float:
                    ...
                    try:
                        ...
                        pass
                    try:
                        if affordance_bias_enabled:
                            bonus += float(affordance_beta) * _affordance_bias_for_action(act)
                    except Exception:
                        pass
                    try:
                        if sleep_edge_bias:
            ...
            action_source = str(getattr(config, 'action_source', 'obs') or 'obs').lower()
            # mix: allow mem candidates in the same ranking pool (still respects possible_moves)
            if action_source == "mix":
                ...
                for items in collections:
                    ...
                    for item in items:
                        ...
                        if getattr(config, 'obs_guard', True):
                            if item.get("action") not in possible_moves_set:
                                continue
                            if not bool(item.get("passable", True)):
                                continue
                        pooled.append(item)
                picked = _select_from_items(pooled)
            ...
            for items in collections:
                ...
                if not obs_items:
                    continue
                picked = _select_from_items(obs_items)
                if picked is not None:
                    return picked
        ...
        def _union_khop_subgraph(graph_obj: nx.Graph,
                                  ...
                                  hop_k: int) -> nx.Graph:
            g1, n1 = extract_k_hop_subgraph(graph_obj, anchors_a, hop_k)
            g2, n2 = extract_k_hop_subgraph(graph_obj, anchors_b, hop_k)
        ...
        def _sp_gain_fixed_pairs_ex(sub_before: nx.Graph, sub_after: nx.Graph, eff_hop: int = 1) -> Tuple[float, float, float]:
            ...
            try:
                ...
                if bound in ("trim", "terminal", "nodes"):
                    g1 = trim_terminal_edges(g1, anchors_core, max(1, int(eff_hop)))
                    g2 = trim_terminal_edges(g2, anchors_core, max(1, int(eff_hop)))
        ...
        def _sp_gain_allpairs_ex(sub_before: nx.Graph, sub_after: nx.Graph, eff_hop: int = 1) -> Tuple[float, float, float]:
            ...
            try:
                ...
                if scope in ("union", "merge", "superset"):
                    ...
                    g2 = sub_after.subgraph(all_nodes).copy()
                if bound in ("trim", "terminal", "nodes"):
                    g1 = trim_terminal_edges(g1, stage_anchor_nodes, max(1, int(eff_hop)))
                    g2 = trim_terminal_edges(g2, stage_anchor_nodes, max(1, int(eff_hop)))
        ...
        def _sp_gain_rel(g_before: nx.Graph, g_after: nx.Graph, eff_hop: int, e_u=None, e_v=None) -> float:
            sp_g1, nodes_sp1 = extract_k_hop_subgraph(g_before, stage_anchor_nodes, eff_hop)
            sp_g2, nodes_sp2 = extract_k_hop_subgraph(g_after, stage_anchor_nodes, eff_hop)
            ...
            if str(core.sp_boundary_mode).lower() in ("trim","terminal","nodes"):
                sp_g1 = trim_terminal_edges(sp_g1, stage_anchor_nodes, eff_hop)
                sp_g2 = trim_terminal_edges(sp_g2, stage_anchor_nodes, eff_hop)

            # coreモード: Coreに委譲
            if not getattr(config, 'sp_cache', False) or str(getattr(config, 'sp_cache_mode', 'core')) == 'core' or e_u is None or e_v is None:
                try:
                    return float(compute_sp_gain_norm(sp_g1, sp_g2, mode=core.sp_norm_mode))
                except Exception:
                    return 0.0
        ...
        # DEBUG: Print at start of inline multi-hop
        if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
            print(f"[DEBUG] run_experiment_query: Starting inline multi-hop max_h={max_h} step={step}", flush=True)
        h_best = 0
        g_best = None
        ...
        if gh_mode == 'radius':
            ...
            for h in range(1, max_h + 1):
                ...
                try:
                    if str(core.ig_source_mode).lower() in ("linkset","paper","strict") and str(core.ig_hop_apply).lower() == "all" and pre_result is not None:
                        ls_h = compute_linkset_metrics(prev_graph, h_graph, pre_linkset_info, query_vector=query_vec_list, ig_fixed_den=ig_fixed_den)
            ...
            for h in range(1, max_h + 1):
                ...
                sp_before_g, sp_nodes = extract_k_hop_subgraph(prev_graph, stage_anchor_nodes, max(1, eff_h_diag))
                try:
        ...
        eval_applied = False
        t_eval_ms = 0.0
        # DEBUG: Print before eval try block
        if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
        ...
        try:
            ...
            if (not eval_applied) or bool(getattr(config, 'force_per_hop', False)) or ag_wants_hops:
                if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
                    print(f"[DEBUG] run_experiment_query: Calling evaluate_multihop", flush=True)
        ...
        if bool(getattr(core, 'enable_ged_min_diag', False) or getattr(config, 'force_sp_gain_eval', False)):
            try:
                ...
                ged_min_proxy_val = float(compute_ged_min_proxy(gp, ga))
            except Exception:
                ged_min_proxy_val = 0.0
            ...
            if (sleep_q_bias or sleep_plan_bias or event_bias_enabled or affordance_bias_enabled or sleep_edge_bias) and pm:
                try:
                    ...
                    for a, qv in zip(pm, q_vals):
                        ...
                        if affordance_bias_enabled:
                            w *= math.exp(float(affordance_beta) * _affordance_bias_for_action(int(a)))
                        if sleep_edge_bias:
        ...
        sleep_edge_applied = bool(sleep_edge_bias)
        sleep_edge_weight_value = 0.0
        sleep_edge_mode_used = ""
        if sleep_edge_applied:
            try:
                sleep_edge_weight_value = float(_sleep_edge_weight_for_action(int(action)))
                sleep_edge_mode_used = str(sleep_edge_mode)
            except Exception:
                sleep_edge_weight_value = 0.0
                sleep_edge_mode_used = str(sleep_edge_mode) if sleep_edge_mode else ""
        ...
        if affordance_bias_enabled:
            try:
                affordance_bias_value = float(_affordance_bias_for_action(int(action)))
                affordance_bias_applied = True
            except Exception:
                affordance_bias_value = 0.0
                affordance_bias_applied = False
        ...
        # Affordance memory update (store on direction node as long-term bias)
        affordance_bias_update = 0.0
        affordance_bias_post = float(affordance_bias_value)
        affordance_nodes_meta_snapshot: List[Dict[str, Any]] = []
        if affordance_update_enabled and event_weights:
            try:
                w = 0.0
                if moved:
                    w += float(event_weights.get("move_success", 0.0))
                ...
                if bool(obs.is_goal):
                    w += float(event_weights.get("goal_reached", 0.0))
                if moved:
                    visits_after = int(visit_counts.get(current_position, 0))
                    visits_before = max(0, visits_after - 1)
                    if visits_before <= 0:
                        w += float(event_weights.get("novel_cell", 0.0))
                    else:
                        w += float(event_weights.get("revisit", 0.0))
                if prev_action_delta is not None:
                    try:
                        if (int(action_delta[0]), int(action_delta[1])) == (-int(prev_action_delta[0]), -int(prev_action_delta[1])):
                            w += float(event_weights.get("immediate_backtrack", 0.0))
                    except Exception:
                        pass
                if "deadend" in event_weights:
                    try:
                        if len(possible_moves_post) <= 1:
                            w += float(event_weights.get("deadend", 0.0))
                    except Exception:
                        pass
                try:
                    if sleep_plan_action is not None and int(action) == int(sleep_plan_action):
                        w += float(event_weights.get("progress", 0.0))
                    elif sleep_q_by_action:
                        q_val = float(sleep_q_by_action.get(int(action), float(sleep_q_max)))
                        if math.isfinite(q_val) and q_val >= float(sleep_q_max) - 1e-9:
                            w += float(event_weights.get("progress", 0.0))
                except Exception:
                    pass
                affordance_bias_update = float(w)
            except Exception:
                affordance_bias_update = 0.0
            try:
                if action_dir_idx is not None:
                    node_id = make_direction_node(last_position, int(action_dir_idx))
                    if node_id in graph:
                        prev = float(graph.nodes[node_id].get("affordance_bias", 0.0) or 0.0)
                        new = prev + float(affordance_lr) * float(affordance_bias_update)
                        if math.isfinite(affordance_clamp) and float(affordance_clamp) > 0:
                            lim = float(affordance_clamp)
                            new = max(-lim, min(lim, new))
                        graph.nodes[node_id]["affordance_bias"] = float(new)
                        graph.nodes[node_id]["affordance_updates"] = int(graph.nodes[node_id].get("affordance_updates", 0)) + 1
                        graph.nodes[node_id]["affordance_last_step"] = int(step)
                        affordance_bias_post = float(new)
                        affordance_nodes_meta_snapshot = [
                            {
                                "node": [int(node_id[0]), int(node_id[1]), int(node_id[2])],
                                "node_type": "direction",
                                "affordance_bias": float(new),
                                "affordance_updates": int(graph.nodes[node_id].get("affordance_updates", 0)),
                                "affordance_last_step": int(step),
                                "step": int(step),
                            }
                        ]
            except Exception:
        ...
        if _need_post:
            try:
                ...
                for h in range(0, max_h_post + 1):
                    ...
                    sp_g1, nodes_sp1 = extract_k_hop_subgraph(sub_b, post_stage_anchors, max(1, eff_h))
                    sp_g2, nodes_sp2 = extract_k_hop_subgraph(sub_a, post_stage_anchors, max(1, eff_h))
                    if str(core.sp_scope_mode).lower() in ("union","merge","superset"):
                        all_nodes = set(nodes_sp1) | set(nodes_sp2)
                        if all_nodes:
                    ...
                    if str(core.sp_boundary_mode).lower() in ("trim","terminal","nodes"):
                        sp_g1 = trim_terminal_edges(sp_g1, post_stage_anchors, max(1, eff_h))
                        sp_g2 = trim_terminal_edges(sp_g2, post_stage_anchors, max(1, eff_h))
        ...
        try:
            if getattr(config, 'sp_ds_sqlite', None):
                ...
                for he in range(1, H_eval_save + 1):
                    ...
                    sp_g2, nodes2 = extract_k_hop_subgraph(graph, anchors_next, max(1, he))
        ...
        if config.linkset_mode and linkset_info is not None:
            try:
                ls = compute_linkset_metrics(prev_graph, graph_eval, linkset_info, query_vector=query_vec_list, ig_fixed_den=ig_fixed_den)
        ...
        # Affordance bias map (per node) for visualization
        affordance_bias_map: Dict[str, float] = {}
        try:
            if bool(getattr(config, "affordance_bias", False)):
                for nid, data in graph.nodes(data=True):
                    if not (isinstance(nid, (list, tuple)) and len(nid) >= 3):
                        continue
                    try:
                        dir_val = int(nid[2])
                    except Exception:
                        continue
                    if dir_val < 0 or dir_val > 3:
                        continue
                    if data is None:
                        continue
                    if "affordance_bias" not in data:
                        continue
                    try:
                        b = float(data.get("affordance_bias", 0.0))
                    except Exception:
                        b = 0.0
                    if abs(b) <= 1e-9:
                        continue
                    key = f"{int(nid[0])},{int(nid[1])},{int(dir_val)}"
                    affordance_bias_map[key] = float(b)
        except Exception:
            affordance_bias_map = {}
        ...
        prev_set = edge_set(diff_baseline_graph)
        now_set = edge_set(graph)
        ...
        normalised_candidates = normalise_container(selection.candidates)
        normalised_links = normalise_container(committed_links)
        normalised_forced = normalise_container(candidate_selection["forced_links"])

        forced_marked = [dict(entry, forced=True) for entry in normalised_forced]
        candidate_pool = merge_unique(normalised_candidates + forced_marked)
        selected_links = merge_unique(normalised_links + forced_marked)
        ranked_candidates = merge_unique(normalise_container(ranked_all_candidates))
        ...
        try:
            if getattr(config, 'persist_sqlite_path', None):
                ...
                # Affordance bias updates (update existing nodes)
                if affordance_nodes_meta_snapshot:
                    ds_nodes_saved_snapshot += store.upsert_nodes(affordance_nodes_meta_snapshot)
        ...
        step_records.append(
            StepRecord(
                ...
                ds_edges_baseline=(ds_edges_baseline if int(step) == 0 else []),
                ...
                affordance_bias_applied=bool(affordance_bias_applied),
                affordance_bias_update=float(affordance_bias_update),
                affordance_bias_post=float(affordance_bias_post),
                affordance_bias_map=dict(affordance_bias_map),
    ...
    for lambda_weight in lambda_values:
        ...
        def append_record(record: StepRecord, *, phase: str) -> None:
            ...
            row = {
                ...
                "affordance_bias_applied": bool(getattr(record, "affordance_bias_applied", False)),
                "affordance_bias_update": float(getattr(record, "affordance_bias_update", 0.0)),
                "affordance_bias_post": float(getattr(record, "affordance_bias_post", 0.0)),
                "affordance_bias_map": getattr(record, "affordance_bias_map", {}),
                "ds_edges_baseline": getattr(record, "ds_edges_baseline", []),
        ...
        # Parallel execution for non-curriculum mode
        ...
        if not use_curriculum and num_workers > 1:
            ...
            # Sequential execution (curriculum mode or workers=1)
            for offset in range(args.seeds):
                ...
                if use_curriculum:
                    ...
                    sleep_edge = None
                    sleep_edge_meta = None
                    if bool(getattr(warm_cfg, "sleep_edge_enabled", False)):
                        sleep_edge, sleep_edge_meta = _build_sleep_edge_weights(
                            warm_artifacts.steps,
                            start_pos=sp,
                            goal_pos=gp,
                            alpha=float(getattr(warm_cfg, "sleep_edge_alpha", 0.4)),
                            gamma=float(getattr(warm_cfg, "sleep_edge_gamma", 0.95)),
                            alpha_explore=float(getattr(warm_cfg, "sleep_edge_alpha_explore", 0.05)),
                            revisit_penalty=float(getattr(warm_cfg, "sleep_edge_revisit_penalty", 0.2)),
                            deadend_penalty=float(getattr(warm_cfg, "sleep_edge_deadend_penalty", 0.2)),
                            blocked_penalty=float(getattr(warm_cfg, "sleep_edge_blocked_penalty", 0.2)),
                            revisit_threshold=int(getattr(warm_cfg, "cortisol_repeat_visits", 2)),
                        )
        ...
        output_payload = {
            "config": {
                ...
                "sleep_q": {
                    ...
                    "deadend_penalty": float(getattr(args, "sleep_q_deadend_penalty", 0.0)),
                    "blocked_penalty": float(getattr(args, "sleep_q_blocked_penalty", 0.0)),
                },
                "sleep_edge": {
                    "enabled": bool(getattr(args, "sleep_edge_weight", False)),
                    "beta": float(getattr(args, "sleep_edge_beta", 1.0)),
                    "alpha": float(getattr(args, "sleep_edge_alpha", 0.4)),
                    "gamma": float(getattr(args, "sleep_edge_gamma", 0.95)),
                    "alpha_explore": float(getattr(args, "sleep_edge_alpha_explore", 0.05)),
                    "revisit_penalty": float(getattr(args, "sleep_edge_revisit_penalty", 0.2)),
                    "deadend_penalty": float(getattr(args, "sleep_edge_deadend_penalty", 0.2)),
                    "blocked_penalty": float(getattr(args, "sleep_edge_blocked_penalty", 0.2)),
                    "mode": str(getattr(args, "sleep_edge_mode", "mul")),
                },
                "event_prior": {
                ...
                "affordance_memory": {
                    "enabled": bool(getattr(args, "affordance_bias", False)),
...
class HopGreedyEvaluator:
    ...
    def _allpairs_sp(self, anchors: Set[Tuple[int,int,int]], g_before: nx.Graph, g_after: nx.Graph, hop: int) -> float:
        core = self.core
        sp_g1, nodes_sp1 = extract_k_hop_subgraph(g_before, anchors, hop)
        sp_g2, nodes_sp2 = extract_k_hop_subgraph(g_after, anchors, hop)
        if str(core.sp_scope_mode).lower() in ("union","merge","superset"):
            all_nodes = set(nodes_sp1) | set(nodes_sp2)
            if all_nodes:
                sp_g1 = g_before.subgraph(all_nodes).copy()
                sp_g2 = g_after.subgraph(all_nodes).copy()
        if str(core.sp_boundary_mode).lower() in ("trim","terminal","nodes"):
            sp_g1 = trim_terminal_edges(sp_g1, anchors, hop)
            sp_g2 = trim_terminal_edges(sp_g2, anchors, hop)
        try:
            return float(compute_sp_gain_norm(sp_g1, sp_g2, mode=core.sp_norm_mode))
        except Exception:
            return 0.0
