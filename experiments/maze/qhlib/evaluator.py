...
def evaluate_multihop(
    *,
    ...
    if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
        ...
        print(f"[DEBUG] g_before_for_expansion.edges()={list(g_before_for_expansion.edges())[:10]}...", flush=True)
    ...
    def _sp_gain_fixed_pairs(sub_before: nx.Graph, sub_after: nx.Graph, eff_hop: int = 1) -> float:
        ...
        g1 = sub_before
        g2 = sub_after
        # DEBUG: Print before scope/bound processing
        import os
        if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
            print(f"[DEBUG] _sp_gain_fixed_pairs INPUT sub_before.nodes={sub_before.number_of_nodes()} sub_before.edges={sub_before.number_of_edges()} sub_after.nodes={sub_after.number_of_nodes()} sub_after.edges={sub_after.number_of_edges()}")
            print(f"[DEBUG] _sp_gain_fixed_pairs scope={scope} bound={bound} eff_hop={eff_hop}")
            print(f"[DEBUG] _sp_gain_fixed_pairs anchors_core={anchors_core}")
            print(f"[DEBUG] _sp_gain_fixed_pairs sub_before.nodes()={list(sub_before.nodes())[:5]}...")
        ...
        if bound in ("trim", "terminal", "nodes"):
            # DEBUG: Print before trim
            if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
                print(f"[DEBUG] _sp_gain_fixed_pairs BEFORE TRIM g1.nodes={g1.number_of_nodes()} g1.edges={g1.number_of_edges()} g2.edges={g2.number_of_edges()}")
                # Check if anchors are in g1
                anchors_in_g1 = [a for a in anchors_core if a in g1]
                anchors_in_g2 = [a for a in anchors_core if a in g2]
                print(f"[DEBUG] _sp_gain_fixed_pairs anchors_in_g1={anchors_in_g1} anchors_in_g2={anchors_in_g2}")
                print(f"[DEBUG] _sp_gain_fixed_pairs g2.edges before trim={list(g2.edges())[:5]}")
            ...
            # DEBUG: Print after trim
            if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
                print(f"[DEBUG] _sp_gain_fixed_pairs AFTER TRIM g1.nodes={g1.number_of_nodes()} g1.edges={g1.number_of_edges()} g2.edges={g2.number_of_edges()}")
                print(f"[DEBUG] _sp_gain_fixed_pairs g2.edges after trim={list(g2.edges())[:5]}")
        try:
            result = float(core._compute_sp_gain_norm(g1, g2, mode=core.sp_norm_mode))
            # DEBUG: Print SP gain calculation
            if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
                print(f"[DEBUG] _sp_gain_fixed_pairs g1.nodes={g1.number_of_nodes()} g1.edges={g1.number_of_edges()} g2.nodes={g2.number_of_nodes()} g2.edges={g2.number_of_edges()} result={result:.4f}")
            return result
        except Exception as ex:
            import os
            if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
                print(f"[DEBUG] _sp_gain_fixed_pairs EXCEPTION: {ex}")
            return 0.0
    ...
    def _ensure_pairs_state(eff: int) -> Tuple[nx.Graph, float, List[Tuple[object, object, float]], str]:
        ...
        # DEBUG: Print subgraph info
        import os
        if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
            print(f"[DEBUG] _ensure_pairs_state eff={eff} nodes_b={len(nodes_b)} sp_b.nodes={sp_b.number_of_nodes()} sp_b.edges={sp_b.number_of_edges()}")
        ...
        if eff not in pairs_by_eff:
            ...
            if ps_loaded is not None and ps_loaded.pairs:
                ...
                # DEBUG: Print pairset info
                import os
                if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
                    print(f"[DEBUG] _ensure_pairs_state eff={eff} pairset.pairs={len(pairset.pairs)} lb_avg={pairset.lb_avg:.4f}")
    ...
    for h in range(1, max(0, int(max_hops)) + 1):
        # DEBUG: Print loop entry
        import os
        if os.getenv('INSIGHTSPIKE_DEBUG_EVAL'):
            print(f"[DEBUG] hop loop h={h} sp_allpairs_exact={sp_allpairs_exact} sp_cache={sp_cache} sp_cache_mode={sp_cache_mode}")
        best_delta = 0.0
        ...
        if (not sp_allpairs) and (not sp_allpairs_exact) and sp_cache and str(sp_cache_mode).lower() == 'cached_incr' and best_item is not None:
            ...
            if (add_edges > add_nodes) or (sp_fp <= 0.0 and not sp_signed) or (sp_signed and sp_fp <= 1e-12):
                ...
                # sp_fp > 0: use fixed-pairs result directly
                sp_h = sp_fp
                sp_mode_used = 'fp'
                sp_imp_cnt = 0; sp_imp_ex = []
