...
class SQLiteStore:
    ...
    def upsert_nodes(self, nodes_meta: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Insert or update node attributes (merge with existing attributes)."""
        saved: List[Dict[str, Any]] = []
        with self._conn() as conn:
            cur = conn.cursor()
            for nm in nodes_meta:
                ...
                node_id = f"{int(rid)},{int(cid)},{int(did)}"
                attrs = {k: v for k, v in nm.items() if k not in ('node',)}
                # Fetch existing attributes (id is global primary key)
                cur.execute("SELECT attributes, node_type FROM graph_nodes WHERE id=?", (node_id,))
                row = cur.fetchone()
                if row:
                    try:
                        prev_attrs = json.loads(row[0]) if row[0] else {}
                    except Exception:
                        prev_attrs = {}
                    merged = dict(prev_attrs)
                    merged.update(attrs)
                    node_type = attrs.get('node_type') or row[1] or 'auto'
                    cur.execute(
                        "UPDATE graph_nodes SET node_type=?, attributes=? WHERE id=?",
                        (node_type, json.dumps(merged), node_id)
                    )
                else:
                    node_type = nm.get('node_type') or 'auto'
                    cur.execute(
                        "INSERT INTO graph_nodes (id, namespace, node_type, attributes) VALUES (?, ?, ?, ?)",
                        (node_id, self.namespace, node_type, json.dumps(attrs))
                    )
                saved.append({"id": node_id, **attrs})
            conn.commit()
        return saved

    def load_node_attributes(self) -> Dict[str, Dict[str, Any]]:
        """Load node attributes for this namespace."""
        out: Dict[str, Dict[str, Any]] = {}
        with self._conn() as conn:
            cur = conn.cursor()
            cur.execute("SELECT id, attributes FROM graph_nodes WHERE namespace=?", (self.namespace,))
