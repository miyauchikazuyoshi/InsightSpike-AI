...
from .core.layers.layer1_error_monitor import KnownUnknownAnalysis, analyze_input
from .detection.eureka_spike import EurekaDetector
from .detection.insight_registry import InsightFactRegistry
from .learning.adaptive_topk import (
...
def cycle(
    memory, question: str, g_old=None, top_k=10, device=None, **kwargs
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Processing question with legacy cycle function: {question[:50]}..."
        )
        ...
        logger.info(
            "🔍 Layer1: Analyzing input for known/unknown information separation"
        )
        ...
        if hasattr(memory, "episodes") and memory.episodes:
            context_docs = [
                getattr(ep, "text", str(ep)) for ep in memory.episodes[:20]
            ]  # Sample for analysis
        ...
        adaptive_topk = {
            k: v
            for k, v in adaptive_topk_result.items()
            if not k.startswith("adaptation")
        }
        adaptation_factors = adaptive_topk_result["adaptation_factors"]

        # Estimate chain reaction potential
        chain_reaction_potential = estimate_chain_reaction_potential(
            l1_analysis.__dict__, adaptive_topk
        )

        logger.info(
            f"L1 Analysis - Known: {len(l1_analysis.known_elements)}, "
        ...
        logger.info(
            f"Adaptive topK - L1:{adaptive_topk['layer1_k']}, "
        ...
        if hasattr(memory, "episodes") and memory.episodes:
            logger.info(
                f"Migrating {len(memory.episodes)} episodes to new memory system"
            )
        ...
        enhanced_processing = (
            l1_analysis.query_complexity > 0.6 or chain_reaction_potential > 0.5
        )

        logger.info(
            f"Processing with {max_cycles} max cycles, enhanced={enhanced_processing}, "
        ...
        try:
            ...
            if (
                response_text and reasoning_quality > 0.3
            ):  # Only process quality responses
                ...
                if discovered_insights:
                    logger.info(
                        f"🧠 Discovered {len(discovered_insights)} insights from response"
                    )
                    for insight in discovered_insights:
                        logger.info(
                            f"  - {insight.relationship_type}: {insight.text[:80]}..."
                        )
                        logger.info(
        ...
        legacy_result = {
            ...
            "insight_quality_avg": sum(i.quality_score for i in discovered_insights)
            / len(discovered_insights)
            if discovered_insights
            else 0.0,
            "insight_optimization_avg": sum(
                i.ged_optimization for i in discovered_insights
            )
            / len(discovered_insights)
            if discovered_insights
            else 0.0,
        ...
        logger.info(
            f"Legacy cycle completed - Quality: {legacy_result['reasoning_quality']:.3f}, "
    ...
    for i, question in enumerate(questions):
        try:
            ...
            if result.get("reasoning_quality", 0) > 0.8:
                logger.info(
                    f"High quality achieved at iteration {i+1}, stopping adaptive loop"
                )
                break
