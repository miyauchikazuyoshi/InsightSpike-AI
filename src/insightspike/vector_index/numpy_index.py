...
class OptimizedNumpyIndex(NumpyNearestNeighborIndex):
    ...
    def radius_search(self, queries: np.ndarray, radius: float, 
        ...
        """Optimized radius search using vectorized operations.
        
        Args:
            queries: Array of shape (n_queries, dimension)
            radius: For normalize=True: minimum similarity threshold
                   For normalize=False: maximum L2 distance threshold
            max_results: Optional maximum number of results per query
            
        Returns:
            distances: List of arrays with distances/similarities for each query
            indices: List of arrays with indices for each query
        """
        if self.vectors is None or len(self.vectors) == 0:
            return [np.array([]) for _ in range(len(queries))], [np.array([]) for _ in range(len(queries))]
        ...
        if self.normalize:
            ...
            # Batch matrix multiplication for all similarities
            similarities = np.dot(normalized_queries, self._normalized_vectors.T)
            ...
            for i, sim in enumerate(similarities):
                ...
                valid_similarities = sim[mask]
                
                # Sort by similarity (descending) and limit results
                sorted_idx = np.argsort(valid_similarities)[::-1]
                if max_results is not None and len(sorted_idx) > max_results:
                    sorted_idx = sorted_idx[:max_results]
                
                distances_list.append(valid_similarities[sorted_idx])
                indices_list.append(self.ids[valid_indices[sorted_idx]])
        else:
            # L2 distance search
            for query in queries:
                ...
                mask = l2_distances <= radius
                valid_indices = np.where(mask)[0]
                valid_distances = l2_distances[mask]
                ...
                distances_list.append(valid_distances[sorted_idx])
                indices_list.append(self.ids[valid_indices[sorted_idx]])
