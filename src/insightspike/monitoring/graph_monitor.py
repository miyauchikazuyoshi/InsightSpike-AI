...
class GraphOperationMonitor:
    ...
    def start_operation(
        self, operation: str, graph_state: Dict[str, int]
    ) -> Dict[str, Any]:
    ...
    def detect_anomalies(self) -> List[Dict[str, Any]]:
        ...
        for operation, stats in self.operation_stats.items():
            ...
            recent_ops = [m for m in self.metrics_history if m.operation == operation][
                -10:
            ]
...
class MonitoredOperation:
    ...
    def __enter__(self):
        self.context = self.monitor.start_operation(
            self.operation, self.graph_state_func()
        )
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.monitor.end_operation(
            self.context, self.graph_state_func(), self.metadata, self.error
        )
