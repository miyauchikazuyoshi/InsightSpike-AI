...
class ReasoningConfig(BaseModel):
    ...
    conflict_threshold: float = Field(default=0.6, ge=0.0, le=1.0)  # Kept for backward compatibility
    weight_ged: float = Field(default=1.0, ge=0.0)
    weight_ig: float = Field(default=1.0, ge=0.0)
    weight_conflict: float = Field(default=0.0, ge=0.0)  # Deprecated, defaults to 0
...
class LLMConfig(BaseModel):
    ...
    # Prompt customization for lightweight models
    use_simple_prompt: bool = Field(
        default=False,
        description="Use simplified prompt for lightweight models like GPT-2"
    )
    prompt_template: Optional[str] = Field(
        default=None, 
        description="Custom prompt template with {context} and {question} placeholders"
    )
    prompt_style: Literal["minimal", "standard", "detailed"] = Field(
        default="standard",
