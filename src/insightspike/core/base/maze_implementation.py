    ...
    def encode_state(self, state: EnvironmentState) -> np.ndarray:
        ...
        encoded = np.concatenate(
            [
                maze_flat,
                [
                    pos_x / self.maze_size,
                    pos_y / self.maze_size,
                    goal_x / self.maze_size,
                    goal_y / self.maze_size,
                ],
            ]
        )
...
class MazeRewardNormalizer(RewardNormalizer):
    ...
    def normalize_reward(self, reward: float, context: Dict[str, Any]) -> float:
        ...
        normalized = (
            2 * (clamped - self.min_reward) / (self.max_reward - self.min_reward) - 1
        )
        return normalized
...
class MazeInsightDetector(InsightDetectorInterface):
    ...
    def detect_insight(
        ...
        if dged < self.dged_threshold and dig > self.dig_threshold:
            ...
            description = (
                f"Navigation strategy breakthrough: ΔGED={dged:.3f}, ΔIG={dig:.3f}"
            )
        ...
        # Goal discovery condition
            ...
            description = (
                f"Goal discovery insight: reached target with reward={reward:.1f}"
            )
    ...
    def update_context(self, state: EnvironmentState, action: Any, reward: float):
        ...
        self.state_visit_count[current_pos] = (
            self.state_visit_count.get(current_pos, 0) + 1
        )
