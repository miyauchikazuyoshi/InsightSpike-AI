...
class L2MemoryManager(L2MemoryInterface):
    ...
    def save(self, path: Optional[Path] = None) -> bool:
        """Save memory state (episodes and index) to disk."""
        try:
            if path is None:
                # Use config to determine save path
                config = get_core_config()
                index_path = Path(config.memory.index_file)
                episodes_path = index_path.parent / "episodes.json"
            else:
            ...
            if self.is_trained and len(self.episodes) > 0:
                    ...
                    episodes_data.append(
                        {
                            ...
                            "c": episode.c,
                            "metadata": episode.metadata,
                            "vec": episode.vec.tolist(),  # Convert numpy to list for JSON
                        }
                    )
                ...
                logger.info(f"Saved {len(episodes_data)} episodes to {episodes_path}")
                return True
            else:
                logger.warning("No trained index or episodes to save")
                return False
    ...
    def load(self, path: Optional[Path] = None) -> bool:
        """Load memory state (episodes and index) from disk."""
        try:
            if path is None:
                # Use config to determine load path
                config = get_core_config()
                index_path = Path(config.memory.index_file)
                episodes_path = index_path.parent / "episodes.json"
            else:
            ...
            if episodes_path.exists():
                ...
                for ep_data in episodes_data:
                    vec = np.array(ep_data["vec"], dtype=np.float32)
                    episode = Episode(
                        vec=vec,
                        text=ep_data["text"],
                        c=ep_data["c"],
            ...
            # Load Faiss index
            if index_path.exists() and len(self.episodes) > 0:
                ...
                # Retrain index with loaded episodes
        ...
        try:
            ...
            for i, episode in enumerate(self.episodes):
                ...
                if len(new_words) == 0 or len(existing_words) == 0:
                    ...
                    content_overlap = len(new_words.intersection(existing_words)) / len(
                        new_words.union(existing_words)
                    )
    ...
    def _integrate_with_existing(
        ...
        try:
            ...
            if total_weight > 0:
                ...
                integrated_vector = (
                    weight_existing * existing_episode.vec + weight_new * new_vector
                )
            else:
