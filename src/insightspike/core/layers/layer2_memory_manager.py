...
class L2MemoryManager(L2MemoryInterface):
    ...
    def _train_index(self):
        ...
        try:
            ...
            self.index = faiss.IndexIVFPQ(quantizer, self.dim, n_clusters, self.pq_segments, 8)
            self.index.train(vecs)
            self.index.add(vecs)
            self.is_trained = True
            logger.info(f"Trained IVF-PQ index with {len(self.episodes)} episodes, {n_clusters} clusters")
