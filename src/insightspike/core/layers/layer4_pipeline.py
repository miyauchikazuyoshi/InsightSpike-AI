...
class Layer4Pipeline:
    ...
    def generate_response(
        ...
        layer4_input = {"context": context, "question": question, "mode": mode}
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "layer4_config": {
                "direct_generation_enabled": getattr(
                    self.config.llm, "use_direct_generation", False
                ),
                "threshold": getattr(
                    self.config.llm, "direct_generation_threshold", 0.7
                ),
    ...
    def set_mode(self, mode: str):
        ...
        if mode == "direct":
            ...
            self.force_direct = False
            self.enable_polish = True
            self.layer4_1.polish_threshold = getattr(
                self.config.llm, "polish_threshold", 0.6
            )
        else:
