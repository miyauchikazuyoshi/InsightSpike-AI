...
class RealOpenAIProvider:
    ...
    def __init__(self, model_name: str = "gpt-3.5-turbo", temperature: float = 0.7):
        if not OPENAI_AVAILABLE:
            raise ImportError("OpenAI SDK not available. Install with: poetry install -E llm")
    ...
    def _estimate_quality(self, response: str, question: str) -> Dict[str, float]:
        ...
        # Insight indicators
        insight_keywords = ["because", "therefore", "however", "although", "relationship", 
                          "connection", "implies", "suggests", "indicates", "reveals"]
        insight_score = sum(1 for kw in insight_keywords if kw in response_lower) / len(insight_keywords)
...
class RealAnthropicProvider:
    ...
    def __init__(self, model_name: str = "claude-3-sonnet-20240229", temperature: float = 0.7):
        if not ANTHROPIC_AVAILABLE:
            raise ImportError("Anthropic SDK not available. Install with: poetry install -E llm")
...
class RealOllamaProvider:
    ...
    def __init__(self, model_name: str = "llama2", temperature: float = 0.7):
        if not OLLAMA_AVAILABLE:
            raise ImportError("Ollama SDK not available. Install with: poetry install -E llm")
