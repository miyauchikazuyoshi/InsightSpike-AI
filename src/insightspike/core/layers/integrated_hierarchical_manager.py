...
class IntegratedHierarchicalManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Integrated Hierarchical Manager initialized: "
            f"cluster_size={cluster_size}, rebuild_threshold={rebuild_threshold}"
        )

    def add_episode(
        self, vector: np.ndarray, text: str, metadata: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        エピソード追加（階層的管理付き）
        ...
        if (
            len(self.memory_manager.episodes)
    ...
    def search(self, query: str, k: int = 10) -> List[Dict[str, Any]]:
        ...
        if self.graph_builder.stats["total_nodes"][0] > 0:
            # 階層構造を使った効率的な検索
            hierarchical_results = self.graph_builder.search_hierarchical(
                query_vector, k=k * 2
            )
    ...
    def _should_rebuild(self) -> bool:
        ...
        # メモリ使用効率が悪化（圧縮率が低下）
        if self.graph_builder.stats["total_nodes"][0] > 0:
            compression_ratio = self.graph_builder.stats["total_nodes"][0] / max(
                1, self.graph_builder.stats["total_nodes"][2]
            )
        ...
        return {
            ...
            "integration": {
                ...
                "compression_ratio": (
                    ...
                    if hierarchy_stats["nodes_per_level"][0] > 0
                    else 0
                ),
            },
        }
    ...
    def load_state(self, path: str):
        ...
        self.graph_builder.child_to_parent = state["hierarchy_mappings"][
            "child_to_parent"
        ]
        self.graph_builder.parent_to_children = state["hierarchy_mappings"][
            "parent_to_children"
        ]
    ...
    def visualize_hierarchy(self) -> Dict[str, Any]:
        ...
        # 階層間接続のサンプル
        for (child_level, child_idx), (parent_level, parent_idx) in list(
            self.graph_builder.child_to_parent.items()
        )[:20]:
