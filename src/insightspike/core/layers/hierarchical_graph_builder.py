...
class HierarchicalGraphBuilder:
    ...
    def __init__(self,
        ...
        # 階層間マッピング
        self.child_to_parent: Dict[Tuple[int, int], Tuple[int, int]] = {}  # (level, idx) -> (parent_level, parent_idx)
        self.parent_to_children: Dict[Tuple[int, int], List[Tuple[int, int]]] = defaultdict(list)
        ...
        logger.info(f"Hierarchical Graph Builder initialized: "
                   f"cluster_size={cluster_size}, super_cluster_size={super_cluster_size}")
    
    def build_hierarchical_graph(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        階層的グラフを構築
        ...
        for level in self.levels:
            if level.nodes:
                ...
                self.stats['total_edges'][level.level] = level.graph.edge_index.size(1) if level.graph else 0
    ...
    def _build_level_0(self, documents: List[Dict[str, Any]]):
        ...
        if self.levels[0].nodes:
            embeddings = np.array([n['embedding'] for n in self.levels[0].nodes], dtype=np.float32)
    ...
    def _build_level_1(self):
        ...
        for i in range(n_clusters):
            ...
            if len(cluster_members) > 0:
                ...
                # メンバーのテキストを要約（簡易版）
                member_texts = [self.levels[0].nodes[idx]['text'][:50] for idx in cluster_members[:3]]
        ...
        if self.levels[1].nodes:
            l1_embeddings = np.array([n['embedding'] for n in self.levels[1].nodes], dtype=np.float32)
    ...
    def _build_level_2(self):
        ...
        if len(self.levels[1].nodes) <= self.super_cluster_size:
            ...
            if self.levels[1].nodes:
                all_embeddings = np.array([n['embedding'] for n in self.levels[1].nodes], dtype=np.float32)
            ...
            # Level 1をさらにクラスタリング
            embeddings = np.array([n['embedding'] for n in self.levels[1].nodes], dtype=np.float32)
            n_super_clusters = max(1, len(self.levels[1].nodes) // self.super_cluster_size)
            
            kmeans = faiss.Kmeans(self.dimension, n_super_clusters, niter=20, verbose=False)
            kmeans.train(embeddings)
            ...
            for i in range(n_super_clusters):
                ...
                if len(cluster_members) > 0:
                    ...
                    # 統計情報
                    total_episodes = sum(len(self.levels[1].nodes[idx]['metadata']['members']) 
        ...
        if self.levels[2].nodes:
            l2_embeddings = np.array([n['embedding'] for n in self.levels[2].nodes], dtype=np.float32)
    ...
    def _build_graph_for_level(self, level: GraphLevel) -> Data:
        """各レベルのグラフ構築"""
        if not level.nodes:
            return Data(x=torch.empty(0, self.dimension), edge_index=torch.empty(2, 0, dtype=torch.long))
            ...
            for i in range(0, len(level.nodes), batch_size):
                ...
                for j, (dist_row, neigh_row) in enumerate(zip(distances, neighbors)):
                    ...
                    for dist, dst_idx in zip(dist_row[1:], neigh_row[1:]):  # Skip self
                        if dist > self.similarity_threshold and dst_idx < len(level.nodes):
    ...
    def search_hierarchical(self, query_vector: np.ndarray, k: int = 10) -> List[Dict[str, Any]]:
        """
        階層的検索
        ...
        if self.levels[2].index and self.levels[2].nodes:
            ...
            for idx in indices2[0]:
                if idx < len(self.levels[2].nodes):
                    ...
                    level1_candidates.extend([child[1] for child in children if child[0] == 1])
            
            # Level 1候補から検索
            if level1_candidates and self.levels[1].index:
                candidate_embeddings = np.array([self.levels[1].nodes[i]['embedding'] 
                                               for i in level1_candidates], dtype=np.float32)
                ...
                for local_idx in indices1[0]:
                    if local_idx < len(level1_candidates):
                        ...
                        level0_candidates.extend([child[1] for child in children if child[0] == 0])
                
                # 最終的なエピソード検索
                if level0_candidates:
                    candidate_embeddings = np.array([self.levels[0].nodes[i]['embedding'] 
                                                   for i in level0_candidates], dtype=np.float32)
    ...
    def add_document(self, document: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.levels[1].index and self.levels[1].nodes:
            ...
            if indices[0][0] < len(self.levels[1].nodes):
                ...
                # クラスタが大きくなりすぎたら分割を検討
                if self.levels[1].nodes[cluster_idx]['metadata']['size'] > self.cluster_size * 2:
                    logger.info(f"Cluster {cluster_idx} is too large, consider rebalancing")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for i in range(len(self.levels) - 1):
            if self.stats['total_nodes'][i] > 0:
                ratio = self.stats['total_nodes'][i] / max(1, self.stats['total_nodes'][i + 1])
