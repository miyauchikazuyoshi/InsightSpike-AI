...
class CleanLLMProvider:
    ...
    def generate_response(self, context: Dict[str, Any], prompt: str) -> Dict[str, Any]:
        ...
        reasoning_quality = self._calculate_fair_reasoning_quality(response, insight_analysis)
    ...
    def _calculate_fair_confidence(self, response: str, context: Dict[str, Any]) -> float:
        """
    ...
    def _analyze_insights_fairly(self, prompt: str, response: str) -> Dict[str, Any]:
        ...
        complexity_words = [
            "complex", "analysis", "principles", "relationships", "systematic", 
            "comprehensive", "multifaceted", "framework", "perspective", "theoretical"
        ]
        
        complexity_score = sum(1 for word in complexity_words if word in response_lower) / len(complexity_words)
        
        # Generic synthesis indicators (not content-specific)
        synthesis_words = [
            "connecting", "integration", "relationship", "perspective", "framework",
            "analysis", "systematic", "comprehensive", "multifaceted"
        ]
        
        synthesis_score = sum(1 for word in synthesis_words if word in response_lower) / len(synthesis_words)
    ...
    def _calculate_fair_reasoning_quality(self, response: str, insight_analysis: Dict[str, Any]) -> float:
        """
...
def get_clean_llm_provider(config=None, provider_type: str = "clean") -> CleanLLMProvider:
    """
...
def verify_no_data_leaks(provider: CleanLLMProvider, test_questions: List[str]) -> Dict[str, Any]:
    """
    ...
    for question in test_questions:
        ...
        response_similarity = len(set(responses)) / len(responses)  # Should be > 0.3 for fair variation
        performance_variation = np.std(performances)  # Should be > 0.01 for realistic variation
        
        # More lenient criteria since we use generic templates
        suspicious = (
            response_similarity < 0.3 or  # Allow some repetition in generic responses
            performance_variation < 0.01 or  # Allow smaller variation
            np.mean(performances) > 0.95  # Flag unrealistically high performance
        )
