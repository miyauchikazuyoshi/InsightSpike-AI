...
class L2EnhancedScalableMemory(L2MemoryManager):
        ...
        if self.use_scalable_graph:
            self.scalable_graph = ScalableGraphManager(
                ...
                conflict_threshold=getattr(
                    self.config.reasoning, "conflict_threshold", 0.8
                ),
            )
            logger.info(
                "Initialized ScalableGraphManager for enhanced memory operations"
            )
        else:
            ...
            if self.use_scalable_graph and self.episodes:
                ...
                if graph_result["success"]:
                    ...
                    if conflicts:
                        ...
                        logger.info(
                            f"Detected {len(conflicts)} conflicts for episode {latest_idx}"
                        )
                    ...
                    if "importance" in graph_result:
                        self._update_episode_importance(
                            latest_idx, graph_result["importance"]
                        )
    ...
    def _handle_conflict_split(self, episode_idx: int, conflicts: List[Dict[str, Any]]):
        ...
        try:
            ...
            if split_indices:
                logger.info(
                    f"Split episode {episode_idx} into {len(split_indices)} new episodes"
                )
    ...
    def split_by_conflict(
        self, episode_index: int, conflict_candidates: List[Tuple[int, float]]
    ) -> List[int]:
        """
        ...
        try:
            if not (0 <= episode_index < len(self.episodes)):
                logger.warning(
                    f"Invalid episode index for conflict split: {episode_index}"
                )
                return []
            ...
            logger.info(
                f"Split episode {episode_index} into {len(new_indices)} parts due to conflicts"
            )
            return new_indices
    ...
    def _update_episode_importance(self, episode_idx: int, importance: float):
        ...
        if 0 <= episode_idx < len(self.episodes):
            ...
            episode.c = new_c
            logger.debug(
                f"Updated episode {episode_idx} C-value based on graph importance: {new_c:.3f}"
            )
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "edges": graph.edge_index.size(1) // 2
            if graph.edge_index.numel() > 0
            else 0,
            "density": (
                graph.edge_index.size(1) / (graph.num_nodes * (graph.num_nodes - 1))
            )
    ...
    def search_episodes_with_graph(
        ...
        for result in base_results:
            ...
            enhanced_score = (
                0.5 * result["weighted_score"]
                + 0.3 * result["similarity"]  # Original weighted score
                + 0.2 * importance  # Pure similarity  # Graph importance
            )
