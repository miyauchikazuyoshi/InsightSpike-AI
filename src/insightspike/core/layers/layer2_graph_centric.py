...
class GraphCentricMemoryManager(L2MemoryManager):
    ...
    def add_episode(self, vector: np.ndarray, text: str, c_value: float = None) -> int:
        ...
        if integration_result["should_integrate"]:
            ...
            self._integrate_episodes(
                target_idx, vector, text, integration_result.get("weight", 0.5)
            )
    ...
    def _check_integration(self, vector: np.ndarray, text: str) -> Dict[str, Any]:
        ...
        should_integrate = (
            best_match["combined_score"] >= threshold
            and best_match["content_overlap"]
        ...
        return {
            ...
            "weight": best_match["graph_weight"]
            if best_match["graph_weight"] > 0
            else best_match["similarity"],
            "graph_assisted": best_match["graph_weight"] > 0,
            "scores": best_match,
        }

    def _integrate_episodes(
        self, target_idx: int, new_vec: np.ndarray, new_text: str, weight: float
    ):
        """
        エピソード統合（C値なし、重みベース）
        """
        ...
        # メタデータ更新
        target.metadata["integration_count"] = (
            target.metadata.get("integration_count", 0) + 1
        )
    ...
    def _split_episode(self, idx: int, reason: str):
        ...
        for i in range(n_splits):
            ...
            metadata.update(
                {"split_from": idx, "split_reason": reason, "split_part": i + 1}
            )
    ...
    def get_importance(self, episode_idx: int) -> float:
        ...
        if self.l3_graph and hasattr(self.l3_graph, "previous_graph"):
            ...
            if graph and hasattr(graph, "edge_index"):
                ...
                degree = (edge_index[0] == episode_idx).sum() + (
        ...
        importance = (
            0.4 * graph_degree
            + 0.3 * access_score  # グラフ構造
            + 0.3 * time_decay  # アクセス頻度  # 時間的関連性
        )
    ...
    def _update_access(self, episode_idx: int):
        ...
        if 0 <= episode_idx < len(self.episodes):
            episode = self.episodes[episode_idx]
            episode.metadata["access_count"] = (
                episode.metadata.get("access_count", 0) + 1
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "integration_rate": self.stats["total_integrations"]
            / max(1, self.stats["total_episodes_added"]),
            "graph_assist_rate": self.stats["graph_assisted_integrations"]
            / max(1, self.stats["total_integrations"]),
