...
class EnhancedL2MemoryManager(L2MemoryManager):
    ...
    def add_episode(self, vector: np.ndarray, text: str, c_value: float = 0.2) -> int:
        ...
        try:
            ...
            integration_result = self._check_episode_integration_enhanced(vector, text, c_value)
    ...
    def _check_episode_integration_enhanced(self, vector: np.ndarray, text: str, c_value: float) -> Dict[str, Any]:
        """
        ...
        if not self.integration_config.enable_graph_integration or self.l3_graph is None:
        ...
        combined_score = vector_weight * original_score + graph_weight * graph_connection
        ...
        should_integrate = (
            ...
            best_candidate["content_overlap"] >= self.integration_config.content_overlap_threshold
        )
    ...
    def _perform_episode_split(self, episode_idx: int, reason: str):
        ...
        logger.info(f"Split episode {episode_idx} into {len(new_episodes)} parts (reason: {reason})")
...
def upgrade_to_enhanced(existing_manager: L2MemoryManager) -> EnhancedL2MemoryManager:
    ...
    logger.info(f"Upgraded to enhanced memory manager with {len(enhanced.episodes)} episodes")
