...
class ConflictScore:
    ...
    def calculate_conflict(self, graph_old: Data, graph_new: Data, 
        ...
        try:
            ...
            return {
                ...
                'total': float(structural_conflict + semantic_conflict + temporal_conflict) / 3
            }
    ...
    def _temporal_conflict(self, context: Dict[str, Any]) -> float:
        ...
        if 'previous_confidence' in context and 'current_confidence' in context:
            conf_diff = abs(context['previous_confidence'] - context['current_confidence'])
...
class GraphBuilder:
    ...
    def build_graph(self, documents: List[Dict[str, Any]], 
                   embeddings: Optional[np.ndarray] = None) -> Data:
        ...
        try:
            ...
            logger.debug(f"Built graph with {graph.num_nodes} nodes, {graph.edge_index.size(1)} edges")
            return graph
...
class L3GraphReasoner(L3GraphReasonerInterface):
    ...
    def _calculate_metrics(self, current_graph: Data, previous_graph: Optional[Data]) -> Dict[str, float]:
    ...
    def _calculate_reward(self, metrics: Dict[str, float], conflicts: Dict[str, float]) -> Dict[str, float]:
        ...
        base_reward = (w1 * metrics.get('delta_ged', 0) + 
    ...
    def _novelty_reward(self, metrics: Dict[str, float], conflicts: Dict[str, float]) -> float:
    ...
    def _detect_spike(self, metrics: Dict[str, float], conflicts: Dict[str, float]) -> bool:
    ...
    def _assess_reasoning_quality(self, metrics: Dict[str, float], conflicts: Dict[str, float]) -> float:
    ...
    def _process_with_gnn(self, graph: Data) -> Optional[torch.Tensor]:
        ...
        try:
            with torch.no_grad():
                ...
                graph_repr = global_mean_pool(x, torch.zeros(graph.num_nodes, dtype=torch.long))
                return graph_repr
    ...
    def _fallback_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'conflicts': {'structural': 0.0, 'semantic': 0.0, 'temporal': 0.0, 'total': 0.0},
