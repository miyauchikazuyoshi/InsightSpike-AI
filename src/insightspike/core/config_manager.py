...
class InsightDetectionConfig:
    ...
    weights: Dict[str, float] = field(
        default_factory=lambda: {"ged": 0.4, "ig": 0.3, "conflict": 0.3}
    )
    thresholds: Dict[str, float] = field(
        default_factory=lambda: {"ged_threshold": -0.5, "ig_threshold": 0.2}
    )
...
class InsightSpikeConfig:
    ...
    insight_detection: InsightDetectionConfig = field(
        default_factory=InsightDetectionConfig
    )
    ...
    def _apply_env_overrides(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if env_value is not None:
                # Type conversion
                if (
                    config_path.endswith("random_seed")
                    or config_path.endswith("hidden_dim")
                    or config_path.endswith("vector_dim")
                ):
                    try:
...
def save_config(
    config: InsightSpikeConfig, config_path: Union[str, Path], format: str = "yaml"
) -> None:
    """
