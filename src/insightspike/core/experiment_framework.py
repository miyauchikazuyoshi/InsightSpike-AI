...
class BaseExperiment(ABC):
    ...
    def _aggregate_metrics(
        ...
        return PerformanceMetrics(
            ...
            insight_detection_count=sum(
                [m.insight_detection_count for m in metrics_list]
            ),
            custom_metrics=self._merge_custom_metrics(
                [m.custom_metrics for m in metrics_list]
            ),
        )

    def _merge_custom_metrics(
        self, custom_metrics_list: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
...
def create_simple_experiment_config(
    name: str, description: str, test_cases: int = 10
) -> ExperimentConfig:
