...
class PatternDatabase:
    ...
    def get_similar_patterns(self, 
                           query_type: str, 
                           min_score: float = 0.7,
                           limit: int = 5) -> List[EvolutionPattern]:
...
class EvolutionTracker:
    ...
    def __init__(self, 
        ...
        self.db_path = db_path or Path.home() / '.insightspike' / 'evolution_patterns.db'
    ...
    def track_evolution(self,
                       ...
                       final_score: float = 0.0) -> EvolutionPattern:
    ...
    def suggest_exploration_strategy(self, 
                                   query: str,
                                   current_state: QueryState) -> Dict[str, Any]:
    ...
    def get_optimization_hints(self, 
                             current_state: QueryState,
                             transformation_history: QueryTransformationHistory) -> List[str]:
        ...
        if current_state.stage == "exploring" and len(current_state.absorbed_concepts) < 2:
            hints.append("Try to connect with more fundamental concepts")
        
        # Suggest multi-hop if single-hop isn't working
        if current_state.confidence < 0.3 and current_state.transformation_magnitude < 0.2:
...
class TrajectoryAnalyzer:
    ...
    def analyze_trajectory(self, 
        ...
        return {
            ...
            'confidence_correlation': float(np.corrcoef(
                range(len(confidences)), 
                confidences
            )[0, 1]) if len(confidences) > 1 else 0,
