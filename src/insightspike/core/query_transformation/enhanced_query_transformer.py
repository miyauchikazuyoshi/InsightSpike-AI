...
class QueryBranch:
    ...
    def merge_insights(self, other_branch: 'QueryBranch') -> List[str]:
        ...
        all_insights = set(self.current_state.insights + other_branch.current_state.insights)
...
class MultiHopGNN(nn.Module):
    ...
    def __init__(self, feature_dim: int = 384, hidden_dim: int = 256, num_hops: int = 3):
        super().__init__()
    ...
    def forward(self, node_features: torch.Tensor, adjacency: torch.Tensor, 
                query_idx: int, return_paths: bool = False) -> Tuple[torch.Tensor, torch.Tensor, Optional[List]]:
        """
        ...
        for hop in range(self.num_hops):
            ...
            if return_paths and hop > 0:
                ...
                query_connections = adjacency[query_idx] * (1 - torch.eye(num_nodes)[query_idx])
        ...
        if len(hop_outputs) > 1:
            ...
            hop_stack = torch.stack(hop_outputs, dim=0).unsqueeze(0)  # [1, hops, nodes, hidden]
            hop_stack = hop_stack.permute(0, 2, 1, 3).reshape(num_nodes, self.num_hops, -1)
...
class AdaptiveExplorer:
    ...
    def select_exploration_direction(self, 
                                   current_state: QueryState,
    ...
    def _calculate_direction_score(self, 
        ...
        exploration_count = sum(1 for _, d in self.exploration_history if d == direction)
        score -= exploration_count * 0.1
        
        # Favor directions with more connections
        direction_nodes = [n for n in knowledge_graph.nodes() 
        ...
        successful_count = sum(1 for pattern in self.successful_patterns 
...
class EnhancedQueryTransformer(QueryTransformer):
    ...
    def transform_query_advanced(self,
                               query_state: QueryState,
                               knowledge_graph: nx.Graph,
                               documents: List[Dict[str, Any]],
                               allow_branching: bool = True) -> Tuple[QueryState, List[QueryBranch]]:
        """Advanced query transformation with branching and multi-hop reasoning"""
        
        # Check if we should branch
        branches = []
        if allow_branching and self.enable_branching and self._should_branch(query_state):
        ...
        if self.use_multi_hop and len(knowledge_graph.nodes()) > 0:
            new_state = self._multi_hop_transform(query_state, knowledge_graph, documents)
        else:
    ...
    def _should_branch(self, query_state: QueryState) -> bool:
        ...
        if hasattr(query_state, 'edge_weights') and len(query_state.edge_weights) > 3:
            strong_connections = sum(1 for w in query_state.edge_weights.values() if w > 0.6)
    ...
    def _create_branches(self, 
                        parent_state: QueryState,
                        knowledge_graph: nx.Graph) -> List[QueryBranch]:
        """Create exploration branches"""
        
        branches = []
        directions = self._get_available_directions(knowledge_graph)[:self.max_branches]
        ...
        for i, direction in enumerate(directions):
            ...
            branch_state = QueryState(
                ...
                embedding=parent_state.embedding.clone() if parent_state.embedding is not None else None,
                stage="exploring",
    ...
    def _multi_hop_transform(self,
                           query_state: QueryState,
                           knowledge_graph: nx.Graph,
                           documents: List[Dict[str, Any]]) -> QueryState:
        """Perform multi-hop transformation"""
    ...
    def _extract_path_insights(self, 
