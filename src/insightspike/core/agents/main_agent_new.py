...
from ..layers.layer1_error_monitor import L1ErrorMonitor
from ..layers.layer2_memory_manager import L2MemoryManager
...
class MainAgent:
    ...
    def __init__(self, config=None):
        ...
        self.l2_memory = L2MemoryManager(config=config)
        self.l3_graph = L3GraphReasoner(config)
        self.l4_llm = get_llm_provider(config)
    ...
    def process_question(self, question: str, max_cycles: int = 10, verbose: bool = False) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing final results and reasoning trace
        """
    ...
    def _compile_results(self, results: List[CycleResult], converged: bool, verbose: bool) -> Dict[str, Any]:
        """Compile final results from all cycles"""
        if not results:
        ...
        final_dict = best_result.to_dict()
        final_dict.update({
