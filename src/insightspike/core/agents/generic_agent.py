...
from ..interfaces.generic_interfaces import (
    EnvironmentInterface, EnvironmentState, GenericAgentInterface,
    InsightDetectorInterface, InsightMoment, MemoryManagerInterface,
    ReasonerInterface, RewardNormalizer, StateEncoder, TaskType
)
...
class GenericMemoryManager(MemoryManagerInterface):
    ...
    def _calculate_similarity(self, state1: EnvironmentState, state2: EnvironmentState) -> float:
...
class GenericReasoner(ReasonerInterface):
    ...
    def analyze_insight_pattern(
        ...
        return {
            ...
            'insight_trend': 'increasing' if len(insights) > 5 and 
                           len(insights[-3:]) > len(insights[-6:-3]) else 'stable'
        }
    
    def predict_next_insight(
        self, 
...
class GenericInsightSpikeAgent(GenericAgentInterface):
    ...
    def __init__(
        ...
        super().__init__(agent_id, environment, insight_detector, state_encoder, reward_normalizer)
    ...
    def update(
        ...
        # Normalize reward
        context = {
            'exploration_ratio': next_state.metadata.get('exploration_ratio', 0) if next_state.metadata else 0
        }
        ...
        # Detect insights
        insight_context = {
            'distance_to_goal': next_state.metadata.get('distance_to_goal', 0) if next_state.metadata else 0,
        ...
        if insight:
            ...
            self.memory.store_experience(state, action, normalized_reward, next_state, insight)
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        base_summary.update({
            ...
            'avg_episode_steps': np.mean(self.episode_steps) if self.episode_steps else 0.0,
            'recent_performance': np.mean(list(self.recent_rewards)[-20:]) if len(self.recent_rewards) >= 20 else 0.0,
            'memory_stats': self.memory.get_memory_stats(),
            'insight_analysis': self.reasoner.analyze_insight_pattern(self.insight_moments)
        })
