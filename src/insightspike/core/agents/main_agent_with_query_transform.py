...
class MainAgentWithQueryTransform(MainAgent):
    ...
    def _process_with_transformation(self, 
                                   question: str, 
                                   transformation_history: QueryTransformationHistory) -> Dict[str, Any]:
    ...
    def _run_single_cycle_with_transformation(self,
                                            query_text: str,
                                            query_state: QueryState,
                                            transformation_history: QueryTransformationHistory,
                                            cycle: int) -> TransformationCycleResult:
        ...
        try:
            ...
            if self.l3_graph and retrieved_documents:
                ...
                reasoning_quality = graph_analysis.get("metrics", {}).get("reasoning_quality", 0.5)
                
                # If spike detected, enhance query state
                if spike_detected:
                    new_query_state.add_insight("Graph analysis detected insight spike")
                    new_query_state.confidence = min(1.0, new_query_state.confidence + 0.2)
        ...
        episode_data = {
            ...
            "metadata": {
                ...
                "transformation": {
                    ...
                    "confidence": result.query_state.confidence if result.query_state else 0,
                    "insights": result.query_state.insights if result.query_state else []
                }
            }
        }
