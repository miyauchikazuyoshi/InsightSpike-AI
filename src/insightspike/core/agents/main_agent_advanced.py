...
from .main_agent_with_query_transform import (
...
class MainAgentAdvanced(MainAgentWithQueryTransform):
    ...
    def _process_with_advanced_transformation(
        ...
        for cycle in range(self.max_cycles):
            ...
            cycle_result = self._run_advanced_cycle(
                question, current_state, main_history, cycle
            )
            ...
            if cycle_result.branches:
                ...
                logger.info(
                    f"Created {len(cycle_result.branches)} exploration branches"
                )
    ...
    def _run_advanced_cycle(
        ...
        try:
            ...
            retrieved_documents = self.l2_memory.search_episodes(
                self._get_effective_query(current_state),
                k=7,  # More documents for multi-hop
            )
    ...
    def _explore_branch(
        ...
        logger.info(
            f"Exploring branch: {branch.branch_id} ({branch.exploration_direction})"
        )
        ...
        if branch_insights:
            self.query_transformer.adaptive_explorer.update_success_pattern(
                [branch.exploration_direction], branch.current_state.confidence
            )

        return branch_insights

    def _perform_multi_hop_analysis(
        self,
        knowledge_graph: nx.Graph,
        ...
        context = {
            ...
            "hop_limit": 3,
            "exploration_strategy": query_state.metadata.get(
                "exploration_direction", "general"
            ),
        }
    ...
    def _extract_reasoning_paths(
        ...
        if "reasoning_paths" in graph_analysis:
            for path in graph_analysis["reasoning_paths"]:
                readable_path = []
                for node_id in path:
                    if isinstance(node_id, int) and node_id < len(
    ...
    def _generate_advanced_response(
        ...
        if query_state.insights:
            context_parts.append(f"Main insight: {query_state.insights[-1]}")
    ...
    def _evaluate_insight_quality(self, result: AdvancedTransformationResult) -> float:
        ...
        if result.branches:
            successful_branches = sum(
                1 for b in result.branches if b.current_state.confidence > 0.5
            )
    ...
    def _check_advanced_convergence(
        self, main_history: QueryTransformationHistory, branches: List[QueryBranch]
    ) -> bool:
        ...
        if branches:
            converged_branches = sum(
                1 for b in branches if b.current_state.stage == "insight"
            )
        ...
        if len(confidence_trajectory) >= 3:
            recent_changes = [
                abs(confidence_trajectory[i] - confidence_trajectory[i - 1])
                for i in range(-3, 0)
            ]
    ...
    def _synthesize_advanced_result(
        self,
        best_result: AdvancedTransformationResult,
        main_history: QueryTransformationHistory,
        all_branches: List[QueryBranch],
    ) -> Dict[str, Any]:
        ...
        result["synthesis"] = {
            ...
            "successful_branches": sum(
                1 for b in all_branches if b.current_state.confidence > 0.5
            ),
    ...
    def _create_empty_advanced_result(
        self, current_state: QueryState
    ) -> AdvancedTransformationResult:
