...
class GraphCentricMainAgent(MainAgent):
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'optimization_rate': removed_count / initial_count if initial_count > 0 else 0
            }
    ...
    def _get_graph_info(self, episode_idx: int) -> Dict[str, Any]:
        ...
        if self.l3_graph and hasattr(self.l3_graph, 'previous_graph'):
            ...
            if graph and hasattr(graph, 'edge_index'):
                ...
                connections = (edge_index[0] == episode_idx).sum() + (edge_index[1] == episode_idx).sum()
    ...
    def get_memory_analysis(self) -> Dict[str, Any]:
        ...
        # 重要度分布
        importances = [self.l2_memory.get_importance(i) for i in range(len(self.l2_memory.episodes))]
        
        # アクセス分布
        access_counts = [ep.metadata.get('access_count', 0) for ep in self.l2_memory.episodes]
        
        # 統合回数分布
        integration_counts = [ep.metadata.get('integration_count', 0) for ep in self.l2_memory.episodes]
        ...
        return {
            ...
            'integration_distribution': {
                'episodes_with_integration': sum(1 for x in integration_counts if x > 0),
                'max_integrations': max(integration_counts) if integration_counts else 0
            }
        }
    
    def configure(self, integration_config: Optional[Dict] = None, 
