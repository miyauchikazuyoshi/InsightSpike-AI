...
class InsightFactRegistry:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        if db_path is None:
            db_path = Path(__file__).parent.parent.parent / "data" / "insight_facts.db"
        ...
        self.concept_index: Dict[str, Set[str]] = defaultdict(set)  # concept -> insight_ids
        
        # Initialize database
        self._init_database()
        self._load_from_database()
        
        logger.info(f"InsightFactRegistry initialized with {len(self.insight_cache)} existing insights")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                conn.execute("CREATE INDEX IF NOT EXISTS idx_quality ON insights(quality_score)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_concepts ON concept_insights(concept)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_validation ON insights(validation_status)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_generated_at ON insights(generated_at)")
            except:
                pass
                
            # Migration: Add updated_at column if it doesn't exist
            try:
                conn.execute("ALTER TABLE insights ADD COLUMN updated_at REAL DEFAULT 0")
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                insight = InsightFact(
                    ...
                    confidence=row[4], quality_score=row[5],
                    ged_optimization=row[6], ig_improvement=row[7],
                    discovery_context=row[8], generated_at=row[9],
                    validation_status=row[10], relationship_type=row[11],
                    usage_count=row[12], last_accessed=row[13]
                )
    ...
    def extract_insights_from_response(self, question: str, response: str, 
                                     l1_analysis: Any, reasoning_quality: float,
                                     graph_before: Optional[nx.Graph] = None,
                                     graph_after: Optional[nx.Graph] = None) -> List[InsightFact]:
        """
        ...
        for candidate in insight_candidates:
            # 2. Evaluate quality
            quality_score = self._evaluate_insight_quality(candidate, question, reasoning_quality)
            ...
            if graph_before and graph_after:
                ...
                if (optimization_result.ged_improvement < self.MIN_GED_IMPROVEMENT or
                    optimization_result.ig_improvement < self.MIN_IG_IMPROVEMENT):
                    continue
            ...
            insight = InsightFact(
                ...
                ged_optimization=optimization_result.ged_improvement if optimization_result else 0.0,
                ig_improvement=optimization_result.ig_improvement if optimization_result else 0.0,
        ...
        logger.info(f"Extracted and registered {len(registered_insights)} insights from response")
        return registered_insights
    
    def _extract_insight_candidates(self, response: str, l1_analysis: Any) -> List[Dict[str, Any]]:
        ...
        if hasattr(l1_analysis, 'unknown_elements') and l1_analysis.unknown_elements:
            ...
            synthesis_text = f"Synthesis of {', '.join(l1_analysis.unknown_elements[:3])}"
            if len(synthesis_text) > 20:
                candidates.append({
                    'text': synthesis_text,
                    'source_concepts': l1_analysis.unknown_elements[:2],
                    'target_concepts': l1_analysis.unknown_elements[2:3] if len(l1_analysis.unknown_elements) > 2 else [],
                    'confidence': 0.6,
    ...
    def _classify_relationship_type(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['cause', 'leads to', 'results in', 'because']):
            return 'causal'
        elif any(word in text_lower for word in ['structure', 'composed of', 'consists of']):
            return 'structural'
        elif any(word in text_lower for word in ['like', 'similar', 'resembles', 'analogy']):
            return 'analogical'
        elif any(word in text_lower for word in ['combining', 'synthesis', 'integration']):
            return 'synthetic'
        else:
            return 'general'
    
    def _evaluate_insight_quality(self, candidate: Dict[str, Any], question: str, reasoning_quality: float) -> float:
        ...
        # 2. Concept richness
        total_concepts = len(candidate['source_concepts']) + len(candidate['target_concepts'])
        ...
        for existing_insight in self.insight_cache.values():
            if self._calculate_text_similarity(insight_text_lower, existing_insight.text.lower()) > 0.8:
    ...
    def _evaluate_graph_optimization(self, graph_before: nx.Graph, graph_after: nx.Graph, 
                                   insight_candidate: Dict[str, Any]) -> GraphOptimizationResult:
        ...
        connectivity_before = nx.average_clustering(graph_before) if graph_before.nodes() else 0
        connectivity_after = nx.average_clustering(graph_after) if graph_after.nodes() else 0
    ...
    def register_insight(self, insight: InsightFact) -> bool:
        ...
        if len(self.insight_cache) >= self.MAX_INSIGHTS:
            logger.warning(f"Maximum insights reached ({self.MAX_INSIGHTS}), cleaning up old insights")
        ...
        logger.info(f"Registered new insight: {insight.text[:50]}... (Quality: {insight.quality_score:.3f})")
        return True
    ...
    def _store_insight_in_db(self, insight: InsightFact):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                insight.confidence, insight.quality_score,
                insight.ged_optimization, insight.ig_improvement,
                insight.discovery_context, insight.generated_at,
                insight.validation_status, insight.relationship_type,
                insight.usage_count, insight.last_accessed
            ))
    ...
    def find_relevant_insights(self, concepts: List[str], limit: int = 10) -> List[InsightFact]:
        ...
        insights = [self.insight_cache[id] for id in relevant_ids if id in self.insight_cache]
        insights.sort(key=lambda x: x.quality_score, reverse=True)
        
        return insights[:limit]
    
    def get_insights_by_type(self, relationship_type: str, limit: int = 10) -> List[InsightFact]:
        ...
        return sorted(matching_insights, key=lambda x: x.quality_score, reverse=True)[:limit]
    ...
    def search_insights_by_concept(self, concept: str, limit: int = 10) -> List[InsightFact]:
        ...
        for insight in self.insights.values():
            ...
            if (any(concept_lower in c.lower() for c in insight.source_concepts) or
                any(concept_lower in c.lower() for c in insight.target_concepts) or
                concept_lower in insight.text.lower()):
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        ged_improvements = [i.ged_optimization for i in insights if i.ged_optimization > 0]
        ig_improvements = [i.ig_improvement for i in insights if i.ig_improvement > 0]
        
        return {
            'total_insights': len(insights),
            'validated_insights': len([i for i in insights if i.validation_status == 'verified']),
            'avg_quality_score': sum(i.quality_score for i in insights) / len(insights),
            'avg_ged_improvement': sum(ged_improvements) / len(ged_improvements) if ged_improvements else 0,
            'avg_ig_improvement': sum(ig_improvements) / len(ig_improvements) if ig_improvements else 0,
    ...
    def _cleanup_old_insights(self):
        ...
        for insight in to_remove:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("DELETE FROM concept_insights WHERE insight_id = ?", (insight.id,))
                conn.commit()
    ...
    def _remove_insight_from_db(self, insight_id: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("DELETE FROM concept_insights WHERE insight_id = ?", (insight_id,))
            conn.commit()
