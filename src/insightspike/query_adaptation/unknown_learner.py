...
class UnknownLearner:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        logger.info(
            f"UnknownLearner initialized with {len(self.weak_relationships)} existing relationships"
        )
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_confidence ON weak_relationships(confidence)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_last_accessed ON weak_relationships(last_accessed)"
                )
            except:
    ...
    def register_question_relationships(
        self,
        known_elements: List[str],
        unknown_elements: List[str],
        question_context: str = "",
    ) -> List[WeakRelationship]:
        """
        ...
        logger.info(
            f"Registered {len(new_relationships)} new weak relationships from question"
        )
        return new_relationships

    def _register_weak_relationship(
        self, concept1: str, concept2: str, source: str, context: str = ""
    ) -> Optional[WeakRelationship]:
        ...
        if key in self.weak_relationships:
            ...
            logger.debug(
                f"Reinforced relationship: {concept1} <-> {concept2} (conf: {rel.confidence:.3f})"
            )
            ...
            if len(self.weak_relationships) >= self.MAX_WEAK_EDGES:
                logger.warning(
                    f"Maximum weak relationships reached ({self.MAX_WEAK_EDGES}), skipping new registration"
                )
                return None
            ...
            self.weak_relationships[key] = rel
            logger.debug(
                f"New weak relationship: {concept1} <-> {concept2} (conf: {rel.confidence:.3f})"
            )
    ...
    def _ensure_sleep_mode_active(self):
        ...
        if not self._sleep_active:
            ...
            self._sleep_thread = threading.Thread(
                target=self._sleep_mode_loop, daemon=True
            )
    ...
    def _sleep_mode_loop(self):
        ...
        while self._sleep_active:
            ...
            if idle_time >= self.SLEEP_INTERVAL:
                ...
                logger.info(
                    f"Sleep cleanup: removed {cleanup_stats['removed']}, "
    ...
    def get_related_concepts(
        self, concept: str, min_confidence: float = 0.2
