...
class BaseStrategy(ExplorationStrategy):
    ...
    # --- Added to satisfy abstract interface (tests instantiate derived strategies) ---
    def adjust_params(self, attempt: int, prev_result: ExplorationResult) -> ExplorationParams:  # type: ignore[override]
        """Adjust parameters based on previous result (simple heuristic).

        Strategy:
        - Reduce radius slightly if spike not detected to explore locally.
        - Increase radius if confidence very low (<0.3).
        - Decay temperature each attempt.
        - Keep topK values constant (could be adapted with future logic).
        """
        radius = prev_result.params.radius * (0.9 if not prev_result.spike_detected else 1.0)
        if prev_result.confidence < 0.3:
            radius *= 1.1
        radius = self._bound_radius(radius)
        temperature = self._calculate_temperature(attempt)
        return ExplorationParams(
            radius=radius,
            topk_l1=prev_result.params.topk_l1,
            topk_l2=prev_result.params.topk_l2,
            topk_l3=prev_result.params.topk_l3,
