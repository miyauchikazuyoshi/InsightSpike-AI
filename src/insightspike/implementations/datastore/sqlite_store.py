        ...
        try:
            ...
            if os.path.exists(mapping_path):
                with open(mapping_path, 'r') as f:
                    ...
                    self.reverse_id_map = {int(k): v for k, v in mappings['reverse_id_map'].items()}
            return True
        except Exception as e:
...
class SQLiteDataStore(AsyncDataStore):
    ...
    def _init_tables_sync(self):
        ...
        # Indices for graph
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_edges_namespace ON graph_edges(namespace)')
    ...
    async def _init_tables(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Indices for episodes
            await db.execute('CREATE INDEX IF NOT EXISTS idx_episodes_namespace ON episodes(namespace)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_episodes_created_at ON episodes(created_at)')
            ...
            # Indices for graph
            await db.execute('CREATE INDEX IF NOT EXISTS idx_edges_namespace ON graph_edges(namespace)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_edges_source ON graph_edges(source_id)')
            await db.execute('CREATE INDEX IF NOT EXISTS idx_edges_target ON graph_edges(target_id)')
    ...
    async def add_episode(self, episode: Dict[str, Any], namespace: str = "default") -> str:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            ''', (
                ...
                namespace,
                episode['text'],
                episode['vec'].tobytes() if isinstance(episode['vec'], np.ndarray) else episode['vec'],
                episode.get('c', 0.5),
    ...
    async def batch_add_episodes(self, episodes: List[Dict[str, Any]], namespace: str = "default") -> List[str]:
    ...
    async def search_episodes_by_vector(
        ...
        # Search in FAISS
        distances, indices = index.search(query_vector.reshape(1, -1), min(k, index.get_size()))
    ...
    async def get_episodes_by_ids(self, ids: List[str], namespace: str = "default") -> List[Dict[str, Any]]:
    ...
    async def update_episode(self, episode_id: str, updates: Dict[str, Any], namespace: str = "default") -> bool:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                if 'vec' in updates:
                    await self._update_vector_index(namespace, episode_id, updates['vec'])
    ...
    async def _update_vector_index(self, namespace: str, episode_id: str, vector: np.ndarray):
        """Update vector index with single vector"""
        index = await self._get_vector_index(namespace)
        if index:
            index.add_vectors(vector.reshape(1, -1), [episode_id])
    
    async def _batch_update_vector_index(self, namespace: str, episode_ids: List[str], vectors: np.ndarray):
    ...
    async def stream_search_results(
        ...
        distances, indices = index.search(
            ...
            min(max_results, index.get_size())
        )
    ...
    def search_vectors(self, query_vector: np.ndarray, k: int = 10, 
        ...
        asyncio.set_event_loop(loop)
        try:
            results = loop.run_until_complete(self.search_episodes_by_vector(query_vector, k, 0.0, namespace))
        finally:
            loop.close()
        ...
        distances = []
        for r in results:
            similarity = np.dot(query_vector, r['vec']) / (np.linalg.norm(query_vector) * np.linalg.norm(r['vec']))
    ...
    def save_graph(self, graph_data: Any, graph_id: str, namespace: str = "graphs") -> bool:
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                loop.run_until_complete(self._save_graph(graph_data, graph_id, namespace))
            finally:
                loop.close()
            return True
        except Exception as e:
    ...
    async def _save_graph(self, graph_data: Dict[str, Any], graph_id: str, namespace: str):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            ''', (
                f"graph_{graph_id}",
                namespace,
                json.dumps({'id': graph_id, 'node_count': len(graph_data.get('nodes', {}))})
            ))
    ...
    async def _load_graph(self, graph_id: str, namespace: str) -> Optional[Dict[str, Any]]:
    ...
    async def _delete(self, key: str, namespace: str):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if namespace == "metadata":
                await db.execute('DELETE FROM metadata WHERE key = ? AND namespace = ?', (key, namespace))
            elif namespace == "graphs":
                # Delete graph nodes and edges
                await db.execute('DELETE FROM graph_edges WHERE namespace = ? AND (source_id = ? OR target_id = ?)', 
                               (namespace, key, key))
                await db.execute('DELETE FROM graph_nodes WHERE id = ? AND namespace = ?', (key, namespace))
            else:
                # Assume episodes
                await db.execute('DELETE FROM episodes WHERE id = ? AND namespace = ?', (key, namespace))
