...
DIAG: Lightweight import tracing added to debug pytest collection hang.
Will print only when INSIGHTSPIKE_DIAG_IMPORT=1.
"""

import os
if os.getenv('INSIGHTSPIKE_DIAG_IMPORT') == '1':
    print('[layer1_error_monitor] module import start', flush=True)
...
if os.getenv('INSIGHTSPIKE_DIAG_IMPORT') == '1':
    print('[layer1_error_monitor] after config import', flush=True)
from ...core.base import L1ErrorMonitorInterface, LayerInput, LayerOutput
if os.getenv('INSIGHTSPIKE_DIAG_IMPORT') == '1':
    print('[layer1_error_monitor] after core.base import', flush=True)
...
class ErrorMonitor(L1ErrorMonitorInterface):
    ...
    def __init__(
        self, layer_id: str = "L1_ErrorMonitor", config: Dict[str, Any] = None
    ):
    ...
    def is_error_spike(self) -> bool:
        ...
        recent_mean = (
            np.mean(self.error_history[-10:-1])
            if len(self.error_history) > 10
            else np.mean(self.error_history[:-1])
        )
...
def analyze_input(
    ...
    unknown_learner: Optional[Any] = None,
    ...
    for concept in query_concepts:
        certainty = _calculate_concept_certainty(
            concept, context_documents, knowledge_base_stats
        )
    ...
    requires_synthesis = _requires_synthesis_analysis(
        query, question_indicators, known_elements, unknown_elements
    )

    # Calculate query complexity
    query_complexity = _calculate_query_complexity(
        query, question_indicators, len(known_elements), len(unknown_elements)
    )
    ...
    analysis_confidence = _calculate_analysis_confidence(
        certainty_scores, context_documents
    )
...
def _extract_key_concepts(query: str) -> List[str]:
    ...
    all_concepts = list(
        set(concepts + [term.lower() for term in proper_nouns + technical_terms])
    )
...
def _detect_question_indicators(query: str) -> Dict[str, bool]:
    ...
    return {
        ...
        "compare": "compare" in query_lower
        ...
        "synthesis": "integrate" in query_lower
        or "synthesize" in query_lower
        or "combine" in query_lower,
    }


def _calculate_concept_certainty(
    concept: str, context_documents: List[str], knowledge_base_stats: Dict[str, Any]
) -> float:
    ...
    if context_documents:
        doc_mentions = sum(
            1 for doc in context_documents if concept.lower() in doc.lower()
        )
    ...
    if knowledge_base_stats:
        ...
        term_freq = knowledge_base_stats.get("term_frequencies", {}).get(
            concept.lower(), 0
        )
        if term_freq > 0:
            # Normalize by max frequency for scaling
            max_freq = (
                max(knowledge_base_stats.get("term_frequencies", {}).values()) or 1
            )
...
def _requires_synthesis_analysis(
    query: str,
    question_indicators: Dict[str, bool],
    known_elements: List[str],
    unknown_elements: List[str],
) -> bool:
    ...
    if (
        question_indicators["compare"]
        or question_indicators["relationship"]
        or question_indicators["synthesis"]
    ):
        return True
    ...
    if (
        question_indicators["analyze"]
        and (len(known_elements) + len(unknown_elements)) > 2
    ):
        return True

    # Complex "how" or "why" questions
    if (question_indicators["how_does"] or question_indicators["why_does"]) and len(
        unknown_elements
    ) > 1:
        return True
...
def _calculate_query_complexity(
    query: str, question_indicators: Dict[str, bool], num_known: int, num_unknown: int
) -> float:
...
def _calculate_error_threshold(
    query_complexity: float, requires_synthesis: bool
) -> float:
...
def _calculate_analysis_confidence(
    certainty_scores: Dict[str, float], context_documents: List[str]
) -> float:
...
if os.getenv('INSIGHTSPIKE_DIAG_IMPORT') == '1':
    # End-of-module marker to verify full execution completes
    print('[layer1_error_monitor] module import end', flush=True)
