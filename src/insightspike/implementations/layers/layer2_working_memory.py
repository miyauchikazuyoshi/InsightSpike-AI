    ...
    def __init__(
        ...
        if isinstance(config, WorkingMemoryConfig):
            ...
            self.config = WorkingMemoryConfig(
                ...
                datastore_namespace=getattr(
                    config.memory, "datastore_namespace", "episodes"
                ),
            )
        else:
            self.config = WorkingMemoryConfig()
        ...
        logger.info(
            f"L2WorkingMemoryManager initialized with working memory size: {self.config.working_memory_size}"
        )
    ...
    def store_episode(
        ...
        try:
            ...
            success = self.datastore.save_episodes(
                [episode], namespace=self.config.datastore_namespace
            )
        ...
        try:
            if query:
                ...
                episodes = self.search_episodes(
                    query, k=self.config.working_memory_size
                )
            ...
            logger.info(
                f"Refreshed working memory with {len(self.working_memory)} episodes"
            )
