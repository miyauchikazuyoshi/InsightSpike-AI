...
class ScalableGraphBuilder:
    ...
    def __init__(self, config=None, monitor: Optional[GraphOperationMonitor] = None):
        ...
        if config is None:
            ...
            config = SimpleNamespace(
                graph=SimpleNamespace(similarity_threshold=0.7),
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        """
        num_nodes = len(self.documents) if self.documents else 0
        
        stats = {
            "num_nodes": num_nodes,
            "num_edges": 0,
            "has_graph": hasattr(self, 'current_graph') and self.current_graph is not None,
            "incremental_mode": hasattr(self, 'incremental') and self.incremental
        }
        
        if hasattr(self, 'current_graph') and self.current_graph is not None:
            stats["num_edges"] = self.current_graph.edge_index.size(1) if hasattr(self.current_graph, 'edge_index') else 0
            
        if hasattr(self, 'index') and self.index is not None:
            stats["index_size"] = self.index.ntotal if hasattr(self.index, 'ntotal') else 0
            
        return stats
    
    def get_current_graph(self) -> Optional[Data]:
        """Get the current graph object.
        
        Returns:
            Current PyTorch Geometric Data object or None
        """
        return self.current_graph
    
    def build(self, embeddings: np.ndarray, documents: Optional[List[Dict[str, Any]]] = None) -> Data:
        """Alias for build_graph to maintain compatibility.
        ...
        Returns:
            PyTorch Geometric Data object
        """
        return self.build_graph(documents or [], embeddings=embeddings)
