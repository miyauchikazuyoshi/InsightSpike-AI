...
from ...core.base.generic_interfaces import TaskType
from ...core.base.maze_implementation import (
...
def create_maze_agent(
    maze_size: int = 10, agent_config: Optional[Dict[str, Any]] = None
) -> GenericInsightSpikeAgent:
    """Quick function to create a maze agent"""
    return InsightSpikeAgentFactory.create_maze_agent(
        maze_size=maze_size, config=agent_config
    )
...
# Q&A Agent creation functions
def create_qa_agent(mode: str = "basic", **kwargs):
    """Create a Q&A agent using ConfigurableAgent"""
    try:
        from .configurable_agent import AgentConfig, AgentMode, ConfigurableAgent

        # Map string mode to enum
        mode_map = {
            "basic": AgentMode.BASIC,
            "enhanced": AgentMode.ENHANCED,
            "query_transform": AgentMode.QUERY_TRANSFORM,
            "advanced": AgentMode.ADVANCED,
