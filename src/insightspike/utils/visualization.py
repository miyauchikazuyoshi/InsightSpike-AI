...
class InsightSpikeVisualizer:
    ...
    def create_comparison_chart(self, data1: List[float], data2: List[float],
                              labels: List[str], title: str = "Performance Comparison",
                              label1: str = "InsightSpike", label2: str = "Baseline",
                              save_path: Optional[str] = None) -> str:
    ...
    def create_progress_visualization(self, episodes: List[int], rewards: List[float],
        ...
        if window_size > 0:
            success_ma = self._moving_average([int(s) for s in success_indicators], window_size)
            episodes_ma = episodes[window_size-1:]
            
            ax2.plot(episodes_ma, success_ma, color='green', linewidth=2, label='Success Rate (MA)')
    ...
    def create_insight_spike_visualization(self, episodes: List[int], 
        ...
        ax1.axhline(y=0.5, color='red', linestyle='--', alpha=0.5, label='ΔGED Threshold')
        ...
        ax2.axhline(y=1.5, color='blue', linestyle='--', alpha=0.5, label='ΔIG Threshold')
        ...
        # Insight spikes
        ax3.scatter(spike_episodes, [1]*len(spike_episodes), 
                   color='gold', s=100, alpha=0.8, label='Insight Spikes')
    ...
    def create_algorithm_comparison(self, algorithm_results: Dict[str, Dict[str, float]],
        ...
        for i, metric in enumerate(metrics):
            ...
            best_idx = np.argmax(values) if metric != 'processing_time' else np.argmin(values)
    ...
    def _create_gauge_chart(self, ax, value: float, title: str, unit: str) -> None:
        ...
        ax.plot(radius * np.cos(theta), radius * np.sin(theta), 
               color='lightgray', linewidth=8, alpha=0.3)
        
        # Value arc
        value_theta = theta[:int(len(theta) * normalized_value)]
        color = 'green' if normalized_value > 0.7 else 'orange' if normalized_value > 0.4 else 'red'
        ax.plot(radius * np.cos(value_theta), radius * np.sin(value_theta),
               color=color, linewidth=8)
        
        # Center text
        ax.text(0, -0.3, f'{display_value:.1f}{unit}', 
...
def quick_performance_chart(metrics: Dict[str, float], title: str = "Performance",
                          save_path: Optional[str] = None) -> str:
...
def quick_comparison(data1: List[float], data2: List[float], labels: List[str],
                    title: str = "Comparison", save_path: Optional[str] = None) -> str:
    """Quickly create a comparison chart"""
    visualizer = InsightSpikeVisualizer()
    return visualizer.create_comparison_chart(data1, data2, labels, title, save_path=save_path)


def quick_progress_chart(episodes: List[int], rewards: List[float],
                        save_path: Optional[str] = None) -> str:
    """Quickly create a progress chart"""
    visualizer = InsightSpikeVisualizer()
    success_indicators = [r > 0 for r in rewards]  # Simple success indicator
    return visualizer.create_progress_visualization(episodes, rewards, success_indicators, save_path=save_path)
