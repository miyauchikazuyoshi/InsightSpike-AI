...
class ImprovedEntropyIG:
    ...
    def calculate_ig(self, graph1: Any, graph2: Any) -> float:
        ...
        try:
            ...
            total_ig = (
                (1 - self.structure_weight) * (cluster_ig + pca_ig) / 2 +
                self.structure_weight * struct_ig
            )
            
            logger.debug(f"Improved IG: cluster={cluster_ig:.3f}, "
    ...
    def _cluster_entropy(self, embeddings: np.ndarray) -> float:
        ...
        try:
            # Perform clustering
            kmeans = KMeans(n_clusters=min(self.n_clusters, len(embeddings)), 
                           random_state=42)
    ...
    def normalize_ig(self, ig_value: float, graph1_size: int, graph2_size: int) -> float:
        """
        IGを正規化（サイズ変化を考慮）
