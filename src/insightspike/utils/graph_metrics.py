...
class GraphMetricsCalculator:
    ...
    def _information_content(self, graph: Any) -> float:
        ...
        try:
            ...
            logging.getLogger(__name__).error(f"Information content calculation failed: {e}")
            return 0.0
    ...
    def _fallback_ged(self, graph1: Any, graph2: Any) -> float:
        ...
        try:
            size1 = getattr(graph1, 'num_nodes', len(graph1) if hasattr(graph1, '__len__') else 1)
            size2 = getattr(graph2, 'num_nodes', len(graph2) if hasattr(graph2, '__len__') else 1)
