...
class GraphMetricsCalculator:
    ...
    def _graph_complexity(self, graph: Any) -> float:
        ...
        try:
            ...
            if hasattr(graph, "num_nodes") and hasattr(graph, "edge_index"):
                ...
                num_edges = (
                    graph.edge_index.size(1) if graph.edge_index is not None else 0
                )
            else:
                return 0.0
            ...
            density = (
                num_edges / max(1, max_possible_edges) if max_possible_edges > 0 else 0
            )
            ...
            logging.getLogger(__name__).error(
                f"Graph complexity calculation failed: {e}"
            )
            return 0.0
    ...
    def _structural_difference(self, graph1: Any, graph2: Any) -> float:
        ...
        try:
            ...
            logging.getLogger(__name__).error(
                f"Structural difference calculation failed: {e}"
            )
            return 0.0
    ...
    def _embedding_difference(self, graph1: Any, graph2: Any) -> float:
        ...
        try:
            ...
            # Get embeddings
            emb1 = (
                graph1.x.cpu().numpy() if hasattr(graph1.x, "cpu") else graph1.x.numpy()
            )
            emb2 = (
                graph2.x.cpu().numpy() if hasattr(graph2.x, "cpu") else graph2.x.numpy()
            )
            ...
            logging.getLogger(__name__).error(
                f"Embedding difference calculation failed: {e}"
            )
            return 0.0
    ...
    def _information_content(self, graph: Any) -> float:
        ...
        try:
            ...
            features = (
                graph.x.cpu().numpy() if hasattr(graph.x, "cpu") else graph.x.numpy()
            )
