...
Always base your responses on the provided context and clearly distinguish between what the evidence supports versus speculative reasoning."""
    ...
    def _build_document_context(self, documents: List[Dict[str, Any]]) -> str:
        ...
        for i, doc in enumerate(documents[:5], 1):  # Limit to top 5 documents
            ...
            if text:
                confidence_indicator = self._get_confidence_indicator(c_value, similarity)
                context_lines.append(f"### Document {i} {confidence_indicator}")
                context_lines.append(f"**Relevance:** {similarity:.3f} | **Confidence:** {c_value:.3f}")
    ...
    def _build_reasoning_context(self, graph_info: Dict[str, Any]) -> str:
        ...
        if metrics:
            ...
            lines.append(f"- ΔGED (Graph Edit Distance Change): {metrics.get('delta_ged', 0):.3f}")
            lines.append(f"- ΔIG (Information Gain Change): {metrics.get('delta_ig', 0):.3f}")
    ...
    def _build_question_section(self, question: str, reasoning_quality: float = 0.0) -> str:
        ...
        if reasoning_quality > 0.7:
            lines.append("High reasoning quality detected. Provide a comprehensive, well-structured answer.")
        elif reasoning_quality > 0.4:
            lines.append("Moderate reasoning quality. Provide a balanced answer with appropriate caveats.")
        else:
            lines.append("Limited context available. Provide what insights you can while acknowledging limitations.")
