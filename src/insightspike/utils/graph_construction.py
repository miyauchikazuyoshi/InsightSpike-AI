...
class GraphBuilder:
    ...
    def _extract_concepts(self, text: str) -> List[str]:
        ...
        # Look for key terms
        key_terms = [
            'entropy', 'information', 'thermodynamic', 'energy', 
            'system', 'quantum', 'probability', 'disorder',
            'state', 'microstate', 'conservation', 'transformation'
        ]
            ...
            for node2 in nodes[i+1:]:
                ...
                if shared:
                    ...
                    if not G.has_edge(node1, node2):
                        G.add_edge(node1, node2, weight=weight, type='concept', shared_concepts=list(shared))
                    else:
    ...
    def add_query_node(self, G: nx.Graph, query: str, query_embedding: Optional[np.ndarray] = None) -> str:
