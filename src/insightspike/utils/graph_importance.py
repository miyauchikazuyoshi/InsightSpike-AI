...
class GraphImportanceCalculator:
    ...
    def calculate_importance(
        ...
        try:
            ...
            combined_score = (
                0.3 * degree_score +      # 30% from connections
                0.2 * betweenness_score + # 20% from path importance
                0.3 * pagerank_score +    # 30% from PageRank
                0.2 * access_score        # 20% from usage patterns
            )
    ...
    def _access_score(self, node_idx: int) -> float:
        ...
        recency_factor = np.exp(-self.decay_factor * time_since_access / 3600)  # Decay per hour
...
class DynamicImportanceTracker:
    ...
    def __init__(self, calculator: Optional[GraphImportanceCalculator] = None):
        ...
        self.importance_history: Dict[int, List[Tuple[float, float]]] = defaultdict(list)
        
    def track_importance(self, graph: Data, node_idx: int, timestamp: Optional[float] = None):
