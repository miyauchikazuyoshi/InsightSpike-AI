...
class HybridMetrics:
    ...
    def calculate_hybrid_metrics(self, 
                               graph1: Any, 
                               graph2: Any,
                               query: Optional[str] = None) -> Dict[str, float]:
        """
        ハイブリッドメトリクスを計算
        ...
        ) * effective_quality * relevance_score
        ...
        logger.debug(f"Hybrid metrics: GED={results['hybrid_ged']:.3f}, "
    ...
    def _analyze_structure(self, graph1: Any, graph2: Any) -> Tuple[float, float]:
        ...
        try:
            ...
            # Structure-based IG
            density1 = nx1.number_of_edges() / max(nx1.number_of_nodes() * (nx1.number_of_nodes() - 1) / 2, 1)
            density2 = nx2.number_of_edges() / max(nx2.number_of_nodes() * (nx2.number_of_nodes() - 1) / 2, 1)
        ...
        try:
            ...
            for graph in [graph1, graph2]:
                if hasattr(graph, 'x') and graph.x is not None:
                    embeddings = graph.x.cpu().numpy() if hasattr(graph.x, 'cpu') else graph.x.numpy()
