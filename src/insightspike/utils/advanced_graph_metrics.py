...
from ..algorithms.graph_edit_distance import GraphEditDistance, compute_delta_ged as nx_delta_ged
...
class AdvancedGraphMetrics:
    ...
    def delta_ged(self, 
                  ...
                  reference_graph: Optional[Data] = None) -> float:
        """
        ...
        try:
            ...
            # Track statistics
            if hasattr(self.pyg_ged, 'nx_ged_calculator') and self.pyg_ged.nx_ged_calculator:
    ...
    def delta_ig(self,
        ...
        try:
            ...
            if old_embeddings is not None and new_embeddings is not None:
                ...
                result = self.ig_calculator.calculate(
                    old_embeddings,
                    new_embeddings
                )
    ...
    def combined_insight_score(self,
                              old_graph: Optional[Data],
                              new_graph: Optional[Data],
                              ged_threshold: float = -0.5,
                              ig_threshold: float = 0.2) -> Tuple[float, bool, Dict[str, float]]:
        """
        ...
        logger.info(f"Insight detection: ΔGED={delta_ged:.3f}, ΔIG={delta_ig:.3f}, "
    ...
    def _fallback_delta_ged(self, old_graph: Any, new_graph: Any) -> float:
        ...
        try:
            ...
            old_edges = old_graph.edge_index.size(1) if hasattr(old_graph, 'edge_index') else 0
            new_edges = new_graph.edge_index.size(1) if hasattr(new_graph, 'edge_index') else 0
...
def delta_ged(old_graph: Optional[Any], 
...
def delta_ig(old_graph: Optional[Any],
             new_graph: Optional[Any],
             query: Optional[str] = None,
             use_advanced: bool = True) -> float:
    """
