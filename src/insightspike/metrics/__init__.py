...
try:
    from ..algorithms import (
        GraphEditDistance, InformationGain, 
...
def compute_fusion_reward(delta_ged: float, 
                         delta_ig: float,
    ...
    logger.debug(f"Fusion reward breakdown: "
...
def analyze_insight(before_state: Any, after_state: Any, 
    ...
    # Detect Eureka spike
    eureka_spike = (
        delta_ged_result <= -abs(thresholds['ged_threshold']) and
        delta_ig_result >= thresholds['ig_threshold']
    )
...
def configure_default_weights(ged_weight: float = 0.4,
                            ig_weight: float = 0.3, 
