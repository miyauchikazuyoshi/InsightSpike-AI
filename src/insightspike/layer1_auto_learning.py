...
class Layer1AutoLearningSystem:
    ...
    def __init__(self, storage_path: str = None):
        ...
        if storage_path is None:
            storage_path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'learning')
        ...
        self.concept_relationships_file = self.storage_path / "concept_relationships.json"
        ...
        logger.info(f"Layer1 Auto-Learning System initialized with {len(self.unknown_concepts)} unknown concepts")
    
    def register_unknown_concepts(self, query: str, unknown_elements: List[str], 
                                 associated_concepts: List[str] = None) -> List[str]:
        """
        ...
        for concept in unknown_elements:
            ...
            if concept_id not in self.unknown_concepts:
                ...
                logger.info(f"Registered new unknown concept: '{concept}' from query: '{query[:50]}...'")
                ...
                existing.associated_concepts = list(dict.fromkeys(existing.associated_concepts))
                
                logger.debug(f"Updated existing unknown concept: '{concept}' (attempts: {existing.resolution_attempts})")
    ...
    def attempt_concept_resolution(self, query: str, response_context: str, 
        ...
        unknown_in_query = [cid for cid, concept in self.unknown_concepts.items() 
        ...
        learning_session = LearningSession(
            ...
            query=query,
            unknown_concepts=[self.unknown_concepts[cid].concept for cid in unknown_in_query],
        ...
        for concept_id in unknown_in_query:
            ...
            learned_info = self._extract_concept_information(concept.concept, response_context)
            ...
            if learned_info:
                ...
                concept.confidence_score = min(1.0, concept.confidence_score + quality_score * 0.3)
                ...
                logger.info(f"Learned about concept '{concept.concept}': {learned_info[:100]}...")
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        resolved_concepts = len([c for c in self.unknown_concepts.values() if c.learned_definition])
        learning_rate = resolved_concepts / total_concepts if total_concepts > 0 else 0
        
        # Concept priority analysis
        high_priority_concepts = [c for c in self.unknown_concepts.values() if c.learning_priority > 0.7]
        ...
        insights = {
            ...
            'top_priority_concepts': [c.concept for c in sorted(high_priority_concepts, 
                                    key=lambda x: x.learning_priority, reverse=True)[:5]],
    ...
    def export_learned_knowledge(self) -> str:
        ...
        knowledge_lines.append(f"# Generated by Layer1 Auto-Learning System on {datetime.now()}")
        knowledge_lines.append(f"# Total concepts learned: {len([c for c in self.unknown_concepts.values() if c.learned_definition])}")
        knowledge_lines.append("")
        
        # Group by learning priority
        resolved_concepts = [c for c in self.unknown_concepts.values() if c.learned_definition]
        ...
        for concept in resolved_concepts:
            ...
            knowledge_lines.append(f"**Associated Concepts**: {', '.join(concept.associated_concepts[:5])}")
            knowledge_lines.append(f"**Learning Priority**: {concept.learning_priority:.2f}")
            knowledge_lines.append("")
    ...
    def _calculate_learning_priority(self, concept: str, query: str) -> float:
        ...
        if any(indicator in concept.lower() for indicator in ['quantum', 'neural', 'algorithm', 'theory', 'principle']):
            priority += 0.2
        
        # Higher priority for question-related concepts
        if any(q_word in query.lower() for q_word in ['how', 'why', 'what', 'when', 'where']):
            priority += 0.1
    ...
    def _create_knowledge_entry(self, concept: UnknownConcept, learned_info: str) -> str:
    ...
    def _update_concept_relationships(self, concept: str, associated_concepts: List[str]):
    ...
    def _count_concept_mentions(self, concept: str) -> int:
        ...
        return sum(1 for c in self.unknown_concepts.values() if c.concept.lower() == concept.lower())
            ...
            try:
                with open(self.unknown_concepts_file, 'r', encoding='utf-8') as f:
                    ...
                    for cid, concept_data in data.items():
                        ...
                        concept_data['discovered_at'] = datetime.fromisoformat(concept_data['discovered_at'])
