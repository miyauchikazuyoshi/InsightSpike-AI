...
class GraphAnalyzer:
    ...
    def analyze_graph_structure(self) -> Dict[str, Any]:
        ...
        if not self.l3_reasoner or not hasattr(self.l3_reasoner, "previous_graph"):
            return {"error": "No graph data available", "nodes": 0, "edges": 0}

        graph = self.l3_reasoner.previous_graph
        if graph is None:
            return {"error": "Graph not initialized", "nodes": 0, "edges": 0}
        ...
        if hasattr(graph, "edge_index"):
            ...
            edge_index = (
                ...
                if hasattr(graph.edge_index, "numpy")
                else graph.edge_index
            )
            edge_weight = (
                graph.edge_weight.numpy()
                if hasattr(graph, "edge_weight") and hasattr(graph.edge_weight, "numpy")
                else None
            )
            ...
            num_nodes = (
                graph.num_nodes if hasattr(graph, "num_nodes") else max(nodes) + 1
            )
    ...
    def _calculate_centrality(
        self, graph: np.ndarray, nodes: List[int]
        ...
        for node in nodes:
            ...
            graph_2d = (
                graph
                if graph.ndim == 2
                else graph.reshape(int(np.sqrt(len(graph))), -1)
            )
            degree = sum(
                1 for i in range(len(graph_2d)) if graph_2d[node][i] > 0 and i != node
            )
    ...
    def _detect_communities(
        self, edges: List[Tuple[int, int, float]], nodes: set
    ) -> List[List[int]]:
    ...
    def export_for_visualization(self, output_path: Path) -> Dict[str, Any]:
        ...
        for i in range(min(num_nodes, len(self.l2_memory.episodes))):
            ...
            nodes.append(
                {
                    "id": i,
                    "label": episode.text[:50] + "..."
                    if len(episode.text) > 50
                    else episode.text,
...
def analyze_command(
    export: Optional[Path] = typer.Option(
        None, "--export", "-e", help="Export analysis to JSON"
    ),
...
def visualize_command(
    ...
    insights_only: bool = typer.Option(
        False, "--insights-only", help="Only show nodes with insights"
    ),
    verbose: bool = typer.Option(
...
def sleep_gc_command(
    namespace: str = typer.Option("graphs", help="Graph namespace to maintain"),
    max_degree: int = typer.Option(12, help="Max retained undirected neighbors per node"),
    min_weight: float = typer.Option(0.2, help="Minimum edge weight to retain"),
    decay: float = typer.Option(0.98, help="Decay factor applied to retained edge weights"),
    keep_pinned: bool = typer.Option(True, help="Do not delete nodes with attributes.pinned=true"),
):
    ...
    try:
        ...
        console.print("[bold green]✅ Sleep/Forget GC completed[/bold green]")
        console.print(
            f"Removed edges: {stats['removed_edges']} | Updated edges: {stats['updated_edges']} | Removed nodes: {stats['removed_nodes']}"
        )
        console.print(
            f"Pairs kept: {stats['kept_pairs']} / {stats['total_pairs']} (namespace='{namespace}')"
        )

    except Exception as e:
        logger.error(f"Sleep/Forget GC failed: {e}")
        console.print(f"[red]Error: {e}[/red]")
