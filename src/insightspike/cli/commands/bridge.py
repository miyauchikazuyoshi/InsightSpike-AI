...
class ConceptBridge:
    ...
    def find_bridges(
        self, concept1: str, concept2: str, max_hops: int = 3, top_k: int = 5
    ) -> List[Dict]:
        ...
        paths = self._find_paths(
            concept1, concept2, neighbors1, neighbors2, max_hops=max_hops
        )
    ...
    def _find_neighbors(self, vector: np.ndarray, k: int = 10) -> List[Dict]:
        ...
        if (
            not hasattr(self.l2_memory, "faiss_index")
            or self.l2_memory.faiss_index is None
        ):
            return []
    ...
    def _concepts_similar(
        self, concept1: str, concept2: str, threshold: float = 0.7
    ) -> bool:
    ...
    def _rank_paths(
        self, paths: List[List[str]], vec1: np.ndarray, vec2: np.ndarray
    ) -> List[Dict]:
    ...
    def _calculate_novelty(
        self, path: List[str], vec1: np.ndarray, vec2: np.ndarray
    ) -> float:
...
def bridge_command(
    ...
    max_hops: int = typer.Option(
        3, "--max-hops", "-h", help="Maximum hops between concepts"
    ),
    top_k: int = typer.Option(5, "--top-k", "-k", help="Number of paths to show"),
    export: Optional[Path] = typer.Option(
        None, "--export", "-e", help="Export paths to JSON"
    ),
    ...
    try:
        ...
        with Progress(
            ...
            if hasattr(agent, "load_state"):
                try:
                    ...
                    console.print(
                        "[dim]Run 'spike embed' first to build knowledge base[/dim]"
                    )
                    return
        ...
        with Progress(
            ...
            bridges = bridge_finder.find_bridges(
                concept1, concept2, max_hops=max_hops, top_k=top_k
            )
