...
class PyGAdapter:
    ...
    def extract_structural_features(pyg_graph: Data) -> dict:
        ...
        try:
            # Basic counts
            features['num_nodes'] = pyg_graph.num_nodes if hasattr(pyg_graph, 'num_nodes') else 0
            features['num_edges'] = pyg_graph.edge_index.size(1) if hasattr(pyg_graph, 'edge_index') else 0
            ...
            if features['num_nodes'] > 0 and features['num_edges'] > 0:
                ...
                features['density'] = features['num_edges'] / max_edges if max_edges > 0 else 0.0
                
            # Feature availability
            features['has_features'] = hasattr(pyg_graph, 'x') and pyg_graph.x is not None
