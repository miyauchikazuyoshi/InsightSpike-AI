...
def calculate_adaptive_topk(l1_analysis: Dict[str, Any], 
                          config: AdaptiveTopKConfig = None) -> Dict[str, int]:
    """
    ...
    return {
        ...
        'adaptation_factors': {
            ...
            'confidence_factor': confidence_factor if analysis_confidence < config.confidence_threshold else 1.0,
            'scaling': {
...
def estimate_chain_reaction_potential(l1_analysis: Dict[str, Any], 
                                    adaptive_topk: Dict[str, int]) -> float:
    """
    ...
    # topK density factor
    avg_topk = (adaptive_topk['layer1_k'] + adaptive_topk['layer2_k'] + adaptive_topk['layer3_k']) / 3
...
def test_adaptive_topk():
    ...
    test_scenarios = [
        ...
        },
        {
            ...
            'analysis': {
                'known_elements': ['probability'],
                'unknown_elements': ['monty', 'hall', 'problem', 'demonstrate', 'relationship', 'information', 'theory'],
                'requires_synthesis': True,
        ...
        },
        {
            ...
            'analysis': {
                'known_elements': [],
                'unknown_elements': ['ship', 'theseus', 'quantum', 'particles', 'measurement', 'observer', 'effect'],
                'requires_synthesis': True,
    ...
    for scenario in test_scenarios:
        ...
        print(f'\n📋 Scenario: {name}')
        print(f'   Known: {len(analysis["known_elements"])}, Unknown: {len(analysis["unknown_elements"])}')
        print(f'   Synthesis: {analysis["requires_synthesis"]}, Complexity: {analysis["query_complexity"]:.2f}')
        ...
        topk_values = {k: v for k, v in adaptive_result.items() if not k.startswith('adaptation')}
        ...
        print(f'   Layer1: {topk_values["layer1_k"]} (scale: {adaptation_factors["scaling"]["layer1"]:.2f}x)')
        print(f'   Layer2: {topk_values["layer2_k"]} (scale: {adaptation_factors["scaling"]["layer2"]:.2f}x)')
        print(f'   Layer3: {topk_values["layer3_k"]} (scale: {adaptation_factors["scaling"]["layer3"]:.2f}x)')
