...
# Prefer public API for examples
...
def example_1_basic_maze_agent():
    ...
    # Use public quick-start agent for a minimal example
    agent = create_agent()
...
def example_2_custom_configured_agent():
    ...
    # Internal factory usage (advanced). Imported lazily to keep top-level public.
    from insightspike.implementations.agents.agent_factory import AgentConfigBuilder, create_maze_agent
    config = (AgentConfigBuilder().learning_rate(0.2).exploration_params(0.5, 0.99, 0.02).insight_thresholds(-0.2, 0.8).build())
...
def example_3_standalone_graph_reasoner():
    ...
    # Create standalone reasoner (advanced)
...
def example_6_custom_environment():
    ...
    # Advanced: internal interfaces used below
