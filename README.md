...
| Maze stage-2 (Wake-Sleep-Wake) | 25x25: success 71.9%→95.3%, steps −41%, edges −34% — **pre-registered sleep-only ablation (2026-07, n=30 paired) found ZERO contribution from reward propagation** (29/30 identical trajectories); improvement re-attributed to curriculum + 10D + dictionary guidance | [`experiments/maze/graph_persistent_dg/`](experiments/maze/graph_persistent_dg/) |
...
- **Pre-registered sleep-only ablation — completed 2026-07-02, defeat recorded**: with 10D + curriculum held fixed (n=30 paired seeds), reward propagation contributed *nothing* — 29/30 pairs produced identical eval trajectories, and the one discordant seed went the wrong way. The stage-2 improvement is therefore **attributed to graph carryover + curriculum + dictionary guidance, not to sleep propagation** ([prereg + defeat record](docs/prereg/maze_sleep_ablation.md)). Registered suspects: value saturation under undirected max-propagation, and a floor effect from dictionary-based guidance. Redesign variants (SPEC-conformant propagation, F-driven sleep) each require a fresh pre-registration (retry budget N=2).
