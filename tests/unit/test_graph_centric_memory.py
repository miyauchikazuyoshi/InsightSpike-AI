...
class TestGraphCentricMemory:
    ...
    def test_graph_informed_integration(self, manager):
        ...
        mock_layer3 = Mock()
        mock_graph = Mock()
        mock_graph.edge_index = torch.tensor(
            [[0], [1]]
        )  # Connect 0 and 1 as PyTorch tensor
        ...
        manager.integration_config.content_overlap_threshold = (
        ...
        assert (
            stats.get("graph_assist_rate", 0) >= 0
        )  # May or may not have graph assistance
