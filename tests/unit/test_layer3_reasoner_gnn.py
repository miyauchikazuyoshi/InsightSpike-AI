...
sys.modules["torch_geometric.nn"] = types.SimpleNamespace(
    SAGEConv=lambda d1, d2: types.SimpleNamespace()
)
sys.modules["torch_geometric.data"] = types.SimpleNamespace(Data=object)

lgp_mod = types.SimpleNamespace(
    load_graph=lambda: types.SimpleNamespace(
        num_node_features=2, x=np.zeros((2, 2)), edge_index=None
    )
)
...
sys.modules["insightspike.embedder"] = types.SimpleNamespace(
    get_model=lambda: types.SimpleNamespace(
        encode=lambda x, normalize_embeddings=True: np.zeros((1, 2))
    )
)
sys.modules["insightspike.loader"] = types.SimpleNamespace(
    load_corpus=lambda: ["a", "b"]
)
