...
if not hasattr(torch, "randn"):
    torch.randn = lambda *args, **kwargs: MagicMock(
...
class TestScalableGraphBuilder:
    ...
    def test_incremental_update(self):
        ...
        graph2 = self.builder.build_graph(self.test_docs[10:20], incremental=True)
...
class TestScalableGraphManager:
    ...
    def setup_method(self):
        ...
        self.test_embeddings = [
            np.random.randn(384).astype(np.float32) for _ in range(50)
        ]
    ...
    def test_add_episode_node(self):
        ...
        result1 = self.manager.add_episode_node(
            self.test_embeddings[0], 0, {"text": "First episode"}
        )
        ...
        result2 = self.manager.add_episode_node(
            self.test_embeddings[1], 1, {"text": "Second episode"}
        )
    ...
    def test_importance_calculation(self):
        ...
        for i in range(10):
            self.manager.add_episode_node(
                self.test_embeddings[i], i, {"text": f"Episode {i}"}
            )
...
class TestGraphImportanceCalculator:
    ...
    def setup_method(self):
        ...
        self.graph = torch_geometric.data.Data(
            x=x, edge_index=torch.tensor(edges, dtype=torch.long).t()
        )
...
@pytest.mark.skipif(torch_geometric is None, reason="torch_geometric not available")
def test_integration():
    ...
    for i, doc in enumerate(docs):
        manager.add_episode_node(doc["embedding"], i, {"text": doc["text"]})
