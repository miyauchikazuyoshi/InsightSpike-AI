...
class TestL2EnhancedScalableMemory:
    ...
    def memory(self, mock_config, mock_embedder):
        ...
        with patch(
            "src.insightspike.core.layers.layer2_enhanced_scalable.get_config",
            return_value=mock_config,
        ):
            with patch(
                "src.insightspike.core.layers.layer2_enhanced_scalable.get_model",
                return_value=mock_embedder,
            ):
                with patch(
                    "src.insightspike.core.layers.layer2_memory_manager.get_model",
                    return_value=mock_embedder,
                ):
    ...
    def test_conflict_detection_and_splitting(self, memory, mock_embedder):
        ...
        with patch.object(memory.scalable_graph, "add_episode_node") as mock_add:
            ...
            with patch.object(
                memory.scalable_graph, "should_split_episode", return_value=True
            ):
    ...
    def test_split_by_conflict(self, memory, mock_embedder):
        ...
        memory.store_episode(
            "This is the first part. This is the second part.", c_value=0.8
        )
    ...
    def test_search_episodes_with_graph(self, memory, mock_embedder):
        ...
        with patch.object(
            memory.scalable_graph, "_calculate_node_importance"
        ) as mock_importance:
            mock_importance.side_effect = (
                lambda idx: 0.1 * idx
            )  # Higher index = higher importance
            ...
            with patch(
                ...
                with patch(
                    ...
                    new_memory = L2EnhancedScalableMemory(
                        dim=384, config=mock_config, use_scalable_graph=True
                    )
            ...
            with patch(
                ...
                with patch(
                    ...
                    memory = L2EnhancedScalableMemory(
                        dim=384, config=mock_config, use_scalable_graph=False
                    )
    ...
    def test_handle_conflict_split_with_candidates(self, memory, mock_embedder):
        ...
        with patch.object(
            memory.scalable_graph, "get_split_candidates"
        ) as mock_candidates:
    ...
    def test_error_handling_in_store_episode(self, memory):
        ...
        with patch.object(memory.scalable_graph, "add_episode_node") as mock_add:
            ...
            with patch(
                "src.insightspike.core.layers.layer2_memory_manager.logger"
            ) as mock_logger:
    ...
    def test_temporal_conflict_handling(self, memory, mock_embedder):
        ...
        memory.store_episode(
            "First observation", c_value=0.5, metadata={"timestamp": 1.0}
        )
        ...
        with patch.object(memory.scalable_graph, "add_episode_node") as mock_add:
            ...
            result = memory.store_episode(
                "Different observation", c_value=0.5, metadata={"timestamp": 1.5}
            )
