...
class TestL2MemoryManagerCore:
    ...
    def test_init_with_different_configs(self):
        ...
        # Test with config
        with patch('insightspike.core.layers.layer2_memory_manager.get_config') as mock_get_config:
            mock_config = Mock()
    ...
    def test_store_episode(self, memory):
        ...
        with patch('insightspike.core.layers.layer2_memory_manager.get_model') as mock_model:
            mock_embedder = Mock()
    ...
    def test_episode_integration(self, memory, mock_config):
        ...
        with patch('insightspike.core.layers.layer2_memory_manager.get_config', return_value=mock_config):
            with patch('insightspike.core.layers.layer2_memory_manager.get_model') as mock_model:
                ...
                memory.episodes.append(Episode(vec1, "Machine learning is fascinating", c=0.7))
                ...
                result = memory.store_episode("Machine learning is very fascinating", c_value=0.8)
                
                # Should still have 1 episode (integrated)
                assert len(memory.episodes) == 1
                assert "Machine learning is fascinating | Machine learning is very fascinating" in memory.episodes[0].text
