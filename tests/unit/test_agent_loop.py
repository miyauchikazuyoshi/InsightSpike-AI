...
sys.modules['networkx'] = types.SimpleNamespace(
    Graph=lambda: types.SimpleNamespace(
        add_nodes_from=lambda x: None,
        add_edges_from=lambda x: None
    )
)
sys.modules['rich'] = types.SimpleNamespace(print=lambda *a, **k: None)

# insightspike依存のダミーモジュール
class MockPath:
...
layer2 = types.SimpleNamespace(Memory=types.SimpleNamespace(
    search=lambda self, q, k: ([1.0], [0]),
    update_c=lambda self, idxs, r, eta=0.1: None,
    train_index=lambda self: None,
    merge=lambda self, idxs: None,
    split=lambda self, idx: None,
    prune=lambda self, c, i: None,
    add_episode=lambda self, v, t, c_init=0.2: None,
    save=lambda self: MockPath('p')
))
...
def test_adaptive_loop_max_iterations():
    ...
    assert len(current_k_values) == 4
    assert current_k_values[-1] == 20
    assert iterations == 4
    assert result is not None
