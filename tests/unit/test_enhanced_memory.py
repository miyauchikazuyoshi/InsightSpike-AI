...
class TestL2EnhancedScalableMemory:
    ...
    def memory(self):
        ...
        with patch("insightspike.utils.embedder.get_model", return_value=mock_embedder):
            with patch(
                "insightspike.core.layers.layer2_enhanced_scalable.get_model",
                ...
                with patch(
                    ...
                    with patch(
                        "insightspike.core.layers.layer2_enhanced_scalable.get_config",
    ...
    def test_conflict_detection_and_splitting(self, memory):
        ...
        memory.store_episode(
            "Python is compiled to bytecode", metadata={"type": "fact"}
        )
    ...
    def test_integration_with_similar_episodes(self, memory):
        ...
        memory.store_episode(
            "Machine learning is very powerful", metadata={"source": "B"}
        )
            ...
            if result and len(memory.episodes) > 0:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    if saved:
                        mock_config = memory.config
                        with patch(
                            "insightspike.utils.embedder.get_model"
                        ) as mock_get_model_load:
                            mock_embedder_load = Mock()
                            mock_embedder_load.get_sentence_embedding_dimension.return_value = (
                                384
                            )
                            ...
                            with patch(
                                "insightspike.core.layers.layer2_enhanced_scalable.get_model",
                                return_value=mock_embedder_load,
                            ):
                                with patch(
                                    "insightspike.core.layers.layer2_memory_manager.get_model",
                                    return_value=mock_embedder_load,
