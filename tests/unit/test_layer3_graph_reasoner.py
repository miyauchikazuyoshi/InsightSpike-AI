...
class TestL3GraphReasoner:
    ...
    def graph_reasoner(self, config_experiment):
        ...
        with patch(
            ...
            # Mock detect_spike to emulate simple threshold logic but still sometimes return a tuple
            def _mock_detect_spike(metrics, conflicts, thresholds):
                ...
                detected = high_ged and high_ig and low_conflict
                return (detected, 0.8 if detected else 0.1)
            reasoner.graph_analyzer.detect_spike = Mock(side_effect=_mock_detect_spike)
    ...
    def test_analyze_documents_with_previous_graph(self, graph_reasoner, mock_graph):
        ...
        assert isinstance(result["candidate_selection"], dict)
    ...
    def test_spike_bool_wrapper_context_score(self, monkeypatch, graph_reasoner):
        """SPIKE_BOOL_WRAPPER=1 で内部 context score が保持されることを検証."""
        monkeypatch.setenv("SPIKE_BOOL_WRAPPER", "1")
        spike = graph_reasoner.detect_eureka_spike(-1.2, 0.9)  # 強めのスパイク条件
        assert isinstance(spike, bool)
        ctx = getattr(graph_reasoner, "_last_spike_context", None)
        assert ctx is not None
        assert "context_spike_score" in ctx
        assert 0.0 <= ctx["context_spike_score"] <= 1.0
        assert ctx["metrics"]["delta_ged"] == -1.2
        assert ctx["metrics"]["delta_ig"] == 0.9

    def test_knowledge_counters(self, graph_reasoner):
        """Knowledge counters が更新され公開されることを確認."""
        # 初期値
        kc0 = graph_reasoner.get_knowledge_counters()
        assert kc0["updates"] == 0
        # ドキュメント分析
        docs = [{"text": "a", "embedding": np.random.randn(384)}, {"text": "b", "embedding": np.random.randn(384)}]
        result = graph_reasoner.analyze_documents(docs)
        assert result["graph"].num_nodes >= 2
        kc1 = graph_reasoner.get_knowledge_counters()
        assert kc1["updates"] >= 1
        assert kc1["total_facts"] >= 2
        # 再度呼び出しで updates 増加
        result2 = graph_reasoner.analyze_documents(docs)
        kc2 = graph_reasoner.get_knowledge_counters()
        assert kc2["updates"] >= kc1["updates"] + 1
