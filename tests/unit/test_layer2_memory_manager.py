...
class TestEpisodeStorage:
    ...
    def memory_manager(self):
        ...
        with patch(
            "insightspike.implementations.layers.layer2_memory_manager.EmbeddingManager"
        ) as mock_embedder:
            ...
            # Don't mock FAISS - let it use real implementation
            # The issue is that mocking faiss.IndexFlatL2 causes problems
            # when the actual code tries to create an index with the dimension
    ...
    def test_store_episode_with_metadata(self, memory_manager):
        ...
        idx = memory_manager.store_episode(
            "Test with metadata", c_value=0.7, metadata=metadata
        )
        assert idx == 0
...
class TestEpisodeSearch:
    ...
    def populated_manager(self):
        ...
        with patch(
            ...
            manager._get_embedding = Mock(
                side_effect=lambda text: manager.episodes[0].vec
                + np.random.randn(384) * 0.1
            )
    ...
    def test_search_episodes_with_filter(self, populated_manager):
        ...
        results = populated_manager.search_episodes(
            "learning", k=5, filter_fn=high_c_filter
        )
...
class TestEpisodeAging:
    ...
    def manager_with_old_episodes(self):
        ...
        config = MemoryConfig(
            enable_aging=True, aging_factor=0.9, min_age_days=1, max_age_days=30
        )
        manager = L2MemoryManager(config)
    ...
    def test_age_episodes_decay_calculation(self):
        ...
        config = MemoryConfig(
            enable_aging=True, aging_factor=0.9, min_age_days=1, max_age_days=100
        )
        manager = L2MemoryManager(config)
...
class TestEpisodeMerging:
    ...
    def manager_with_similar_episodes(self):
        ...
        episodes = [
            (
                ...
                base_vec + np.random.randn(384) * 0.01,
                0.8,
            ),
            (
    ...
    def test_find_most_similar_episodes(self, manager_with_similar_episodes):
        ...
        similar_pair = manager_with_similar_episodes._find_most_similar_episodes(
            num_candidates=4
        )
...
class TestSizeLimitEnforcement:
    ...
    def manager_near_limit(self):
        ...
        for i in range(12):  # Exceed limit
            ...
            episode = Episode(
                text=f"Episode {i}", vec=vec, c=max(0.1, c_value), timestamp=timestamp
            )
            manager.episodes.append(episode)
    ...
    def test_enforce_size_limit_basic(self, manager_near_limit):
        ...
        if initial_count > manager_near_limit.config.max_episodes:
            ...
            assert (
                len(manager_near_limit.episodes)
                <= manager_near_limit.config.max_episodes + 100
            )
    ...
    def test_enforce_size_limit_preserves_valuable(self, manager_near_limit):
        ...
        high_value_texts = [
            ep.text for ep in manager_near_limit.episodes if ep.c >= 0.7
        ]
...
class TestConflictDetection:
    ...
    def test_detect_conflicts_basic(self):
        ...
        episode2 = Episode(
            text="The system is not efficient",
            vec=vec1 + np.random.randn(384) * 0.01,
            c=0.7,
        )
    ...
    def test_texts_conflict_negation(self):
        ...
        result1 = manager._texts_conflict(
            "The model works well", "The model does not work well"
        )
        result2 = manager._texts_conflict(
            "Results are positive", "Results are not positive"
        )
...
class TestBackwardCompatibility:
    ...
    def test_memory_config_default_vector_index_fields(self):
        """MemoryConfig should auto-inject vector index defaults via normalizer.

        We construct a manager from a raw dict missing 'faiss_index_type' and 'metric'
        and verify the dynamic attributes plus tracking list are attached.
        """
        raw_config = {"memory": {"episodic_memory_capacity": 42}}
        manager = L2MemoryManager(raw_config)
        # The underlying normalized MemoryConfig lives on manager.config (legacy L2). For
        # our injected defaults we attached dynamic attributes on that object.
        cfg = manager.config
        # These attributes are not part of the pydantic schema, so use getattr with default.
        assert getattr(cfg, "faiss_index_type", None) == "FlatL2"
        assert getattr(cfg, "metric", None) == "l2"
        applied = getattr(cfg, "defaults_applied", [])
        # Order not strictly guaranteed; compare as set superset.
        assert set(["faiss_index_type", "metric"]) <= set(applied)
