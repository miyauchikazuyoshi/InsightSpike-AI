...
def test_reusability_comprehensive():
    ...
    for name, docs in document_sets:
        ...
        print(
            f"   {name}: Spike={result['spike_detected']}, "
    ...
    config_diversity = len(
        [c for c in agent_results.values() if c["total_insights"] > 0]
    )
    print(
        f"✅ Agent Configurations: {len(agent_results)} tested, {config_diversity} with insights"
    )

    # Reasoner analysis capability
    spike_detection_rate = len(
        [r for r in reasoner_results.values() if r["spike_detected"]]
    ) / len(reasoner_results)
    print(
        f"✅ Reasoner Analysis: {len(reasoner_results)} contexts, {spike_detection_rate:.1%} spike rate"
    )
    ...
    print(
        f"✅ Learning Persistence: {'Effective' if learning_effective else 'Needs tuning'}"
    )

    # Component integration
    integration_successful = (
        results["component_interoperability"]["interactions_logged"] > 0
    )
    print(
        f"✅ Component Interoperability: {'Successful' if integration_successful else 'Failed'}"
    )
