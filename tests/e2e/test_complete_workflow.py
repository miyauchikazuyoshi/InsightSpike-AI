...
class TestCompleteWorkflow:
    ...
    def test_knowledge_building_workflow(self, temp_data_dir):
        ...
        for question in questions:
            ...
            if "InsightSpike" in question:
                assert any("InsightSpike" in doc.get("text", "") 
    ...
    def test_insight_discovery_workflow(self, temp_data_dir):
        ...
        for i, concept in enumerate(concepts):
            ...
            if i >= 2:  # After some knowledge is built
                query_result = agent.process_question(
                    "What happens when systems combine?",
                    max_cycles=3
                )
        ...
        final_result = agent.process_question(
            "Explain the concept of emergence in system integration",
            max_cycles=5
        )
        ...
        assert any(term in response_lower for term in ["emergence", "combine", "together", "capabilities"])
...
""")
        ...
        result = subprocess.run(
            [sys.executable, "-m", "insightspike.cli.spike", "embed", str(knowledge_file)],
            capture_output=True,
            text=True,
        ...
        # Test query command
        result = subprocess.run(
            [sys.executable, "-m", "insightspike.cli.spike", "query", "What is InsightSpike?"],
            capture_output=True,
            text=True,
            env={**os.environ, "INSIGHTSPIKE_DATA_DIR": str(temp_data_dir)}
        )
    ...
    def test_cli_config_workflow(self, temp_data_dir):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "insightspike.cli.spike", "config", "export", str(config_file)],
            capture_output=True,
            text=True
        )
        ...
        result = subprocess.run(
            [sys.executable, "-m", "insightspike.cli.spike", "config", "validate", str(config_file)],
            capture_output=True,
            text=True
        )
...
class TestMultiModalWorkflow:
    ...
    def test_structured_data_workflow(self, temp_data_dir):
        ...
        structured_data = [
            {"concept": "Machine Learning", "definition": "Algorithms that improve through experience", "type": "technology"},
            {"concept": "Neural Networks", "definition": "Computing systems inspired by biological neurons", "type": "technology"},
            {"concept": "Deep Learning", "definition": "Neural networks with multiple layers", "type": "technology"},
            {"relationship": "Deep Learning", "uses": "Neural Networks", "strength": 0.9},
            {"relationship": "Neural Networks", "part_of": "Machine Learning", "strength": 0.8}
        ]
        ...
        assert result.success
        assert any(term in result.response.lower() for term in ["neural", "deep", "layer", "multiple"])
    ...
    def test_incremental_learning_workflow(self, temp_data_dir):
        ...
        agent.learn("This superposition enables quantum parallelism for certain algorithms.")
        ...
        assert "superposition" in result2.response.lower() or "qubit" in result2.response.lower()
        
        # Add connecting insight
        agent.learn("Quantum computing's power comes from exploiting superposition and entanglement together.")
