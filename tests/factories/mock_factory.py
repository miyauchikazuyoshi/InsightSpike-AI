...
class GraphFactory:
    ...
    @staticmethod
    def create_simple_graph(num_nodes: int = 5, num_edges: Optional[int] = None) -> nx.Graph:
        ...
        if num_edges is None:
            ...
            # Add random edges
            import random
            possible_edges = [(i, j) for i in range(1, num_nodes + 1) 
    ...
    @staticmethod
    def create_graph_with_embeddings(num_nodes: int = 5, embedding_dim: int = 384) -> nx.Graph:
...
class EmbeddingFactory:
    ...
    @staticmethod
    def create_similar_embeddings(base_embedding: np.ndarray, 
    ...
    def create_mock_embedding_model():
        ...
        mock_model = Mock()
        mock_model.encode = Mock(side_effect=lambda texts: 
                                np.random.randn(len(texts) if hasattr(texts, '__len__') else 1, 384))
        return mock_model
...
class MemoryFactory:
    ...
    @staticmethod
    def create_episodes(num_episodes: int = 5, 
                       embedding_dim: int = 384) -> List[MockEpisode]:
    ...
    def create_mock_memory_manager():
        ...
        mock_memory.store_episode = Mock(side_effect=lambda ep: mock_memory.episodes.append(ep))
...
class LLMFactory:
    ...
    def create_mock_llm_provider():
        ...
        mock_llm.generate_batch = Mock(side_effect=lambda prompts: 
