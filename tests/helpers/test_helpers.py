...
def assert_graphs_equal(g1: nx.Graph, g2: nx.Graph, 
                       check_attributes: bool = False) -> None:
    ...
    if check_attributes:
        for node in g1.nodes():
            assert g1.nodes[node] == g2.nodes[node], f"Node {node} attributes don't match"
        for edge in g1.edges():
            assert g1.edges[edge] == g2.edges[edge], f"Edge {edge} attributes don't match"


def assert_graphs_similar(g1: nx.Graph, g2: nx.Graph, 
                         node_tolerance: int = 2,
                         edge_tolerance: int = 3) -> None:
    ...
    assert node_diff <= node_tolerance, f"Node count differs by {node_diff}, max allowed: {node_tolerance}"
    assert edge_diff <= edge_tolerance, f"Edge count differs by {edge_diff}, max allowed: {edge_tolerance}"


def assert_embeddings_similar(emb1: np.ndarray, emb2: np.ndarray,
                            cosine_threshold: float = 0.9) -> None:
    ...
    assert cosine_sim >= cosine_threshold, f"Cosine similarity {cosine_sim} below threshold {cosine_threshold}"


def create_test_embedding(dim: int = 384, 
                         normalize: bool = True,
...
def create_embedding_batch(num_samples: int, 
                          dim: int = 384,
...
def assert_execution_time(max_seconds: float):
    ...
    assert elapsed <= max_seconds, f"Execution took {elapsed:.2f}s, max allowed: {max_seconds}s"
...
def assert_metric_in_range(value: float, 
                          expected: float,
                          tolerance: float = 0.1,
...
def create_test_layer_input(num_documents: int = 5,
...
class PerformanceMetrics:
    ...
    def assert_performance(self, name: str, 
                          max_mean: float,
                          max_std: Optional[float] = None) -> None:
