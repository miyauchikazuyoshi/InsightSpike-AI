...
def test_actual_insightspike_workflow():
    ...
    print(
        f"  ✅ 蓄積完了: {memory.graph.x.size(0)}エピソード, {memory.graph.edge_index.size(1)}接続"
    )
    ...
    for domain_id in range(len(domains)):
        # 各ドメインからの代表的なクエリ
        domain_indices = [
            i for i, label in enumerate(domain_labels) if label == domain_id
        ]
    ...
    if hasattr(memory.graph.edge_index, "size") and memory.graph.edge_index.size(1) > 0:
        ...
        if using_mock:
            ...
            global_knowledge_shape = (
                1,
                128,
            )  # Use tuple instead of torch.Size for mock compatibility
        else:
    ...
    for i in range(len(domains) - 1):
        domain1_indices = [
            idx for idx, label in enumerate(domain_labels) if label == i
        ][:5]
        domain2_indices = [
            idx for idx, label in enumerate(domain_labels) if label == i + 1
        ][:5]
    ...
    for test in conflict_tests:
        print(
            f"    • {test['comparison']}: 総合競合度 {test['conflicts'].get('total', 'N/A'):.3f}"
        )
    ...
    # 総合効果計算
    total_time = (
        episode_accumulation_time
        + reasoning_time
        + gnn_integration_time
        + conflict_detection_time
    )

    results["overall_performance"] = {
        "total_execution_time": total_time,
        "episodes_per_second": memory.graph.x.size(0) / episode_accumulation_time
        if episode_accumulation_time > 0
        else 0,
...
def generate_improvement_analysis(results):
    ...
    print(
        f"  • 処理速度: {speed_improvement} ({overall['episodes_per_second']:.1f}エピソード/秒)"
    )
...
def main():
    ...
    try:
        ...
        # 結果をJSONファイルに保存
        with open(
            "insightspike_performance_test_results.json", "w", encoding="utf-8"
        ) as f:
