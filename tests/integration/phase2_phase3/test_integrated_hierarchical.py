...
def test_rebuild_trigger():
    ...
    manager = IntegratedHierarchicalManager(
        dimension=384,
        cluster_size=20,
        rebuild_threshold=50  # 低い閾値で再構築をトリガー
    )
    ...
    for i in range(60):
        ...
        if (i + 1) % 10 == 0:
            ...
            print(f"   Episodes: {i+1}, Rebuilds: {rebuilds}, Compression: {stats['integration']['compression_ratio']:.1f}x")
...
def test_scalability():
    ...
    for batch_size in batch_sizes:
        ...
        print(f"{batch_size:>10,} | {total_docs:>10,} | {add_time:>12.2f}s | {avg_search_time*1000:>12.2f}ms")
    ...
    print(f"   Removed {opt_results['memory_optimization']['removed_count']} low-importance episodes")
...
def test_visualization():
    ...
    for conn in vis_data['connections'][:5]:
        print(f"   Level {conn['from']['level']} node {conn['from']['idx']} → "
