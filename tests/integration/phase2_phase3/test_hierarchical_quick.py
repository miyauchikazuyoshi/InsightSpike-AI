...
def quick_hierarchical_test():
    ...
    print(
        f"   Structure: {result['nodes_per_level'][0]} → {result['nodes_per_level'][1]} → {result['nodes_per_level'][2]}"
    )
    print(f"   Compression: {result['compression_ratio']:.1f}x")
    print(
        f"   Total edges: Level0={result['edges_per_level'][0]}, Level1={result['edges_per_level'][1]}, Level2={result['edges_per_level'][2]}"
    )
    ...
    if results:
        ...
        for i, res in enumerate(results[:3]):
            ...
            print(
                f"      Path: {' → '.join([f'L{level}[{idx}]' for level, idx in res['path']])}"
            )
...
def test_integrated_manager():
    ...
    manager = IntegratedHierarchicalManager(
        dimension=384, cluster_size=20, super_cluster_size=5, rebuild_threshold=50
    )
...
def test_scalability_simple():
    ...
    for size in sizes:
        ...
        builder = HierarchicalGraphBuilder(
            dimension=384, cluster_size=int(np.sqrt(size)), super_cluster_size=10
        )
        ...
        for i in range(size):
            ...
            vec = vec / np.linalg.norm(vec)
            docs.append(
                {"embedding": vec, "text": f"Document {i}", "metadata": {"id": i}}
            )
    ...
    for res in results:
        print(
            f"{res['size']:>8} | {res['build_time']:>10.2f} | {res['search_time']*1000:>12.2f} | {res['compression']:>12.1f}x"
        )
