...
def test_basic_rag_performance():
    ...
    for size in corpus_sizes:
        ...
        speedup = avg_standard_time / avg_hierarchical_time if avg_hierarchical_time > 0 else 1.0
        ...
        print(f"  Build time: Standard={result_info['standard_build']:.1f}ms, Hierarchical={result_info['hierarchical_build']:.1f}ms")
        print(f"  Search time: Standard={result_info['standard_search']:.2f}ms, Hierarchical={result_info['hierarchical_search']:.2f}ms")
...
def display_performance_summary(results: List[Dict]):
    ...
    print("\n\n=== Performance Summary ===")
    print("-" * 60)
    print(f"{'Size':>6} | {'Build Diff':>12} | {'Search(ms)':>12} | {'Speedup':>8} | {'Compress':>10}")
    print("-" * 60)
    
    for r in results:
        build_diff = r['hierarchical_build'] - r['standard_build']
        print(f"{r['size']:>6} | {build_diff:>+12.1f} | {r['hierarchical_search']:>12.2f} | "
    ...
    if len(results) > 1:
        ...
        search_time_ratio = results[-1]['hierarchical_search'] / results[0]['hierarchical_search']
