    ...
    def add_documents(self, documents: List[Dict[str, Any]]):
        ...
        for doc in documents:
            ...
            if 'embedding' in doc:
                ...
                # テキストから埋め込み生成
                embedding = self.model.encode(doc['text'], normalize_embeddings=True, convert_to_numpy=True)
...
def generate_test_corpus(size: int = 1000) -> List[Dict[str, Any]]:
    ...
    # トピック定義
    topics = {
        'AI': ['machine learning', 'neural networks', 'deep learning', 'artificial intelligence', 'algorithms'],
        'Climate': ['global warming', 'carbon emissions', 'renewable energy', 'climate change', 'sustainability'],
        'Finance': ['stock market', 'cryptocurrency', 'investment', 'banking', 'economics'],
...
def evaluate_rag_quality(results: List[Dict], expected_topic: str) -> Dict[str, float]:
    ...
    # 精度: 期待トピックと一致する結果の割合
    correct = sum(1 for r in results if r.get('metadata', {}).get('topic') == expected_topic)
...
def test_rag_performance():
    ...
    for size in corpus_sizes:
        ...
        for query, expected_topic in test_queries:
            ...
            standard_quality.append(evaluate_rag_quality(standard_results, expected_topic))
            
            # 階層的RAG検索
            start = time.time()
            hierarchical_results = hierarchical_rag.search(query, k=10)
            hierarchical_times.append(time.time() - start)
            hierarchical_quality.append(evaluate_rag_quality(hierarchical_results, expected_topic))
        ...
        result = {
            ...
            'hierarchical_precision': np.mean([q['precision'] for q in hierarchical_quality]),
            'standard_relevance': np.mean([q['relevance'] for q in standard_quality]),
            'hierarchical_relevance': np.mean([q['relevance'] for q in hierarchical_quality]),
            'speedup': np.mean(standard_times) / np.mean(hierarchical_times),
            'compression': hierarchical_rag.get_statistics()['integration']['compression_ratio']
        }
        ...
        print(f"   Build time: Standard={standard_build_time:.2f}s, Hierarchical={hierarchical_build_time:.2f}s")
        print(f"   Avg search: Standard={result['standard_avg_search']:.2f}ms, Hierarchical={result['hierarchical_avg_search']:.2f}ms")
        print(f"   Speedup: {result['speedup']:.2f}x")
        print(f"   Precision: Standard={result['standard_precision']:.2%}, Hierarchical={result['hierarchical_precision']:.2%}")
...
def test_edge_cases():
    ...
    different_docs = []
    topics = ['quantum physics', 'ancient history', 'marine biology', 'abstract art', 'culinary arts']
...
def display_final_results(results_table: List[Dict]):
    ...
    print("-" * 80)
    print(f"{'Size':>8} | {'Build(s)':>10} | {'Search(ms)':>12} | {'Speedup':>8} | {'Precision':>10} | {'Compress':>10}")
    print("-" * 80)
    
    for r in results_table:
        print(f"{r['corpus_size']:>8} | "
    ...
    avg_precision_diff = np.mean([r['hierarchical_precision'] - r['standard_precision'] for r in results_table])
