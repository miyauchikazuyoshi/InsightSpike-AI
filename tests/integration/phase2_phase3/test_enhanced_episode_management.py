...
def demonstrate_enhanced_features():
    ...
    for i, text in enumerate(test_documents):
        ...
        if result["success"]:
            ...
            if integration_info.get("total_integrations", 0) > 0:
                print(
                    f"  → Integrated! Total integrations: {integration_info['total_integrations']}"
                )
                print(
                    f"    Integration rate: {integration_info['integration_rate']:.1%}"
                )
            ...
            if splitting_info.get("episodes_split", 0) > 0:
                print(
                    f"  ⚡ Split occurred! Episodes split: {splitting_info['episodes_split']}"
                )
                print(
                    f"    New episodes created: {splitting_info['new_episodes_created']}"
                )
        else:
    ...
    print(
        f"    Successful integrations: {integration_stats.get('successful_integrations', 0)}"
    )
...
def test_specific_scenarios():
    ...
    if splitting_info.get("episodes_split", 0) > 0:
        print(
            f"✓ Document was split! New episodes: {splitting_info['new_episodes_created']}"
        )
