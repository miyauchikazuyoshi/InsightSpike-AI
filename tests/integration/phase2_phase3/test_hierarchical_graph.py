...
def test_small_scale():
    ...
    print(
        f"   Compression ratios: {[f'{r:.1f}x' for r in stats['compression_ratios']]}"
    )
    if "cluster_size_distribution" in stats:
        dist = stats["cluster_size_distribution"]
        print(
            f"   Cluster sizes: mean={dist['mean']:.1f}, std={dist['std']:.1f}, range=[{dist['min']}, {dist['max']}]"
        )
...
def test_large_scale():
    ...
    print(
        f"   Memory compression: {final_stats['nodes_per_level'][0] / max(1, final_stats['nodes_per_level'][2]):.1f}x"
    )
    ...
    for i in range(test_queries):
        ...
        if i == 0:
            print(
                f"   First query: {search_time*1000:.2f}ms, found {len(results)} results"
            )
...
def test_scalability_comparison():
    ...
    for size in sizes:
        ...
        builder = HierarchicalGraphBuilder(
            dimension=384, cluster_size=int(np.sqrt(size)), super_cluster_size=50
        )
    ...
    print("\n" + "=" * 70)
    print(
        f"{'Size':>10} | {'Build(s)':>10} | {'Search(ms)':>12} | {'Compression':>12} | {'Structure'}"
    )
    print("=" * 70)
    ...
    for res in results:
        ...
        print(
            f"{res['size']:>10,} | {res['build_time']:>10.2f} | {res['search_time']*1000:>12.2f} | {res['compression']:>12.1f}x | {levels_str}"
        )
