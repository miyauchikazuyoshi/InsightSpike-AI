...
def test_basic_functionality():
    ...
    queries = ["machine learning", "climate science", "AI applications"]
...
def test_importance_dynamics():
    ...
    for idx in indices:
        ...
        print(
            f"  Episode {idx}: {info['importance']:.3f} (accesses: {info['access_count']})"
        )
    ...
    old_episode_idx = agent.l2_memory.add_episode(
        agent.l2_memory.episodes[0].vec, "Old unused content"
    )
    # 古いタイムスタンプを設定
    agent.l2_memory.episodes[old_episode_idx].metadata["last_access"] = (
        time.time() - 40 * 86400
    )
