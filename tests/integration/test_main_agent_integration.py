...
class TestMainAgentInitialization:
    ...
    def test_config_determines_memory_dimension(self, mock_datastore):
        ...
        config = InsightSpikeConfig(
            embedding={"model_name": "sentence-transformers/all-mpnet-base-v2", "dimension": 768}
        )
...
class TestMainAgentProcessing:
    ...
    def test_process_with_multiple_cycles(self, agent):
        ...
        agent.l2_memory.store_episode("It discovers insights through synthesis.", c_value=0.9)
        
        result = agent.process_question(
            "How does InsightSpike work?",
            max_cycles=3,
            verbose=True
        )
    ...
    def test_spike_detection(self, agent):
        ...
        if agent.l3_graph:
            with patch.object(agent.l3_graph, 'analyze_documents') as mock_analyze:
                ...
                result = agent.process_question(
                    "What happens when A and B integrate?",
                    max_cycles=2
                )
    ...
    def test_error_handling(self, agent):
        ...
        with patch.object(agent.l2_memory, 'search_episodes', side_effect=Exception("Search error")):
...
class TestMainAgentInsights:
    ...
    def test_search_insights(self, agent):
        ...
        agent.l2_memory.store_episode("Emergence occurs in complex systems", c_value=0.9)
...
class TestMemoryIntegration:
    ...
    def test_memory_retrieval_in_processing(self, agent):
        ...
        # Pre-populate memory
        agent.l2_memory.store_episode("InsightSpike is an AI system for insights.", c_value=0.8)
        ...
        assert any("InsightSpike" in doc.get("text", "") for doc in result.retrieved_documents)
