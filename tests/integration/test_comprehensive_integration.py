...
def test_memory_efficiency_comparison():
    ...
    try:
        ...
        large_subgraph = memory.get_subgraph(list(range(0, min(20, memory.graph.x.size(0)))))
        print(f"✓ Large subgraph: {large_subgraph.x.size(0)} nodes, {large_subgraph.edge_index.size(1)} edges")
        ...
        print(f"✓ Cluster 1 subgraph: {cluster1_subgraph.x.size(0)} nodes, {cluster1_subgraph.edge_index.size(1)} edges")
...
def test_gnn_information_processing():
    ...
    try:
        ...
        print(f"✓ Knowledge graph: {data.x.size(0)} nodes, {data.edge_index.size(1)} edges")
