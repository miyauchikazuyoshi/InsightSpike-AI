        ...
        for i, episode in enumerate(episodes):
            try:
                ...
                self.assert_condition(
                    "graph_analysis" in result,
                    f"Episode {i+1}: graph_analysis present"
                )
        ...
        for question in questions:
            try:
                ...
                self.assert_condition(
                    ...
                    f"Question returns dict result"
                )
                
                self.assert_condition(
                    "response" in result,
                    f"Response contains 'response' field"
                )
                
                self.assert_condition(
                    len(result.get("response", "")) > 0,
                    f"Response is not empty"
                )
...
class PerformanceScenario(TestScenario):
    ...
    def run(self) -> bool:
        ...
        self.assert_condition(
            memory_mb < 1000,  # 1GB以下
            f"Memory usage < 1GB (actual: {memory_mb:.0f}MB)"
        )
...
def run_integration_tests():
    ...
    for result in results:
        ...
        print(f"{status} {result['scenario']}: {result['passed']}/{result['total']} ({result['success_rate']*100:.0f}%)")
