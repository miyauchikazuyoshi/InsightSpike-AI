...
def main():
    ...
    graph_data = {
        ...
        'edges': [
            {'source': 'node1', 'target': 'node2', 'type': 'related', 'attributes': {'weight': 0.5}},
            {'source': 'node2', 'target': 'node3', 'type': 'related', 'attributes': {'weight': 0.7}}
        ]
    }
    ...
    if loaded_graph:
        print(f"✓ Loaded graph with {len(loaded_graph['nodes'])} nodes and {len(loaded_graph['edges'])} edges")
