...
from src.insightspike.implementations.datastore.sqlite_store import SQLiteDataStore, FAISSVectorIndex
...
async def test_sqlite_datastore():
    ...
    # Get subgraph
    subgraph = await store.get_subgraph(["node1", "node2", "node3"], namespace="test_graph")
    ...
    batch_results = await store.search_vectors_batch(query_vectors, k=2, namespace="test")
