...
6. **構造的確証バイアス**: 本調査は設問自体が「空き地の区別が最重要」と指定し、5 角度のうち 1 つを
   空き地プローブ専用に割いた — **空き地を探しに行った調査が空き地を見つけた**構造である。
   検証は敵対的(3 票)だが、探索の方向づけ自体は geDIG に好意的。§9 の逆引き(反証努力)が
   その対抗策であり、逆引き完了までは「空き地」を根拠にした新規性主張を外部に出さない。
...
- **「忘却」が最も鮮烈な空き地**。分野は「時間切れ削除か、容量上限追い出しか、何もしないか」しかない
  ...
  直接の対案。そして origin story 167 行が半年前にこれを書いていた。動的迷路実験の方向は、
  **分野自身の未解決宣言と一致している**(「裏付け」とまでは言わない — §0-6 のバイアス注意)。
...
- **2 つの空き地は、geDIG の 2 つの主張とちょうど一致する**。(a)=「when to accept/restructure/forget を
  ...
  最終目標)。理論は空き地を狙って作られたものではない(origin story は 2026-01、空き地の認識は今日)
  — ただし §0-6 のとおり**空き地はこちらが探しに行って見つけたものでもある**。一致の強さの評価は
  §9 の逆引き完了後に確定する。
...
2. **最優先の狙撃対象は「忘却」**。分野が最も粗雑(§4)、geDIG が最も鋭い対案(F 裁定)、
   origin story が半年前に予言、動的迷路で安く検証できる。次の迷路実験(動的迷路)の方向は
   **分野の未解決宣言と一致**(バイアス注意は §0-6)。
...
**空き地プローブ(未精読・未活用、§9 で確認予定)**: EvolMem [2601.03543] / LongMemEval-V2 [2605.12493] /
[2406.14546] / [2606.01223](後 2 者は fetch 済みだが findings に非昇格 = 中身未活用)
