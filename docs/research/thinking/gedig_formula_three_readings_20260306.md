...
### 4.3 Helmholtz 対応は 3 つの読みの中でこの形が正しい

3 つの読みの中で、Helmholtz `F = U - TS` への対応は **読み 2（`(EPC - B) - H`）が確実に正しい**と判断する。
...
| `T`（温度） | `λ`（情報温度） | 利得の相対重み、annealing スケジュール候補（§4.4） |
| `S`（エントロピー） | `H`（Shannon エントロピー） | 確率側の未整理性 |
| `F < 0` で自発過程 | `F < 0` で閃き発火 | 「起こりやすい」方向 |

**なぜ `EPC - B` が `U` に対応するのか**:

- `EPC` 単体は「編集の総量」だが、そのうち `B = β₁` 分（冗長サイクル生成/消失のぶん）は位相秩序の変化であって、内部エネルギーの本質とは言えない
- `EPC - β₁` は「冗長サイクルを除いた本質構造の変化コスト」となり、これが熱力学的な **内部エネルギー U** の自然な離散類推になる
- Bourbaki 的視点（[insight_bourbaki_three_structures.md](insight_bourbaki_three_structures.md)）と組み合わせると、計量（EPC）から位相（β₁）を差し引いた量 = **物質的な状態量**に近づく

**読み 1（canonical）との違い**:

- 読み 1 は `EPC - (H + B) = cost - gain` の**経済学的収支**として読む。これは geDIG の AG/DG 制御に直結する工学的表現で、実装・論文本文の主軸
- 読み 2 は**同じ式の別括り**で、Helmholtz との構造的対応を見るためのレンズ
- 2 つは**代数的に同値**、**意味論的に補完**

### 4.4 λ を情報温度として動的に制御する（補足）

Helmholtz 読みを受け入れるなら、λ は単なるハイパーパラメータではなく **温度** として振る舞う。これは **simulated annealing (SA)** と構造的に一致する:

- SA の受容確率: `P(accept) = exp(−ΔE / T)`
- geDIG の受容判定: `F < 0`（決定的閾値版）

λ は「受容閾値の逆温度」として機能し、以下のような動的制御の余地がある:

- **Wake 初期（探索）**: λ 高 → 情報利得の重視 → 多様な探索
- **Sleep（consolidation）**: λ 徐々に低下 → 安定構造を選好
- **Wake' （収束）**: λ 低 → 確定構造の活用

これは現行の quantile-calibrated な**静的閾値設定**を、**動的温度制御**に拡張する方向を示唆する。

**検証可能な仮説 H_temp**: λ の温度スケジューリングは、固定 λ よりも迷路/RAG の探索効率を改善する。
実験候補:
...
には有効。  
ただし canonical として前面に出すと、実装との距離が広がる。読み 1 を canonical、読み 2 を secondary interpretation とする原則（§7）を守る。
