...
### Episode (エピソード)
**定義（理論）**: 知識の基本単位。**局所文脈（状態）**・**操作（行為／クエリ）**・**帰結（観測・成否・報酬近傍）**からなる最小まとまり。

エピソード間の遷移（記憶再生／推論の飛び先）は、グラフ上のエッジとして表す。

**抽象分解（スキーマ）**（表現はドメイン依存）:
- `context`（文脈／状態）
- `operation`（操作）
- `affordance`（アフォーダンス：可用性／制約／近傍性など）
- `salience`（顕著性：頻度／重要度）
- `outcome`（帰結：受容/棄却、成否、観測など）
- `goal`（目標）
...
**定義（理論）**: エピソード（ノード）とその遷移（エッジ）からなる、**動的な知識グラフ**としての記憶。

- ノード: エピソード（観測・操作・帰結の最小まとまり）
- エッジ: 想起／推論の飛び先／遷移（近傍・短絡・再配線など）
- 操作: エピソードの受容・結合・棄却・保留、再配線、剪定（エビクション）など

> メモ: 表現（埋め込み）はドメイン依存だが、運用は単一ゲージとゲート（AG/DG）で横断制御する、というのが geDIG の立て付け。

### Embedding Space Φ (埋め込み空間Φ)
**役割**: エピソード間の近傍（類似度）を定義し、候補集合や確率分布（エントロピー）推定の基盤になる。

**要件（最小）**:
- **意味勾配保存**: 意味的に近いほど距離が小さい
- **スケール正規化**: ノルムや距離スケールが揃い、閾値/分位が安定する
- **局所滑らかさ**: 入力の微小変化に対して埋め込みが連続的に変化する
