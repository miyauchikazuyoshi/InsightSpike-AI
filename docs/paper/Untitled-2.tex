...
%====================================================================
\begin{abstract}
人間の「洞察」は、単なる情報量の増加ではなく、知識の\textbf{構造的再編成}によって生じる。%
本研究では、この認知プロセスをモデル化するため、脳の海馬リプレイに着想を得た%
内発的動機付けフレームワーク\emph{geDIG}を提案する。%
geDIGは、海馬におけるシナプス刈り込み（構造の単純化）と%
情報圧縮（情報の整理）という2つのプロセスを、%
グラフ編集距離（$\Delta$GED）と情報利得（$\Delta$IG）に写像し、%
単一の報酬指標として統合する。%
階層的知識ベースを用いた実験により、本フレームワークが%
従来手法では不可能だった多層概念の統合を検出できることを示した。%
特に、問題が複雑なほど検出精度が向上する（難問で\textbf{100\%}）という%
\textbf{「難易度逆転現象」}を発見した。%
これは、geDIGが既存の統計的学習とは異なる、洞察の核心的メカニズムを%
捉えていることを示唆する。本稿は、構造変化を報酬とする初の試みであり、%
追跡・修正可能な仮説生成AIへの道筋を示すものである。
\end{abstract}
