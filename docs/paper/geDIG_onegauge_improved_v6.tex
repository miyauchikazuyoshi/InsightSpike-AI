...
本節の主目的は、\textbf{未知迷路における探索効率の改善}を定量化することである。代表的な設定（15/25/50\,\mbox{$\times$}\,15 迷路）では、geDIG は成功率 95--100\% を維持しつつ、ステップ数をベースライン比で約 20--30\% 削減し、候補エッジの約95\% を剪定できたことを確認した（詳細は表~\ref{tab:maze_v4_main}）。参照（Dijkstra／A*）は\,\emph{理想上限}として整合度（Regret／SPL や順位相関）で扱う\footnote{Regret はオラクル（最短路）との差の累積。SPL（Success weighted by Path Length）は成功時の効率（実行経路長と最短経路長の比）を表す。}。代表結果（25\,\mbox{$\times$}\,25, \,$N{=}32$；L3\,only, max\,steps=250）の要点を先にまとめる：
\begin{itemize}[leftmargin=1.4em]
