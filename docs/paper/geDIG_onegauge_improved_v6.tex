...
Dynamic knowledge graphs lack a normative criterion for \emph{when} to accept new information, even as RAG systems excel at optimizing \emph{what} to retrieve. We propose \textbf{geDIG}, a unified control framework that uses a single gauge $\F$ combining normalized structural cost (edit-path cost) and information gain (Shannon entropy difference and path shortening), and a two-stage gating mechanism (AG at 0-hop; DG via multi-hop) to control exploration and integration decisions. We evaluate geDIG on a maze proof-of-concept and a practical RAG pipeline, and report consistent reductions in redundant exploration and knowledge contamination under an operating-curve protocol (PSZ). We release a reproducible implementation and scripts.
\end{abstract}
