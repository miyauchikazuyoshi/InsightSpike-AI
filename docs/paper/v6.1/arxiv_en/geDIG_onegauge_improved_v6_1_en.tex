...
\title{A Unified Gauge Framework for Dynamic Knowledge Graphs\\[0.3em]
  \large geDIG v6.1: One-Gauge Control of Static and Dynamic RAG}
\author{Kazuyoshi Miyauchi\\
  \small\texttt{miyauchikazuyoshi@gmail.com}}
\date{Draft (v6.1, English) --- claims aligned with measured results; no new experiments vs.\ v6}
...
We propose \textbf{geDIG}, a unified control framework that uses a \emph{single gauge} $\F$ combining normalized structural cost (edit-path cost; $\gednorm$) and information gain (Shannon entropy difference and path shortening; $\ignorm$), and a \textbf{two-stage gating mechanism}---AG at 0-hop for ambiguity detection and DG via multi-hop for shortcut confirmation---to control exploration and integration decisions in an event-driven manner.
Evaluations on a partial-observation maze PoC and a lightweight RAG pipeline (500 synthetic queries) show that the single gauge drives event-driven control in real time: in the maze it prunes about 98--99\% of candidate edges while keeping success close to a greedy-DFS baseline, and in RAG it enables selective acceptance that improves answer quality (PER $0.17\to0.42$) over non-accepting baselines. The measured False Merge Rate (FMR $\approx 0.63$), however, remains far above the PSZ target, so contamination control is stated explicitly as future work.
...
\noindent\textbf{Sign convention}: $\Delta H_{\mathrm{norm}}=(H_{\mathrm{after}}-H_{\mathrm{before}})/\log K$ (ordering: a decrease is \emph{negative}), $\;\Delta\mathrm{SP}_{\mathrm{rel}}=(L_{\mathrm{before}}-L_{\mathrm{after}})/\max\{L_{\mathrm{before}},\varepsilon\}$ (shortening is \emph{positive}).
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{H1 (Thermodynamic Inference; outlook)}: If inference behaves as free-energy minimization, real Transformer attention should produce lower geDIG $\F$ than random/uniform baselines. This hypothesis motivates the Transformer outlook (\S\ref{sec:limits_future}) and is \emph{not} tested in this paper. (New in v6)
\end{itemize}
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{Principle verification in Maze and RAG}: In a partial-observation maze PoC and a 500-query synthetic RAG suite, the single gauge and two-stage gates operate stably in real time, achieving 98--99\% edge pruning in the maze and a PER improvement ($0.17\to0.42$) over non-accepting baselines in RAG. False-acceptance suppression (measured FMR $\approx 0.63$) and per-component ablation separation are \emph{not yet achieved}; we state both explicitly as open issues.
  ...
  \item \textbf{Semantic Alignment (supplemental)}: DG-confirmed subgraphs show a small but statistically significant directional alignment with answer embeddings ($\Delta s \approx +0.021$, $N{=}124$, sign test $p\ll0.001$). This is a \emph{preliminary, correlational} observation consistent with --- but not validating --- the reading that minimizing $\F$ corresponds to reasoning; no causal claim is made. (New in v6)
\end{itemize}
...
\paragraph{Generalizability}\; Environment-dependent conditions in pseudo-code (e.g., wall check in maze) are kept in \emph{preprocessing/representation layer}, while \textbf{decision criteria for acceptance/exploration/pruning themselves are commonized by $\F$ and AG/DG rules}. By replacing representation design, the same control principle can be applied to other domains, as demonstrated in the RAG experiments below.
...
All methods are evaluated with the same embedding model, ANN index, top-$k$, answer-synthesis settings, and hardware budget; no future data or labels are used at test time (no-peeking). Note that the lite suite synthesizes answers from retrieved documents by templates (\textbf{no LLM generation}); PER is a token-overlap proxy that measures retrieval/acceptance quality, not generation quality. LLM-based generation and real corpora (e.g., HotpotQA) are future work.
...
Table~\ref{tab:maze_main_result} shows the measured $15{\times}15$ results ($n{=}100$).
geDIG reaches a \textbf{98\% success rate --- on par with a plain greedy-DFS baseline (99\%) and oracle BFS (99\%), not exceeding them}.
The distinctive result is the \textbf{very high map compression ($\sim$98\%)}: $\F$-driven pruning leaves only the topological skeleton (intersections/dead ends) and removes straight-line redundancy. The PoC's contribution is the emergent AG/DG control (no hard-coded rules) and this compression, not raw success superiority.

\begin{table}[H]
    \centering
    \caption{Maze PoC Results ($15{\times}15$, $n{=}100$, measured from \texttt{docs/paper/data/maze\_15x15\_*.json}). geDIG matches the simplest complete baselines on success/steps; its distinctive property is high compression. Oracle (BFS) assumes a known map and is an upper-bound reference.}
    \label{tab:maze_main_result}
    \small
    \begin{tabular}{lcccc}
    \toprule
    Method & Success & Steps & Oracle Ratio & Compression \\
    \midrule
    Random Walk & 0.75 & 141.9 & 3.0x & - \\
    Greedy DFS & 0.99 & 83.7 & 1.7x & - \\
    Oracle (BFS, Ref) & 0.99 & 48.0 & 1.0x & - \\
    \textbf{geDIG (Proposed)} & 0.98 & 92.8 & 1.9x & \textbf{0.98} \\
    \bottomrule
    \end{tabular}
\end{table}
...
Upon entering a dead end, observed options vanish. $\Delta\mathrm{EPC}$ increases (wall/collision cost), or $\Delta\mathrm{IG}$ drops (no entropy reduction). $g_0$ shoots up, exceeding $\theta_{\mathrm{AG}}$.
...
\subsection{Metric 2: Dead-End Detection Delay (Preliminary)}
We measured the ``hesitation'' steps between hitting a dead end and initiating a turn/backtrack.
\textit{Note: the figures below come from earlier prototype logs; packaged reproduction scripts for these specific measurements are not yet available, so treat them as indicative.}
\begin{itemize}
    \item \textbf{Random}: $\approx 8.5$ steps (wanders around).
    \item \textbf{Greedy}: $\approx 1.0$ step (immediate algorithmic turn).
    \item \textbf{geDIG}: $\approx 1.4$ steps. Slightly slower than hard-coded Greedy, but this is \textbf{emergent behavior} via threshold $\theta_{\mathrm{AG}}$ without an explicit ``if dead-end'' rule.
\end{itemize}
This suggests that $\F$-based control can autonomously detect ambiguity and decide to switch modes, acting as a soft error detection mechanism.

\subsection{Robustness to Noise (Jitter Stability; Preliminary)}
To probe \verb|StabilityAudit|, we injected random noise ($\pm 5\%$) into edge costs and thresholds in earlier prototype runs.
With Audit enabled, false positive DG firings (hallucinated shortcuts) dropped sharply (roughly $-85\%$) while true shortcuts were largely preserved ($\approx$98\% recall).
\textit{These figures are preliminary (prototype logs; reproduction scripts not yet packaged)} but suggest the ``margin check'' $m(t)$ helps ensure robust structural updates.
...
\begin{enumerate}
    ...
    \item \textbf{Compression is evident}: $\sim$98\% edge compression indicates it accurately captures the topological skeleton.
    \item \textbf{Scale behavior}: at $15{\times}15$ geDIG reaches 98\% ($n{=}100$); at $25{\times}25$ success drops to $\sim$56--75\% depending on configuration (preliminary, $n{=}60$), indicating headroom rather than scale-invariance.
\end{enumerate}
This justifies proceeding to the RAG experiment. Although spatial and semantic graphs differ in structure, they share the common framework of structural cost versus information gain. We map ``Dead-end'' to ``Unknown/Hallucination'' and ``Shortcut'' to ``Insight/Logical Connection''.

% -----------------------------------------------------------
\section{Experiment II: Static RAG (Baseline Establishment)}
\label{sec:rag_experiment}

\subsection{Objective: Quantifying the Behavior of Static Criteria}
We quantify the behavior of static retrieval (Static Top-$k$) and heuristic filters (Frequency / Cosine) in the lite suite to establish a baseline for geDIG.
\textbf{Core Question}: Can a static criterion alone provide a useful ``When to accept'' signal?
(Measured answer: no --- in our harness these baselines never accept an update; they yield PER $\approx 0.17$ with no KG growth.)

\subsection{Experimental Conditions (Common)}
\paragraph{Dataset}\;
\begin{itemize}
    \item \textbf{Source}: Synthetic multi-hop QA corpus generated by \texttt{generate\_dataset.py} (supporting/distractor documents with templated answers). Evaluation on HotpotQA / 2WikiMultihopQA is future work.
...
Table~\ref{tab:rag_static_result} summarizes the measured lite-suite results.
\begin{itemize}
    \item \textbf{Static baselines (Static / Frequency / Cosine)}: identical answer quality (PER $\approx 0.17$) and \textbf{zero acceptance} --- they never update the KG in this harness, so acceptance-side FMR is degenerate (undefined).
    \item \textbf{Implication}: a static criterion alone provides no ``When'' signal; any update behavior must come from an explicit gate. The PSZ question is therefore decided entirely by the dynamic gate (next section).
\end{itemize}

\begin{table}[H]
    \centering
    \caption{Static RAG results (lite suite, $N=500$ synthetic queries; measured). The baselines accept nothing, so acceptance-side FMR is degenerate.}
    ...
    \midrule
    Static Top-$k$ & 0.172 & 0.000 & -- & 160 \\
    Frequency & 0.172 & 0.000 & -- & 160 \\
    Cosine threshold & 0.172 & 0.000 & -- & 160 \\
    \textbf{Target PSZ} & -- & \textbf{$\ge$0.95} & \textbf{$\le$0.02} & \textbf{$\le$200} \\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{AG Fire}: If ambiguity is high ($g_0 > \theta_{\mathrm{AG}}$), expand retrieval (retrieve neighbors).
    \item \textbf{DG Fire}: If multi-hop gain high ($\F < \theta_{\mathrm{DG}}$), \textbf{commit} to context.
\end{itemize}

\subsection{Results: Acceptance Turns On; Selectivity Does Not Yet}
(See Figure~\ref{fig:psz_operating_curves_lite_ja} in the appendix.)
\begin{itemize}
    \item \textbf{Measured shift}: geDIG raises acceptance from 0.0 to \textbf{0.374} and PER from 0.172 to \textbf{0.421}; the PSZ shortfall $s_{\mathrm{PSZ}}$ shrinks from 1.93 to \textbf{1.22}.
    \item \textbf{Open issue}: \textbf{FMR $\approx$ 0.626}, far above the PSZ target ($\le$0.02). The gate decides \emph{when} to accept, but does not yet discriminate \emph{what} deserves acceptance.
    \item \textbf{Latency cost}: P50 increases from 160ms to \textbf{240ms} (+80ms for graph reasoning), within the interactive range.
\end{itemize}

\begin{table}[H]
    \centering
    \caption{Dynamic RAG results (lite suite; measured). geDIG turns on selective acceptance and improves PER, but FMR remains far above the PSZ target.}
    \label{tab:rag_dynamic_result}
    \small
    \begin{tabular}{lccccc}
    \toprule
    Method & PER & Acc & FMR & P50(ms) & $s_{\mathrm{PSZ}}$ \\
    \midrule
    Static Top-$k$ & 0.172 & 0.000 & -- & 160 & 1.93 \\
    \textbf{geDIG (Prop)} & \textbf{0.421} & \textbf{0.374} & \textbf{0.626} & \textbf{240} & \textbf{1.22} \\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{AG (red)}: fires on ambiguous queries, triggering retrieval expansion (``checking behavior'').
    \item \textbf{DG (blue)}: fires when a candidate clears the confirmation threshold (``discovery of a connection'').
    \item \textbf{Interpretation}: firing rates vary with the calibrated quantiles and run configuration; the qualitative pattern is frequent checking (AG) and sparser confirmation (DG), with most queries handled by the default path.
\end{itemize}

\subsection{Design Rationale: Structural Validation (and Its Current Gap)}
Static thresholds look only at ``pairwise similarity'' and cannot distinguish ``semantically close but irrelevant (distractor)'' from ``bridge to answer''.
geDIG is \emph{designed} to use \textbf{structural gain} ($\Delta\mathrm{IG}, \Delta\mathrm{SP}$): a distractor adds nodes ($\Delta\mathrm{EPC}\uparrow$) without creating bridges ($\Delta\mathrm{SP}\approx 0$), keeping $\F$ high $\to$ reject; a true bridge shortens paths ($\Delta\mathrm{SP}\uparrow$), lowering $\F$ $\to$ accept.
The measured FMR ($\approx$0.63), however, shows that this discrimination is \emph{not yet realized} in the lite suite; sharpening it (stricter DG calibration, evidence checks, rollback) is the main open problem.

% -----------------------------------------------------------
\section{Experiment IV (Supplemental): Insight Vector Alignment (LLM Alignment)}
\label{sec:alignment_experiment}

\subsection{Objective: Directional Alignment of Insight (Correlational)}
The ultimate goal of geDIG is not just to filter documents, but to \textbf{support reasoning}.
If DG-confirmed subgraphs are indeed ``structures representing insight'', their vector representation (Insight Vector) should align with the \textbf{semantic direction of the answer}.
This experiment tests directional alignment only; \emph{it does not establish causality} (causal intervention is future work).
...
\subsection{Results: Small but Consistent Positive Shift}
\begin{enumerate}
    \item \textbf{Significance}: $\Delta s > 0$ in the majority of cases ($N{=}124$); sign test $p \ll 0.001$; mean $\Delta s \approx +0.021$.
    \item \textbf{Effect Size}: Cohen's $d \approx 1.0$ relative to the paired variance --- statistically consistent, but \emph{small in absolute magnitude}.
    \item \textbf{Interpretation}: The subgraph selected by geDIG ($\approx 30$ nodes) points toward the answer direction more than random selection, by a small but consistent margin.
    \item \textbf{Implication}: This correlational result is \emph{consistent with} --- but does not validate --- the Thermodynamic Inference reading (minimizing free energy corresponds to reasoning). Causal intervention experiments (manipulating KG quality and observing answer quality) are future work.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Measured result (lite suite)}: all variants (base / EPC-only / 0-hop-only / IG-emphasis) produce \emph{identical} headline metrics (PER 0.421, Acc 0.374, FMR 0.626; see the table above). At this scale and setting, the ablation \textbf{does not separate} the contributions of the individual terms.
    \item \textbf{Maze ablation} ($15{\times}15$, full-scale rerun) similarly showed no significant differences in success rate or steps between $\Delta$EPC-only / $\Delta$IG-only / full geDIG.
    \item \textbf{Possible causes}: the synthetic data may be too easy, and the acceptance decision may be threshold-dominated so that gauge-term differences do not surface.
\end{itemize}

\textbf{Conclusion}: The necessity of each term ($\Delta\mathrm{EPC}, \Delta H, \Delta\mathrm{SP}$) and of the two-stage gate is a \emph{design hypothesis} that the current experiments do not yet isolate. Constructing settings hard enough to separate per-component contributions is explicit future work.
...
\begin{enumerate}
    ...
    \item \textbf{PSZ as a yardstick}: In the RAG lite suite, geDIG reduces the PSZ shortfall ($s_{\mathrm{PSZ}}$: $1.93\to1.22$) over non-accepting baselines, but remains \emph{outside} PSZ --- acceptance 0.374 (target $\ge$0.95) and FMR 0.626 (target $\le$0.02). Closing this gap, especially contamination control, is the main open problem.
...
\textit{Note: these statistics come from an earlier prototype run whose configuration differs from the main result table (mean steps 92.8 in Table~\ref{tab:maze_main_result}); treat them as indicative.}
\begin{table}[H]
    \centering
    \caption{15×15 Maze Detailed Stats ($N=100$; earlier prototype run, indicative)}
...
An earlier prototype produced a per-query-type breakdown (acceptance 96--98\%, FMR 1.2--2.8\%), but its setup and scoring differ substantially from the lite suite reported in this paper (acceptance 0.374, FMR 0.626) and the two are not comparable. We therefore omit that table from this revision; a per-type breakdown on the lite suite will be reported after re-measurement.
% (Former tab:query_type removed in the v6 revision due to inconsistency with the measured headline results; see git history.)
...
geDIG moves the operating point in the PSZ direction (shortfall $s_{\mathrm{PSZ}}$: $1.93\to1.22$) at the cost of latency ($160\mathrm{ms} \to 240\mathrm{ms}$), but remains outside the PSZ band (FMR $\approx$ 0.63 vs.\ target $\le$ 0.02).
...
\begin{figure}[H]
  ...
  \caption{Operating Curves (Acceptance vs FMR). PSZ band is Acc$\ge$0.95, FMR$\le$0.02. geDIG moves toward the band but does not reach it (FMR $\approx$ 0.63).}
