% geDIG One-Gauge 改善版 v6.1 (2026-06)
% v6 からの変更: 主張と実測の整合性改訂（FMR 実測の明示、旧プロトタイプ数値の参考値化、
%   重複ブロックの削除、計画値と実測値の分離、LLM 不使用の明記）。実験データの追加はなし。
% v6 (2025-12): 実験規模の拡充と詳細化、関連研究の充実を図った版
...
\date{\today\; v6.1 改訂版草稿（v6 の主張と実測の整合性を改訂）}
...
\begin{abstract}
動的に成長する知識グラフにおいて、Retrieval-Augmented Generation（RAG）は「何を取得するか（What）」の最適化には長ける一方で、「いつ新しい知識を受け入れるか（When）」を決める規範を欠いている。本稿ではこの課題に対し、構造コスト（正規化編集経路コスト）と情報利得（Shannonエントロピー差および経路短縮）を単一ゲージ $\F$ に統合し、探索と統合の意思決定を一貫して制御する枠組み \textbf{geDIG} を提案する。geDIG は、0-hop の曖昧検知（Attention Gate; AG）と multi-hop の確証（Decision Gate; DG）からなる二段ゲートにより、更新の受容／保留／棄却と探索の深掘りをイベント駆動で制御する。さらに、本設計は Free Energy Principle（FEP）および Minimum Description Length（MDL）に対する\textit{操作的対応}として解釈可能である。

迷路PoCと軽量RAGパイプライン（合成500クエリ）で評価した結果、迷路では候補エッジの約98--99\%を剪定しつつ Greedy DFS に近い成功率を維持し、RAGでは受容を行わないベースラインに対して回答品質（PER 0.17$\to$0.42）を改善しつつ、受容判定がイベント駆動で機能することを確認した。一方、誤統合の選別精度は未達であり（実測 FMR$\approx$0.63）、知識汚染の抑制は今後の課題として明示する。運用評価として、受容率・False Merge Rate（FMR）・レイテンシからなる目標帯域 \textbf{PSZ（Perfect Scaling Zone）} を定義し、Operating Curve によりトレードオフを可視化する。実装と再現手順を公開する。

\par\medskip
\noindent\textbf{Abstract (English).}\;
Dynamic knowledge graphs lack a normative criterion for \emph{when} to accept new information, even as RAG systems excel at optimizing \emph{what} to retrieve. We propose \textbf{geDIG}, a unified control framework that uses a single gauge $\F$ combining normalized structural cost (edit-path cost) and information gain (Shannon entropy difference and path shortening), and a two-stage gating mechanism---AG at 0-hop and DG via multi-hop---to control exploration and integration decisions. We evaluate geDIG on a maze proof-of-concept and a lightweight RAG pipeline (500 synthetic queries): the gauge prunes about 98--99\% of candidate edges in the maze while keeping success rates close to a greedy-DFS baseline, and enables selective, event-driven acceptance in RAG (PER $0.17\to0.42$ over non-accepting baselines). The measured False Merge Rate (FMR $\approx 0.63$), however, remains far above the PSZ target, so contamination control is stated explicitly as future work. We release a reproducible implementation and scripts.
\end{abstract}
...
\noindent\textbf{符号規約}: $\Delta H_{\mathrm{norm}}=(H_{\mathrm{after}}-H_{\mathrm{before}})/\log K$（秩序化＝低下で\emph{負}）、$\;\Delta\mathrm{SP}_{\mathrm{rel}}=(L_{\mathrm{before}}-L_{\mathrm{after}})/\max\{L_{\mathrm{before}},\varepsilon\}$（短縮で\emph{正}）。$\F$は\textbf{小さいほど良い}（構造コストが低く、$\Delta\mathrm{IG}_{\mathrm{norm}}$ が大きいほど小）。\;\textit{注}: 秩序化の利得は $-\Delta H_{\mathrm{norm}}>0$ として読める。
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{MazeとRAGによる原理検証}: 部分観測迷路における PoC 実験と、合成500クエリの軽量 RAG 実験の双方で、単一ゲージと二段ゲートが実時間で安定動作することを示した。迷路では候補エッジの約98--99\%の剪定、RAGでは受容なしベースライン比の PER 改善（0.17$\to$0.42）を確認した。ただし偽受理の抑制（実測 FMR$\approx$0.63）と、アブレーションによる構成要素の寄与分離は\emph{現時点で未達}であり、今後の課題として明示する。
...
\paragraph{PSZ/SLO の定義と集計} 本稿の SLO として \textbf{PSZ（Perfect Scaling Zone）} を用いる。直感的には、「\emph{受容率は十分高く（Acc$\ge95$\%）／誤った統合はほとんど起こらず（FMR$\le2$\%）／動的制御による追加遅延も許容範囲内（P50$\le$200ms）}」といった運用上の目標帯を意味する。\\
...
ここで geDIG（ジーディグ）は \textit{graph edit Distance and Information Gain} の略であり、既存グラフに対する\,\textbf{編集経路コストの変化}と\,\textbf{情報利得}を一つのゲージ $\F$ に束ね、静的RAGでは\emph{再ランキングスコア}として、動的RAGでは\emph{更新ゲート}として「いつ受容するか（When）」を判定する枠組みである。
ここで「情報利得」は、\emph{確率的}情報利得（Shannonエントロピー差 $\Delta H$）と\emph{位相的}情報利得（ベッチ数変化 $\Delta\beta_1$；知識グラフにおける独立サイクルの生成・消滅を捉える）の双方を広義に包含する。
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{$\Delta\mathrm{EPC}/\Delta H/\Delta\mathrm{SP}$}: $\Delta\mathrm{EPC}$ は編集経路コストの正規化差分（構造コスト）、$\Delta H$ はシャノンエントロピーの正規化差分（after−before; 秩序化は負）、$\Delta\mathrm{SP}$ は平均最短路長の変化（経路短縮）を表す（簡約形は \S\ref{sec:background_overview}、完全な定義は \S\ref{sec:gauge_theory}）。
\end{itemize}

\paragraph{研究の立場と投稿方針}
本研究は、\textbf{在野の独立研究者}が\,\emph{AI（大規模言語モデル）との対話に触発}されて着手した試みである。理論の\,\emph{完全性}よりも、\textbf{最小の運用仮説を再現・反証可能な形で提示すること}を目的とし、AI の補助（情報整理・言語化）を受けつつ\,\textbf{日本語で理論と実装を構造化}した。\,\emph{PSZ 未達}や計算資源の制約を踏まえ、\,\textbf{本稿の投稿は完成の宣言ではなくコミュニティへの問いかけ}であり、理論および実験設計に対する\,\textbf{レビュー・再現・批判的検証}を広く求める。さらに、\textbf{共同研究や共著としての展開も歓迎する}。\,\textbf{論文内容と解釈の最終責任は著者にある}。
...
前節で述べた「When」の課題に対し、geDIG は単一ゲージ $\F$（定義は §\ref{sec:gauge_theory}）と二段ゲート（AG/DG）により、”曖昧なら探索を深化（AG）／短絡が確認された時だけ統合（DG）” をイベント駆動で行う。
RAG へ写すと、\textbf{FMR$\downarrow$（偽受理率の低減）}・\textbf{PSZ 近傍への接近}・\textbf{多ホップ選別}を同一ロジックで駆動することを\emph{設計目標とする}（現時点の実測では FMR は未達である；\S\ref{sec:psz_metrics}）。

\section{背景と概要}\label{sec:background_overview}

\subsection{動的RAGの文脈}
静的な Retrieval\,\mbox{-}Augmented Generation (RAG) は\,\textbf{何を取るか（What）}の最適化に長ける一方、\,\textbf{いつ受け入れるか（When）}の規範を欠くため、知識汚染や冗長化、応答遅延といったトレードオフが場当たり的になりやすい。
...
その判断には、構造編集によるコストと情報利得（エントロピー差＋経路効率）の天秤が必要である。
\\
...
\end{equation}
ここで $\gednorm$ は正規化編集経路コスト、$\Delta H_{\rm norm}$ はエントロピーの正規化差分（符号付き）、$\Delta\mathrm{SP}_{\rm rel}$ は平均最短路長の相対短縮であり、$\F$ は小さいほど「構造コストが低く、情報利得（$\Delta\mathrm{IG}_{\rm norm}$）が大きい統合」であると解釈する（完全な定義は \S\ref{sec:gauge_theory}）。
\\
...
\subsection{なぜ構造と確率の同時評価が必要か}\label{sec:why_both}
従来手法は、類似度（確率的指標）またはグラフ構造のいずれか一方のみを評価する（表\ref{tab:structure_probability_comparison}）。
BM25 や Contriever は文書間の類似度スコアのみで検索を打ち切るため、「高スコアだが冗長な情報」を排除できない。
一方、GraphRAG はグラフ構造を活用するが、「構造は整っているが不確実性が残る」状態を検知できない。
...
geDIG の核心は、\textbf{構造変化のコスト}（$\gednorm$）と\textbf{確率的な情報利得}（$\ignorm$）の\textbf{収支}を単一スカラーで評価する点にある。
直感的には、「新しいエッジを張る構造的コストに見合うだけの不確実性削減が得られるか？」という問いに答える。
この収支計算により、以下の判断が可能になる：
\begin{itemize}[leftmargin=1.6em]
    \item 構造コスト大・情報利得小 $\to$ 検索不要（冗長）
    \item 構造コスト小・情報利得大 $\to$ 検索有効（統合）
    \item 構造コスト大・情報利得大 $\to$ 探索継続（AG発火）
\end{itemize}
この「いつ」の判断は、構造と確率を\textbf{同時に}見なければ下せない。
これが geDIG の本質的な新規性である。

\subsection{指標の直観的な読み方}
\noindent\textit{三つの観点}\; geDIG が扱う主な量は、(1) グラフ編集の「つらさ」を測る $\gednorm$、(2) 分布変化（秩序化/均質化の方向）を測る $\Delta H_{\rm norm}$、(3) 経路の「回り道の減り具合」を測る $\Delta\mathrm{SP}_{\rm rel}$ である。
これらを一つにまとめたものが $\F=\gednorm-\lambda(\Delta H_{\rm norm}+\gamma\,\Delta\mathrm{SP}_{\rm rel})$ であり、\textbf{$\F$ が小さいほど「少ない編集で、情報利得（$\Delta\mathrm{IG}$）が大きい統合」}と読む。
直観的には、$\gednorm$ は「グラフの健全性コスト」、$\Delta H_{\rm norm}$ は「エントロピーの符号付き変化（秩序化は負、均質化/多様化は正）」、$\Delta\mathrm{SP}_{\rm rel}$ は「効率性（平均経路長の短縮）」に対応する（秩序化の利得は $-\Delta H_{\rm norm}>0$ として読める）。
\\
\noindent\textit{簡単な数値例（玩具）}\; 例えば、ある更新候補について $\gednorm=0.2$（ノード/エッジを少数だけ追加）、$\Delta H_{\rm norm}=-0.1$（分布がシャープになるため負）、$\Delta\mathrm{SP}_{\rm rel}=0.7$（平均最短路が70\%短縮）、$\lambda{=}1,\\gamma{=}0.5$ とすると、
\[
  \F \approx 0.2 - 1\times(-0.1+0.5\times0.7)=0.2-0.25=-0.05
\]
となり、秩序化（$\Delta H_{\rm norm}<0$）が負として入っても経路短縮が上回り $\Delta\mathrm{IG}_{\rm norm}>0$ となるため「良い統合」と判定される。
...
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{One‑Gauge 統合制御}: \;構造コスト（$\Delta$EPC; 編集経路コストの正規化差分）と情報利得（$\Delta H{+}\gamma\,\Delta\mathrm{SP}$; エントロピー差と平均最短路長短縮）を\,\textbf{単一スカラー $\F$} に統合する。さらに、\,\textbf{AG/DG の二段ゲート}で探索・配線・バックトラック・エビクションを\,\emph{イベント駆動}で制御する（§\ref{sec:gauge_theory}, §\ref{sec:ag_dg_gate}）。
  ...
  \item \textbf{Phase 1 横断実証（迷路PoC）}: \;部分観測迷路で、単一ゲージと AG/DG が実時間で連携し、候補エッジの約98--99\%を剪定しつつ Greedy DFS に近い成功率を維持することを示す（§実験I; 図表は該当節を参照）。

  \item \textbf{Phase 1 横断実証（RAG; 合成500クエリの lite スイート）}: \;equal‑resources 下で、\,\textbf{静的}は性能を維持。\,\textbf{動的}は PSZ 指標で \textbf{s\textsubscript{PSZ}} 短欠が縮小（1.93$\to$1.22）するが、PSZ 三軸（特に FMR$\le$2\%）には未達である（図~\ref{fig:psz_operating_curves_lite_ja} など）。
...
\paragraph{章の概要}\; \textbf{動的知識グラフへのエピソード挿入シナリオと課題設計} —— 動的に変化する知識グラフに新たなエピソードが注入される場面を考える。このときシステムは、グラフ構造を編集（ノード・エッジの追加）してエピソードを統合すべきか、あるいは統合せず現行知識で対処すべきかを即座に判断しなければならない。その判断には、構造編集によるコストと情報利得（エントロピー差＋経路効率）の天秤が必要である。すなわち、新規ノード接続に伴う\,\emph{編集経路コスト（EPC）の増大というコスト}と、エピソードによってもたらされる\,\emph{エントロピー差（$\Delta H$；符号付き）と経路効率の向上（= 平均最短路長の減少）}とのトレードオフである（定義は \S\ref{sec:gauge_theory}）。
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    $\ignorm^{+}$ & 拡張IG差分（$\ignorm{+}\beta\,\Delta\mathrm{SS}$；\S\ref{sec:structural_similarity_extension}） \\
    ...
    $\beta$ & 構造類似度項の重み係数（$\ignorm^{+}$内；\S\ref{sec:structural_similarity_extension}） \\
    $\Delta\mathrm{SS}$ & 構造類似度変化（局所パターン間のGED類似度） \\
...
\\
\noindent\textit{解釈}: $\Delta H_{\mathrm{norm}}$ は \emph{符号付きのエントロピー変化}であり、秩序化（$H$低下）は $\Delta H_{\mathrm{norm}}<0$ として現れる。したがって $\ignorm$ は、（i）分布の均質化/多様化（$\Delta H_{\mathrm{norm}}>0$）と（ii）経路短縮（$\Delta\mathrm{SP}_{\mathrm{rel}}>0$）がどれだけ構造コストを相殺し得るかを、単一尺度で測る情報項として扱う。
\\
...
\]
$\F$ は\textbf{小さいほど良い}（構造コストが低く、$\Delta\mathrm{IG}_{\mathrm{norm}}$ が大きいほど小）。以後、断りのない限り\,\textbf{正規化形（$\gednorm,\,\Delta H_{\rm norm},\,\Delta \mathrm{SP}_{\rm rel}$）}を用いる。
...
\paragraph{発展: 構造類似度 $\Delta\mathrm{SS}$ の導入}\label{sec:structural_similarity_extension}
情報利得 $\ignorm$ の構成要素として、エントロピー変化（$\Delta H$）と経路短縮（$\Delta\mathrm{SP}$）に加え、\textbf{構造類似度}（Structural Similarity; SS）を導入する拡張が考えられる：
\begin{equation}
\ignorm^{+}\;=\;\Delta H_{\mathrm{norm}}\; +\; \gamma\,\Delta \mathrm{SP}_{\mathrm{rel}}\; +\; \beta\,\Delta \mathrm{SS},\qquad \beta\ge 0,\label{eq:ignorm_extended}
\end{equation}
ここで $\Delta\mathrm{SS}$ は、グラフ構造間の類似度変化を測る項である。具体的には、グラフ編集距離に基づく正規化類似度として：
\begin{equation}
\mathrm{SS}(G_1, G_2)\;=\;1 - \frac{\mathrm{GED}(G_1, G_2)}{\max(|G_1|, |G_2|)}
\end{equation}
を用い、$\Delta\mathrm{SS}$ は統合前後での「目標構造」への類似度変化を表す。

\noindent\textit{適用条件}: 構造類似度が有効に機能するのは、\textbf{局所構造が大域的な意味と相関する}場合に限られる。例えば：
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{知識グラフ}: 「革命」と「感情」が同じ5段階構造を持つ場合、両者は意味的にも関連している可能性がある（仮説；定量検証は今後の課題）。
  \item \textbf{科学史}: 太陽系と原子が同じ「中心-周回」構造を持つことが、ボーアの原子モデル発見につながった。
\end{itemize}
逆に、迷路のように局所パターン（T字路など）と大域的な正解（ゴールへの経路）が相関しない場合、$\Delta\mathrm{SS}$ は有効に機能しない。この\textbf{適用条件の識別}自体が、geDIG を新しいドメインへ適用する際の重要な設計判断となる。

\paragraph{メタ階層性への展望}\label{sec:meta_hierarchy}
構造類似度は単なる評価指標にとどまらない。発見された\textbf{構造パターン自体を特徴量として抽出}することで、より高次の抽象化が可能となる：
\begin{align*}
\text{Level 1:} \quad & \text{要素の類似性} \quad \mathrm{sim}(a, b) = \cos(\phi(a), \phi(b)) \\
\text{Level 2:} \quad & \text{構造の類似性} \quad \mathrm{SS}(G_1, G_2) \\
\text{Level 3:} \quad & \text{構造パターンの類似性} \quad \mathrm{SS}(\mathrm{Pattern}_1, \mathrm{Pattern}_2) \\
& \vdots
\end{align*}
例えば、geDIG が「革命 $\approx$ 感情」と「学習 $\approx$ 遺伝子発現」という2つのアナロジーを発見した場合、両者に共通する「5段階プロセス」という\textbf{メタパターン}を抽出できる。このメタパターン間の構造類似度を評価することで、さらに高次の抽象化（例：「段階的変化」という普遍パターン）へと自動的に到達する。

この階層的抽象化は、\S\ref{sec:rag_limited_reasoning} で述べた\textbf{再帰的洞察ベクトル（RIA）}と接続する。RIA は洞察ベクトル $\mathbf{h}^{(t)}$ を再帰的に利用してプロンプト/検索を強化するが、構造類似度のメタ階層化は、この再帰プロセスに\textbf{抽象度の自動的上昇}という方向性を与える。すなわち、geDIG は単に知識を蓄積するだけでなく、\textbf{知識の構造を抽象化し、その抽象構造をさらに構造化する}という自己組織化的な発展が可能となる。
...
% （0-hop／multi-hop の運用定式・最小例・状態識別表は \S\ref{sec:dual_fep_mdl} に集約。
%   旧版でここに重複していたブロックは v6 改訂で削除した。）
...
\begin{equation}
  \boxed{\;\Delta H^{(h)}_{\mathrm{norm}}\;=\;\frac{H\big(Q\mid S_h(Q)\big)\; -\; H\big(Q\mid S_{h-1}(Q)\big)}{\log |S_h(Q)|}\;}\label{eq:cond_dh}
\end{equation}
条件集合が増えるほど $H(Q\mid S_h)$ は単調に減少するため、$\Delta H^{(h)}_{\mathrm{norm}}\le 0$ が保証される（秩序化=曖昧さの減少で負）。したがって、秩序化の\textbf{利得}は $-\Delta H^{(h)}_{\mathrm{norm}}\ge 0$ として読める。以後の運用では、multi\,\mbox{-}hop の経路短縮（$\Delta\mathrm{SP}_{\rm rel}>0$）がこの負の項を上回るときに $\F$ が低下し、DG が発火する。
\\[0.2em]
...
% 単位と換算（U1–U3）の詳細な定式化は、後半の FEP--MDL 節に集約した。

% ここでの詳細な補題・命題・証明スケッチ（構造符号長/データ符号長と $\gednorm,\ignorm$ の対応、および運用MDL命題）は、
% 後半の FEP--MDL 節（\S\ref{sec:fep_mdl_bridge}）にまとめ、本文では概要のみ扱う。

\paragraph{適用範囲と限界（概要）}\label{sec:fep_mdl_bridge_applicability}
本ゲージは、有限ホライズン・有界な編集コスト・安定した正規化を前提とした設定に対して設計されている。熱力学的な読み替えや「工程分離としてのFEP運用」の詳細な議論（Helmholtz 自由エネルギーとの対応や anytime 性の整理など）は、後半の FEP--MDL 節（\S\ref{sec:fep_mdl_bridge}）および付録に移し、本節では概要のみを示す。
...
本節の主目的は、\textbf{未知迷路における探索効率の改善}を定量化することである。代表的な設定では、geDIG は 15\,\mbox{$\times$}\,15 で成功率 98.0\%（$N{=}100$）、25\,\mbox{$\times$}\,25 で 56--75\%（設定依存）を示した。成功率自体は単純な Greedy DFS（15\,\mbox{$\times$}\,15: 99\%, 25\,\mbox{$\times$}\,25: 95\%）と同等以下であり、geDIG の特徴は\emph{生の成功率}の優位ではなく、候補エッジの約 98\% を剪定できる点と、ドメイン固有ルールに依らない創発的な AG/DG 制御にある（詳細は表~\ref{tab:maze_v4_main}）。参照（Dijkstra／A*）は\,\emph{理想上限}として整合度（Regret／SPL や順位相関）で扱う\footnote{Regret はオラクル（最短路）との差の累積。SPL（Success weighted by Path Length）は成功時の効率（実行経路長と最短経路長の比）を表す。}。代表結果（25\,\mbox{$\times$}\,25, \,$N{=}32$；L3\,only, max\,steps=250）の要点を先にまとめる：
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{成功率}: 56.3\%（25\,\mbox{$\times$}\,25, \,$N{=}32$；max\,steps=250）／ 98.0\%（15\,\mbox{$\times$}\,15, \,$N{=}100$）。\textit{注}: 25\,\mbox{$\times$}\,25 の成功率は実行条件で変動する（max\,steps=250 で 56--75\%、500 で $\approx$1.0；\S\ref{sec:maze_25x25_500}）。本稿の\emph{代表値}は表~\ref{tab:maze_results}（$N{=}60$, 75\%）とする
...
\paragraph{設計要素と結果の対応}\; (1) \textbf{統一指標制御}は実時間で安定動作することを確認した。ただし対照（$\Delta$EPC\,only／$\Delta$IG\,only）とのアブレーションでは成功率・平均ステップに有意差が出ておらず、両項の寄与分離は本スケールでは未達である（表~\ref{tab:maze_results} 注記参照）。(2) \textbf{二段ゲート}の適切性は、0\,\mbox{-}hop と 1\,\mbox{-}hop の役割分担（図~\ref{fig:minimal_example}）と発火統計（DG/AG 比率）の安定から確認できる。 (3) \textbf{クエリ中心 multi\,\mbox{-}hop} の効率は、$H\in\{1,2,3\}$ の軽量設定で短絡検出と計算量のバランスが取れる点（§\ref{sec:graph_model_premise} のコスト見積もり）に表れる。
...
\begin{figure}[H]
  ...
  \caption{迷路スケーリング（L3; max\,steps=250/500/1000）: 成功率（左軸, 青）と平均ステップ数（右軸, 赤）のサイズ依存。15\,\mbox{$\times$}\,15（max\,steps=250）と25\,\mbox{$\times$}\,25（max\,steps=500）では高成功率を保ちつつステップ数を抑え、51\,\mbox{$\times$}\,51 でもより大きな予算（max\,steps=1000）を与えることで成功率を回復できることを示す。}
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=9mm and 11mm,>=Stealth,rounded corners]
    \node[draw,fill=gray!10,minimum width=26mm] (ep) {候補エピソード};
    \node[draw,fill=gray!10,minimum width=27mm,right=of ep] (gzero) {$g_0$（0-hop 評価）};
    \node[draw,fill=gray!10,minimum width=24mm,right=of gzero] (ag) {AG ゲート};
    \node[draw,fill=gray!10,minimum width=27mm,below=of ag] (mh) {multi-hop 評価 $g_h$};
    \node[draw,fill=gray!10,minimum width=24mm,right=of mh] (dg) {DG ゲート};
    \node[draw,fill=green!10,minimum width=20mm,right=of dg] (accept) {受容（confirmed）};
    \node[draw,fill=yellow!10,minimum width=20mm,above=of dg] (pending) {保留（pending）};
    \node[draw,fill=red!10,minimum width=22mm,below=of dg] (bt) {BT／剪定（rollback）};

    \draw[->] (ep) -- node[above,font=\scriptsize] {$\F$ 計算} (gzero);
    \draw[->] (gzero) -- node[above,font=\scriptsize] {$g_0 \le \theta_{\mathrm{AG}}$} (ag);
    \draw[->] (gzero) |- node[left,font=\scriptsize] {$g_0 > \theta_{\mathrm{AG}}$} (mh);
    \draw[->] (mh) -- node[above,font=\scriptsize] {$g_{\min}$ 更新} (dg);

    \draw[->] (ag) -- node[above,font=\scriptsize] {0-hop 即時受容} (accept);
    \draw[->] (dg) -- node[above,font=\scriptsize] {$b(t)\le\theta_{\mathrm{DG}}$} (accept);
    \draw[->] (dg) -- node[right,font=\scriptsize] {境界事例} (pending);
    \draw[->] (dg) -- node[below,font=\scriptsize] {$b(t)>\theta_{\mathrm{DG}}$} (bt);
  \end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.68\linewidth]{figures/fig_gate_rates}
  \caption{AG/DG 発火率分布（代表例；25クエリ／500クエリ）。分位制御（$\alpha,\beta$）で設計したターゲット頻度帯に、実測発火率が収まっていることを確認する。}
...
\centering
\small
\caption{探索効率（25×25迷路、$N{=}60$、実測ログ \texttt{maze\_25x25\_*\_s500.json}）。成功率は同一条件・部分観測での実測値。探索率/訪問重複/BT長/検出遅延は geDIG の実測（baseline 側は同指標を未測定のため N/A）。}
\label{tab:exploration_efficiency}
\begin{tabular}{@{}l p{1.6cm} p{1.6cm} p{1.6cm} p{1.2cm} p{1.8cm}@{}}
\toprule
手法 & 探索率 & 訪問重複 & BT長 & 検出遅延 & 成功率 \\
\midrule
Random Walk & N/A & N/A & N/A & N/A & 0.40 \\
Greedy DFS & N/A & N/A & N/A & N/A & 0.95 \\
\textbf{geDIG} & 0.256 & 1.33 & $\approx$1.0 & $\approx$0 & 0.75 \\
\bottomrule
\end{tabular}
\end{table}
...
部分観測迷路という最小構成でも、geDIG の統一指標と二段ゲートが想定どおり連携することを確認できた。表~\ref{tab:maze_results} に示すように、geDIG は 15×15 で成功率 98\%（単純な Greedy DFS の 99\% と同等）、25×25 で 75\% を達成し、候補エッジの 98\% 以上を削減した。\emph{生の成功率では単純な Greedy DFS に及ばない}ものの、本 PoC の主眼は探索ソルバとしての優劣ではなく、ドメイン固有ルールに依らない\emph{創発的な制御}と高い圧縮率の実証にある。AG 発火率・DG 発火率が低位（数\%）に収まることは、曖昧さ検出と洞察統合がイベント駆動で機能し、無駄な探索を抑制することと整合する（発火頻度は表~\ref{tab:maze_gate_stats}）。一方で、15×15 のアブレーション（$\Delta$EPC-only／$\Delta$IG-only）では成功率・平均ステップに有意差が出ておらず、本スケールでは各項の寄与を分離できていない（構造・情報の同時評価の必要性は、より難度の高い環境での今後の検証課題として残る）。
...
\begin{table}[H]
\centering
\caption{迷路ナビゲーション\textbf{実測}結果。値は実測ログ（\texttt{docs/paper/data/maze\_*\_s*.json}）に基づく。成功率は同一条件・部分観測での比較。Oracle(BFS) は既知地図を前提とする\emph{上限参照}。エッジ削減(\%)は候補辺集合 $E_{\mathrm{link}}$ に対する最終採用辺 $E_{\mathrm{final}}$ の相対削減率 $1-\lvert E_{\mathrm{final}}\rvert/\lvert E_{\mathrm{link}}\rvert$。}
\label{tab:maze_results}
\begin{tabular}{lcccc}
\toprule
手法 & 成功率(\%) & 平均ステップ & エッジ削減(\%) & $N$ \\
\midrule
\multicolumn{5}{l}{\textit{15×15迷路}} \\
Random Walk & 75.0 & 141.9 & - & 100 \\
Greedy DFS & 99.0 & 83.7 & - & 100 \\
Oracle (BFS, 上限参照) & 99.0 & 48.0 & - & 100 \\
\textbf{geDIG (提案)} & \textbf{98.0} & 92.8 & \textbf{98.3} & 100 \\
\midrule
\multicolumn{5}{l}{\textit{25×25迷路}} \\
Random Walk & 40.0 & 396.9 & - & 60 \\
Greedy DFS & 95.0 & 255.1 & - & 60 \\
Oracle (BFS, 上限参照) & 95.0 & 118.4 & - & 60 \\
\textbf{geDIG (提案)} & 75.0 & 315.6 & \textbf{99.3} & 60 \\
\bottomrule
\end{tabular}
\\[0.4em]
\footnotesize\textit{注}: geDIG の\emph{生の成功率}は単純な Greedy DFS と同等以下（15×15: 98 vs 99\%, 25×25: 75 vs 95\%）であり、本 PoC の主眼は探索ソルバとしての優劣ではなく、統一ゲージ $\F$ と AG/DG による\emph{制御原理}の実証と高いマップ圧縮（約98--99\%）にある。15×15 のアブレーション（$\Delta$EPC-only / $\Delta$IG-only / AG・DG無効）は成功率・平均ステップに差が出ず（\texttt{maze\_15x15\_ablation\_fullscale.json}）、本スケールではコンポーネント寄与を分離できていない（より大規模・高難度環境での検証は今後の課題）。架空の中間ベースライン（Curiosity / Q-learning 等）および 50×50 の数値は実測データが揃っていないため本表から除外した（スケーリングの定性傾向は図~\ref{fig:maze_scaling} を参照）。
\end{table}
...
\paragraph{探索効率}
実測（表~\ref{tab:maze_results}）では、geDIG の平均ステップは 15×15 で 92.8（Greedy DFS 83.7、Oracle 48.0）、25×25 で 315.6（同 255.1、118.4）であり、\emph{ステップ効率では単純ベースラインを上回らない}。一方、エッジ削減率は 98--99\% に達し、グラフメモリの圧縮という観点で明確な差を示す。

\paragraph{統合制御の寄与}
$\Delta$EPC only／$\Delta$IG only とのアブレーション（15×15）では成功率・平均ステップに有意差が出ておらず、構造・情報の同時評価の必要性は本スケールの迷路では実証できていない（表~\ref{tab:maze_results} 注記）。寄与分離はより大規模・高難度な環境での今後の課題である。

\paragraph{スケール依存性}
Query-Hub v4 の実測（表~\ref{tab:maze_v4_main}）では、成功率は 15×15: 98\%、25×25: 56\%（max~steps=250）、51×51: 55\%（max~steps=1500; seeds=11; 暫定）と規模拡大に伴い低下する。一方、圧縮率は 0.97--0.99 を維持しており、剪定能力は規模に対して頑健である。
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figures/fig_maze25_seed5_spike.png}
    \caption*{(a)\;25$\times$25 迷路（seed=5, 500 step）と対応グラフ。丸印は geDIG による折りたたみが生じた代表地点を示す。}
  \end{minipage}\hfill
  \begin{minipage}[t]{.49\linewidth}
    \centering
    \includegraphics[width=.32\linewidth]{figures/fig_maze25_seed5_abstract1.png}\hfill
    \includegraphics[width=.32\linewidth]{figures/fig_maze25_seed5_abstract2.png}\hfill
    \includegraphics[width=.32\linewidth]{figures/fig_maze25_seed5_abstract3.png}
    \caption*{(b)\;抽象グラフ（Show all query nodes）。左から順に、折りたたみ前（corridor）、分岐周辺（T字路）、折りたたみ後（dead end 周辺）のスナップショットを示す。}
  \end{minipage}
  \caption{25$\times$25 迷路における geDIG の折りたたみ挙動の例（seed=5, 500 step）。(a) 具体グラフ上の地点と経路、(b) 対応する抽象グラフ（query-hub DS）を対応付けて示す。}
  \label{fig:maze25_seed5_abstract_triptych}
\end{figure}
...
\noindent\textbf{代表挙動（25×25; 120steps）}: $\theta_{\rm AG}{=}{-}0.15$ で 0\,\mbox{-}hop の改善が大きい場面は AG により multi\,\mbox{-}hop を節約（$g_0$ が負域に集中）。\,一方、分岐が濃い区画では forced 候補（赤破線）が保持され、DG 閾値を下げると\,\textit{想起\,\mbox{-}\,確証\,\mbox{-}\,コミット}の連鎖が確認できる（図\,\ref{fig:maze25_seed5_abstract_triptych} のT字路に対応）。
...
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{正答率（PER）}: 実測のある比較は lite スイート（500クエリ; \texttt{experiments/exp2to4\_lite} の \texttt{exp23\_paper} 設定）に基づく。Static／Frequency／Cosine の PER$\approx$0.17 に対し、geDIG は PER$\approx$0.42 と、およそ $+0.25$ の絶対改善を示した（表~\ref{tab:rag_summary}）。GNN／Graph\,Transformer リトリーバ（B1/B2）との比較は設計のみで、実測は今後の課題である。
  \item \textbf{レイテンシ}（P50）: lite 実測では静的 160ms に対し geDIG 240ms と、multi\,\mbox{-}hop 評価分の追加遅延（+80ms）に収まる（表~\ref{tab:rag_summary}）。
  \item \textbf{根拠整合・Subgraph品質}（引用一致／Path\,Faithfulness／Recall@k 等）: 本稿の lite スイートは LLM 生成を伴わないため未測定であり、これらの指標による検証は今後の課題である。
\end{itemize}
...
\centering
\small
\caption{静的RAGにおけるプロンプト強化と関連指標（旧プロトタイプ実装による予備測定; KB 168項目・クエリ数は少数; \emph{参考値}）。}
...
\end{tabular}
\end{table}
\noindent 表\ref{tab:rag_static_enrichment}は\emph{旧プロトタイプ実装による予備測定}（KB 168項目・少数クエリ；再現スクリプト未整備のため参考値。v6 以前の「$n{=}168$ クエリ」表記は KB アイテム数の取り違え）であり、本稿の主結果は lite スイートの表~\ref{tab:rag_summary} とする。予備測定の傾向としては、geDIG によるプロンプト強化率・関連度・多様性・クロスドメイン橋渡しの改善が観察された。

\subsection{比較対象（固定；静的）}
\label{sec:rag_baselines}
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{実測済（lite スイート）}: Static（常時 Top-$k$）／Frequency（頻度ベース）／Cosine（類似度しきい値）／geDIG（AG/DG）
  \item \textbf{設計のみ（実測は今後の課題）}: B0: 平面RAG（ベクトル検索→LLM）、B1: 静的GraphRAG（GNNリトリーバ）、B2: 静的GraphRAG（Graph Transformerリトリーバ）、G1: B2＋geDIG\,F（ソフト刈り込み）
  % 動的（B3/G2）は次章で扱う（同じく設計のみ）
\end{itemize}

\subsection{比較手法の詳細（静的；設計仕様）}\label{sec:rag_methods_detail}
\noindent\textit{注}: 本節の B0--B2／G1 は比較実験の\emph{設計仕様}であり、本稿で実測値を報告するのは lite スイートの Static／Frequency／Cosine／geDIG のみである（表~\ref{tab:rag_summary}）。\\
...
\noindent\textit{注}: 本節のデータセット構成（50ドメイン・公開データ併用）は\emph{設計仕様}であり、本稿の実測（lite スイート）は合成コーパス（支援/撹乱文書＋テンプレート回答; \texttt{generate\_dataset.py}）で行った。公開データでの評価は今後の課題である。\\
...
回答合成 & 検索文書からのテンプレート合成（\textbf{LLM 生成は不使用}；下記注参照） \\
ハードウェア & 単一マシン・CPU 実行（全手法共通） \\
並列度 & シングルスレッド（公平性） \\
測定 & ウォームアップ5回 + 本測定3回 \\
\bottomrule
\end{tabular}
...
\noindent\textit{注（回答生成について）}\; 本稿の lite スイートは LLM による自由生成を行わず、検索文書からの\emph{テンプレート合成}で回答を構成する。したがって PER（トークン重なりに基づく F1 代理指標）は\emph{検索・受容判定の品質}を測るものであり、生成品質そのものではない。LLM 生成・実コーパス（HotpotQA 等）での評価は今後の課題である。
...
% （旧「Simple Heuristic との定量比較（テンプレ）」表は未計測セルを含むため v6 改訂で削除。
%   実測の迷路比較は表~\ref{tab:maze_results} を参照。）

\subsection{実験設定の拡充}

\paragraph{データセットと評価クエリ}
RAG 評価は三段階で計画している：小規模 25 クエリ（旧プロトタイプ）、中間（KB 168項目 / 20 ドメイン; 旧プロトタイプ; 表~\ref{tab:rag_static_enrichment} の予備測定）、主要 500 クエリ（本稿の lite スイート; 合成コーパス）。拡張 1000+/100+ ドメインおよび実コーパス（HotpotQA 等）での評価は引き続き計画中である。詳細なサンプリング条件とクエリ例は補足資料にまとめる予定で、本文では設計意図のみ記載する。

\paragraph{比較ベースライン}
Equal\,\mbox{-}resources（同一埋め込み器・ANN 設定・トークン予算・計算資源）を前提に、本稿では Static RAG／Frequency／Cosine-threshold と提案手法（geDIG）を実測比較した。GraphRAG\cite{graphrag2024}／DyG-RAG\cite{dygrag2024}／KEDKG\cite{kedkg2024} との比較、および各手法固有の最適設定による比較は\emph{今後の課題}である。

\subsection{評価指標の詳細定義}

\paragraph{主要評価指標}
\begin{itemize}
  \item \textbf{PrER (Prompt Enrichment Rate)}：強化前後のトークン数比率で定義するプロンプト強化率
...
\begin{table}[H]
\centering
\caption{2段階制御の動作指標（旧プロトタイプの予備測定に基づく\emph{参考値}；本稿の主結果ではない。実測の主結果は表~\ref{tab:rag_summary}）}
...
\paragraph{予備測定で観察された傾向（参考）}
\begin{itemize}
  \item \textbf{AG発火率}: 約40\%のクエリで既存知識不足を検出
  \item \textbf{DG発火率}: AG発火の約67\%（全体の約29\%）で洞察検出によりKG更新
  \item \textbf{KG品質}: DG発火で選別された知識の平均geDIG値は-0.28（構造的・情報的価値が高い候補に偏る）
  \item \textbf{慎重な知識受容}: 単純閾値（58.3\%更新）と比較して、提案手法は28.7\%のみ更新（約半減）
\end{itemize}
\noindent\textit{注}: 上記は旧プロトタイプ系列（実験IVの $N{=}50$ 予備実験と同系統）での観察であり、lite スイートの主結果（受容率 0.374／FMR 0.626）とは設定・採点が異なる。同一プロトコルでの再計測を今後の課題とする。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{fig7_psz_scatter.pdf}
  \caption{PSZ分析。各手法の受容率・FMR・追加P50遅延を散布図として可視化し、薄緑の帯が PSZ 要件（受容$\ge 95$\%, FMR$\le 2$\%, P50$\le \SI{200}{ms}$）を満たす領域を表す。}
  \label{fig:psz_analysis}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{fig_r_baseline_comparison.pdf}
  \caption{主要指標のベースライン比較（exp2to4\_lite; Static／Frequency／Cosine／geDIG\mbox{-}lite）。geDIG\mbox{-}lite は受容率を押し上げ、追加遅延も同一オーダに保つ（ただし FMR は高水準に留まる; 本文参照）。}
  \label{fig:baseline_comparison}
\end{figure}

図\ref{fig:baseline_comparison} は、exp2to4\_lite の equal-resources 実験（Exp~II–III; 500クエリ, \texttt{exp23\_paper} 設定）における Static／Frequency／Cosine／geDIG\mbox{-}lite の主要指標（受容率・FMR・追加P50）を同一座標上に並置したものである。geDIG\mbox{-}lite は受容率を押し上げ（0.0$\to$0.374）、追加遅延も同一オーダ（+80ms）に保つ一方、\emph{FMR は 0.626 と高水準に留まる}。比較対象のベースラインは受容を行わない（Acc=0）ため FMR の直接比較は成立せず、三軸の\emph{同時}改善は現時点では未達である（短欠 $s_{\mathrm{PSZ}}$ は 1.93$\to$1.22 に縮小）。

\subsection{Multi-hop効果の詳細分析}

表\ref{tab:multihop_analysis}は\emph{旧プロトタイプ実装による予備測定}（参考値; 再現スクリプト未整備）である。\textbf{注意}: 本表の数値は\emph{プロンプト強化率}（multi-hop 文脈追加によるプロンプト長の相対増加; 旧実装の enrichment ratio）であり、PER（完全エピソード率）ではない。v6 以前の版では誤って「PER」と表記されていた（経緯は \texttt{docs/audits/per\_metric\_audit.md}）。ホップ数の増加により以下の傾向が観察された：

\begin{table}[H]
\centering
\caption{Multi-hop効果の詳細分析（旧プロトタイプによる予備測定; 参考値）}
\label{tab:multihop_analysis}
\begin{tabular}{lcccc}
\toprule
ホップ数 & 単一ドメイン & クロスドメイン & 深い推論 & 平均強化率(\%) \\
\midrule
...
本章で\emph{実測}を報告するのは lite スイートの動的構成（AG/DG による受容判定つき geDIG）であり、受容を行わない静的ベースラインに対して PER の改善（0.17$\to$0.42）と、受容判定のイベント駆動動作を確認した（表~\ref{tab:rag_summary}）。一方、増分更新・時間整合を備えた動的GRAG（B3; DyG\,RAG系）との直接比較、および Temporal Consistency などの時間整合指標の測定は\emph{未実施}であり、本節の設定記述（イベント・更新・時間）は設計仕様として提示する。
...
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{受容率 Acc}: $N$ 件のクエリに対し受容（TP/FP）された更新の割合（$\mathrm{Acc}=(\mathrm{TP}+\mathrm{FP})/N$）。PSZ では $\mathrm{Acc}\ge 95\%$ を目標とする。
...
\paragraph{サンプルと統計}\; 本稿の\emph{主結果}は lite 系列（$N{=}124$）の $\Delta s\approx+0.021$（符号検定 $p\approx5.2\times10^{-9}$, Cohen's $d\approx1.0$）である。表\ref{tab:insight_vector_preliminary} は旧プロトタイプによる $N{=}50$ の予備実験であり、$\Delta s$ の絶対値は設定・正規化が異なるため主結果と直接比較できない（\emph{参考値}; 符号の正は両者で一致）。今後は $N\!\ge\!200$ までサンプル数を拡大し、95\%CI を含む詳細な統計表を追加する予定である。\\
...
\begin{table}[H]
\centering
\caption{KG構成手法と導出ベクトルの方向一致度（N=50, 旧プロトタイプ予備実験; 参考値。主結果は $N{=}124$ の $\Delta s\approx+0.021$）}
...
\paragraph{観察された傾向}
予備実験（$N{=}50$; 参考値）では、DG発火により選別されたKG（更新率28.7\%）から導出された統合ベクトルの方向一致度が $\Delta s = +0.23$ と最も高く、ランダムサンプリング（-0.05）や単純閾値（+0.12）を上回った。主結果（$N{=}124$）でも符号は一致するが効果の絶対量は小さい（$\Delta s\approx+0.021$）。これは\textbf{KG品質向上が導出ベクトルの方向性改善に寄与する可能性}を示唆するに留まる。
...
表\ref{tab:ablation}に、旧プロトタイプ実装で geDIG の各成分を無効化した場合の性能変化を示す（\emph{参考値}）。\textbf{重要な注意}: 現行実装での再実行（\texttt{maze\_15x15\_ablation\_fullscale.json}; 表~\ref{tab:maze_results} 注記）では成功率・平均ステップに\emph{有意差が再現されておらず}、また lite スイートの RAG アブレーション（表~\ref{tab:exp23_ablation_lite_ja}）でも全バリアントの主要指標が一致した。したがって構成要素の寄与分離は現時点で\emph{未達}であり、下表は設計仮説の参考としてのみ示す。

\begin{table}[H]
\centering
\caption{アブレーション分析：各成分の寄与（旧プロトタイプ; 迷路15×15, $N{=}100$; \emph{参考値}。現行実装では差が再現されていない点に注意）。}
\label{tab:ablation}
\begin{tabular}{lcc}
\toprule
構成 & 成功率(\%) & 平均ステップ \\
...
\noindent 補足: lite スイートでの RAG アブレーション（表~\ref{tab:exp23_ablation_lite_ja}）は base／EPC-only／0-hop-only／IG-emphasis の全構成で PER・受容率・FMR が一致しており、本設定では各項の寄与が判別できない。これは（i）合成データの判別容易性、（ii）受容判定がしきい値支配でゲージ項の差が表面化しない、等の可能性があり、寄与を分離できる難度設定の構築自体を今後の課題とする。
...
表\ref{tab:hop_sensitivity}（旧プロトタイプによる予備測定; \emph{参考値}。第2列はプロンプト強化率であり PER ではない）では、$H=3$が最もバランスが良く、$H>3$では計算コスト増大に対する性能向上が飽和する傾向が観察された。lite スイートでの再計測は今後の課題である。

\begin{table}[H]
\centering
\caption{ホップ数の影響分析（旧プロトタイプ; 参考値）}
\label{tab:hop_sensitivity}
\begin{tabular}{lccccc}
\toprule
$H$ & 強化率(\%) & 受容率(\%) & FMR(\%) & P50遅延(ms) & 計算コスト \\
\midrule
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{multi\,\mbox{-}hop（MDL側）}: 経路短縮優位（複雑さ圧縮）—— decision gate (DG) に相当。\,\emph{hop 拡張は条件集合の追加} $S_{h-1}(Q)\to S_h(Q)$ と読み替えると、$H(Q\mid S_h)$ は単調に減少する。したがって本稿の符号規約（after−before）では $\Delta H^{(h)}_{\mathrm{norm}}\le0$（式\,\ref{eq:cond_dh}）が成立し、秩序化利得は $-\Delta H^{(h)}_{\mathrm{norm}}\ge0$ として読める。
...
\]
であるが、\,\textbf{構造側と情報側の直交性}を強調する見通しのため、次の分解を併記しておく（これは上式の単なる代数的並べ替えであり、本稿での規範形は常に式~\eqref{eq:F_short_intro} とする）：
\[
...
\subsubsection{Phase 2: オフライン全体最適化への拡張}
...
\paragraph{エッジ特徴量の拡張と Phase 2 での利用}
Phase 2 では、単一のスカラー重みだけでなく、複数種類のエッジ特徴量を用いた多面的な最適化を想定する。具体的には、各エッジ $e\in E$ に対して
\[
  \mathbf{f}(e)\in\mathbb{R}^d
\]
の特徴ベクトルを付与し、\emph{圧縮・再配線・重み更新}の材料とする。特徴量の例を挙げる：
\begin{itemize}[leftmargin=*]
  \item \textbf{共起・頻度ベース特徴}: 共起回数、クエリ単位の同時出現頻度、時系列の同時活性パターン（同じ時間帯に頻出するエッジ群）など。
  \item \textbf{埋め込み・幾何ベース特徴}: ノード埋め込み間の類似度、エッジ追加前後の $\Delta\mathrm{SP}_{\mathrm{rel}}$ の寄与、局所クラスタ係数の変化など。
  \item \textbf{モデル・注意ベース特徴}: LLM の注意重みや勾配ベースの saliency、生成時に実際に引用された回数、Chain-of-Thought 中で参照された頻度など。
  \item \textbf{運用ログベース特徴}: AG/DG 通過回数、pending$\to$confirmed／rollback の履歴、FMR に寄与した回数、長期未使用エッジのフラグなど。
\end{itemize}

Phase 1 では、これらの特徴を「ログとして蓄積するだけ」に留める一方で、Phase 2 では $\mathbf{f}(e)$ に基づき次のような処理を行う：
\begin{itemize}[leftmargin=*]
  \item \textbf{エッジごとのスコア学習}: 受容／棄却ラベルや FMR ログを教師として、$s(e)=\mathbf{w}^\top\mathbf{f}(e)$ のような線形（あるいは浅い）スコア関数を学習し、「残すべきエッジ／剪定すべきエッジ／再配線候補」をランク付けする。
  \item \textbf{多目的最適化}: $\mathrm{GED}_{\min}$・エントロピー・エッジ数に加え、「よく使われるが汚染に寄与しやすいエッジ」「ほとんど使われないが長距離橋になっているエッジ」など、$\mathbf{f}(e)$ から見える性質を補助目的として組み込む。
  \item \textbf{エッジ型ごとのポリシー分化}: 例えば「時系列整合の高いエッジは残しやすく、共起のみで支えられているエッジは剪定しやすい」「局所注意重みが安定して高いエッジは橋候補として一段階低コストに扱う」といった、特徴タイプ別のルールベース／ハイブリッド方策を定義する。
\end{itemize}

このように、Phase 1 では $\F$ と AG/DG に基づく局所編集を実時間で行い、Phase 2 では $\mathbf{f}(e)$ による複数チャネルのエッジ特徴量を用いて、\emph{大域的な圧縮・再配線・重み更新}を実行する。両相は同じゲージ $\F$ とログ（AG/DG の履歴）を共有しつつ、時間スケールと目的関数（オンライン運用 vs オフライン最適化）を分離する役割分担になる。
...
\section{展望II: 閃きの計算的モデルに向けて}
\label{sec:insight_model}

本稿は「いつ情報を受け入れるか」の制御に焦点を当てたが、geDIG の枠組みは\textbf{閃き（insight）}の計算的モデルへと自然に拡張できる。本節では、GED編集操作が閃きの実体であるという仮説を提示し、将来の研究方向を示す。

\subsection{GED編集操作 = 閃きの実体}

科学史における重要な発見は、異なるドメイン間の\textbf{構造的類似性}の認識から生まれることが多い。ボーアの原子モデル（1913）は太陽系と原子の構造的類似性（hub-spoke構造）に、ケクレのベンゼン環発見（1865）は蛇が自身の尾を噛むウロボロスと分子構造の類似性（環状構造）に基づく。

これらの\emph{閃き}は、以下の階層で理解できる：
\begin{enumerate}[leftmargin=1.8em]
  \item \textbf{Level 1: パターンマッチ}（要素が似ている）
  \item \textbf{Level 2: アナロジー}（構造が対応する）
  \item \textbf{Level 3: 同型発見}（変換で同一になる）
\end{enumerate}

geDIG の構造類似度評価はLevel 2に対応し、予備実験ではCross-Domain QAタスクでF1 +60\%の改善を確認した（「太陽系 $\approx$ 原子」のような古典的アナロジーを高精度で検出）。

\subsection{アインシュタイン的閃きのモデル化}

Level 3の\emph{同型発見}は、より深い閃きに対応する。アインシュタインの特殊相対論（1905）を例にとると：
\begin{itemize}[leftmargin=1.8em]
  \item 電磁気学と古典力学は矛盾する2つの理論構造
  \item 両者を\textbf{同型にする最小編集操作}がローレンツ変換
  \item この編集操作の発見が「相対論」という閃きの実体
\end{itemize}

数学的には、2つの知識グラフ $G_1$, $G_2$ に対し：
\begin{equation}
  T^* = \arg\min_T \mathrm{GED}(T(G_1), G_2)
  \label{eq:insight_transform}
\end{equation}
が発見すべき変換であり、$T^*$ そのものが閃きの計算的表現となる。

\subsection{分子設計AIとの対応}

この枠組みは、創薬AIにおける\textbf{Scaffold Hopping}（骨格変換）と数学的に同型である：
\begin{center}
\begin{tabular}{ll}
\toprule
分子設計AI & geDIG \\
...
創薬分野では分子グラフのトポロジカル探索が実用化されている。同様の手法を知識グラフに適用することで、「新しい理論を発見するAI」への道が開ける可能性がある。

\subsection{パラダイムシフトの可能性}

式\eqref{eq:insight_transform}の変換 $T^*$ を自動発見するアルゴリズムが実現すれば：
\begin{itemize}[leftmargin=1.8em]
  \item \textbf{現在のAI}: 既存知識の検索と組み合わせ
  \item \textbf{提案AI}: 新しい知識構造の発見
\end{itemize}
という質的飛躍が達成される。これは「検索エンジン」から「科学者」への転換を意味し、人工知能研究における重要なマイルストーンとなりうる。

本稿のgeDIG（Phase~1: When制御）は、この長期ビジョンの最初の一歩である。構造変化のコストと利得を評価する基盤が確立されれば、同型発見アルゴリズムへの拡張が自然に可能となる。
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{効率の改善と選別精度の課題}: equal\,\mbox{-}resources 条件下で、迷路では高い剪定率（98--99\%）とイベント駆動制御の安定動作を、RAG では受容なしベースライン比の PER 改善（0.17$\to$0.42）と短欠 $s_{\mathrm{PSZ}}$ の縮小（1.93$\to$1.22）を示した。一方、実測 FMR は 0.626 と PSZ 目標（$\le$2\%）から大きく乖離しており、受容の\emph{選別精度}（何を受け入れるべきかの弁別）は未達である。
...
\noindent\textit{注}: 下表は\emph{旧プロトタイプ実装の実行系列}に基づく詳細統計であり、平均ステップ等は本文の代表表（表~\ref{tab:maze_results}; 平均 92.8）と実行条件が異なる（参考値）。

\begin{table}[H]
\centering
\caption{15×15迷路の詳細統計（N=100; 旧プロトタイプ系列; 参考値）}
...
旧プロトタイプ実装によるクエリ種別分析（受容率 96--98\%・FMR 1.2--2.8\% 程度の予備測定）が存在するが、lite スイートの実測（受容率 0.374／FMR 0.626）と設定・採点が大きく異なり整合しないため、本版では数表の掲載を見送る。クエリ種別ごとの内訳分析は、lite スイート上での再測定後に報告する。
% （旧表 tab:query_type は実測と不整合のため v6 改訂で削除。履歴は git を参照。）
...
\paragraph{備考（受容判定）。} 補助図では、\textit{分位受容}（上位2\%）を採用して PSZ 帯の到達可能性を例示している（本文の主結果は固定しきい値判定を用いる）。運用要件に応じ、分位/固定のいずれかを選好できる。なお本稿では PSZ を \emph{運用目標（SLO帯域）} として扱い、等資源下では短欠 $s_{\mathrm{PSZ}}$（式~\eqref{eq:psz_deficit}）を併記して比較する。geDIG の $s_{\mathrm{PSZ}}$ はベースラインより小さい（1.22 vs 1.93）が、比較対象が受容を行わない構成である点には注意が必要であり、PSZ 帯への実質的な到達には選別精度（FMR）の改善が必須である。
...
\end{table}
\noindent\textit{注}: 本構成では全バリアントの主要指標が一致しており、lite スイートの規模・設定では構成要素の寄与を\emph{分離できていない}（迷路 15×15 のフルスケール再実行と同様）。寄与を分離できる難度設定の構築は今後の課題である。
