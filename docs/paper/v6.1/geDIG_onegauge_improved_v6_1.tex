...
\centering
\small
\caption{静的RAGにおけるプロンプト強化と関連指標（旧プロトタイプ実装による予備測定; $n{=}168$ クエリ; \emph{参考値}）。}
...
\end{tabular}
\end{table}
\noindent 表\ref{tab:rag_static_enrichment}は\emph{旧プロトタイプ実装による予備測定}（$n{=}168$；再現スクリプト未整備のため参考値）であり、本稿の主結果は lite スイートの表~\ref{tab:rag_summary} とする。予備測定の傾向としては、geDIG によるプロンプト強化率・関連度・多様性・クロスドメイン橋渡しの改善が観察された。
...
RAG 評価は三段階で計画している：小規模 25 クエリ（旧プロトタイプ）、中間 168 クエリ / 20 ドメイン（旧プロトタイプ; 表~\ref{tab:rag_static_enrichment} の予備測定）、主要 500 クエリ（本稿の lite スイート; 合成コーパス）。拡張 1000+/100+ ドメインおよび実コーパス（HotpotQA 等）での評価は引き続き計画中である。詳細なサンプリング条件とクエリ例は補足資料にまとめる予定で、本文では設計意図のみ記載する。
...
表\ref{tab:multihop_analysis}は\emph{旧プロトタイプ実装による予備測定}（参考値; 再現スクリプト未整備。PER の絶対値は lite スイートと正規化が異なる）である。ホップ数の増加により以下の傾向が観察された：
...
表\ref{tab:hop_sensitivity}（旧プロトタイプによる予備測定; \emph{参考値}）では、$H=3$が最もバランスが良く、$H>3$では計算コスト増大に対する性能向上が飽和する傾向が観察された。lite スイートでの再計測は今後の課題である。
