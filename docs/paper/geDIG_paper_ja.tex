...
これは、データから擬似ラベルを生成する自己教師あり学習(Self-Supervised Learning)とも異なり、%
知識構造そのもののポテンシャル変化を直接報酬とする、より高次の自己組織化プロセスである。
...
\paragraph{Transformerアーキテクチャ}
さらに、本研究の理論はTransformerアーキテクチャそのものの解釈にも新たな視座を提供する。我々は、自己注意機構が`geDIG`ポテンシャルのマイクロスケールでの実装であるという仮説を立てている。すなわち、Query-Keyの相互作用が構造的適合性（GED）の評価、Valueの集約が情報利得（IG）の獲得に対応すると解釈できる。スパースアテンションなどの研究が計算効率を主目的とするのに対し、本理論は「なぜそのスパースな構造が優れているのか」を構造コストと情報利得のトレードオフとして説明する、より根源的な理論的枠組みを提供する。
