...
\end{equation}
直前の状態$G_{before}$から現在の状態$G_{after}$への構造変化を測定。%
...
\begin{table}[H]
\centering
\caption{実験2：数学概念進化におけるgeDIGスコアと記憶操作}
\begin{tabular}{lccc}
\toprule
学習段階遷移 & 記憶操作 & ΔGED & $\mathcal{F}_t$ \\
\midrule
小学→中学 & 統合 & -1.8 & 2.1 \\
中学→高校 & 分裂 & -2.3 & 2.7 \\
高校→大学 & 再編成 & -3.1 & 3.5 \\
...
数学概念の進化過程で観察された3つのパターンは、%
人間の概念学習の認知モデルとも整合的である：
\begin{itemize}
\item \textbf{統合}：初期の具体的理解が抽象的理解へと昇華される過程
\item \textbf{分裂}：単純な理解が文脈依存的な複数の理解へと分化する過程
\item \textbf{再編成}：新しいパラダイムによる既存知識の再解釈
\end{itemize}

これらの結果は、geDIGが単なる洞察検出を超えて、%
知識の構造的進化を駆動するメカニズムであることを示唆している。


%====================================================================
\section{議論}

\subsection{理論的整合性：時間スケールの統一}
本研究では、ΔGEDとΔIGの両指標を直前状態からの変化として定義することで、%
時間スケールの一貫性を確保した。%
初期の実装では、ΔGEDを累積的変化として計算していたが、%
これは「地殻変動の速度」と「風の速度」を同じ式で扱うような%
根本的な不整合を含んでいた。

直前状態からの変化への統一により、以下の利点が得られる：
\begin{itemize}
\item 人間の「アハ！体験」の瞬間性との整合
\item 知識グラフの動的変化のリアルタイム検出
\item 数学的に一貫した報酬関数の定義
\end{itemize}

実装修正後も実験1では85\%の精度を維持し、%
理論的整合性と実用的性能の両立を実証した。
