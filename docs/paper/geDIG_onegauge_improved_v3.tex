...
\end{equation}
\noindent\textit{注（$K$ の定義）}\; $K$ はカテゴリサイズ（記号一覧を参照）。\\
...
迷路環境では「周囲が壁」「訪問済み」「行き詰まり」といった条件は明示的に判定可能であり、ドメイン固有の if 文による実装でも解ける課題である。本章は\textbf{ドメイン普遍性を主張するものではなく}、設計原理の\emph{技術的実現可能性}を確認するための\textbf{原理検証（Proof of Concept）}である。具体的には、（i）$\F=\Delta\mathrm{EPC}_{\mathrm{norm}}-\lambda\,\Delta\mathrm{IG}$ の\emph{実時間計算}、（ii）\emph{2段階ゲート}（0\,\mbox{-}hopで注意喚起、multi\,\mbox{-}hopで短絡評価）の\emph{設計通りの挙動}、（iii）\emph{クエリ中心の multi\,\mbox{-}hop 評価}による\emph{計算量と精度のトレードオフ管理}、の3点を目的とする。
