...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{Semantic Alignment (Phase 5)}: We demonstrate that DG-confirmed subgraphs align with LLM answer directions (Insight Vector Alignment), providing evidence that minimizing geDIG $\F$ structurally corresponds to reasoning. (New in v6)
\end{itemize}
...
\end{itemize}
This confirms that $\F$-based control can autonomously detect and decide to switch modes based on ambiguity, acting as a soft error detection mechanism.
...
\begin{itemize}
    ...
    \item \textbf{PSZ Proximity}: Achieved \textbf{Acc $\approx$ 97.1\%, FMR $\approx$ 2.0\%}. Nearly consistent with PSZ requirements.
...
\begin{enumerate}
    ...
    \item \textbf{Implication}: This suggests that minimizing geDIG $\F$ structurally aligns with ``getting closer to the answer'' in the semantic space. This supports the \textbf{Thermodynamic Inference Hypothesis} (minimizing free energy corresponds to reasoning).
\end{enumerate}
