% geDIG One-Gauge 改善版 v6 (2025-12)
...
迷路PoCと実用的RAGパイプラインで評価した結果、迷路では冗長探索を抑制しつつ効率的な到達を促し、RAGでは知識汚染（誤統合）を抑えながら有用な更新を選択的に統合できる傾向を確認した。運用評価として、受容率・False Merge Rate（FMR）・レイテンシからなる目標帯域 \textbf{PSZ（Perfect Scaling Zone）} を定義し、Operating Curve によりトレードオフを可視化する。実装と再現手順を公開する。

\par\medskip
\noindent\textbf{Abstract (English).}\;
Dynamic knowledge graphs lack a normative criterion for \emph{when} to accept new information, even as RAG systems excel at optimizing \emph{what} to retrieve. We propose \textbf{geDIG}, a unified control framework that uses a single gauge $\F$ combining normalized structural cost (edit-path cost) and information gain (Shannon entropy difference and path shortening), and a two-stage gating mechanism---AG at 0-hop and DG via multi-hop---to control exploration and integration decisions. We evaluate geDIG on a maze proof-of-concept and a practical RAG pipeline, and report consistent reductions in redundant exploration and knowledge contamination under an operating-curve protocol (PSZ). We release a reproducible implementation and scripts.
\end{abstract}
...
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{知識グラフ}: 「革命」と「感情」が同じ5段階構造を持つ場合、両者は意味的にも関連している（Cross-domain QA で F1 +60\%）。
...
% (3.6 enabled)
% （詳細版の仮定列挙は FEP--MDL 節に移したため、ここではコメントアウトしておく）
%\begin{comment}
...
\begin{equation}
  \boxed{\;\Delta H^{(h)}_{\mathrm{norm}}\;=\;\frac{H\big(Q\mid S_h(Q)\big)\; -\; H\big(Q\mid S_{h-1}(Q)\big)}{\log |S_h(Q)|}\;}\label{eq:cond_dh_b}
\end{equation}
条件集合が増えるほど $H(Q\mid S_h)$ は単調に減少するため、$\Delta H^{(h)}_{\mathrm{norm}}\le 0$ が保証される（秩序化=曖昧さの減少で負）。したがって、秩序化の\textbf{利得}は $-\Delta H^{(h)}_{\mathrm{norm}}\ge 0$ として読める。なお本稿のゲージは、0\,\mbox{-}hop での過度な集中化（$\Delta H_{\rm norm}\ll0$）を\emph{即時受容の根拠}とはみなさず、multi\,\mbox{-}hop の経路短縮（$\Delta\mathrm{SP}_{\rm rel}>0$）が上回る場合にのみ $\F$ が低下し DG が発火する。
\\[0.2em]
...
\paragraph{備考（受容判定）。} 補助図では、\textit{分位受容}（上位2\%）を採用して PSZ 帯の到達可能性を例示している（本文の主結果は固定しきい値判定を用いる）。運用要件に応じ、分位/固定のいずれかを選好できる。なお本稿では PSZ を \emph{運用目標（SLO帯域）} として扱い、等資源下では短欠 $s_{\mathrm{PSZ}}$（式~\eqref{eq:psz_deficit}）を併記して比較する。geDIG は $s_{\mathrm{PSZ}}$ が一貫して小さく、フロンティアも優位である。
