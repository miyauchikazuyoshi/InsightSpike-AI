...
小さい。そして正直に言うと **p=0.038 (n=3) は 4 つの α 条件への多重比較補正をかけると非有意**（Bonferroni α'=0.0125）。
さらに別対照では **random 値正則化に負けており**（66.5% vs 69.5%）、「F 特有の効果」は未確立。

ポスターの主張は「F 正則化が精度を悪化させない」のレベルの傍証に留める。
方向（最小化/最大化）を含め、符号決定実験（n≥10 seeds・事前登録）を計画中（`docs/audits/f_sign_audit.md`）。

### Q15. 迷路 98% の具体的なベースラインは？

論文 表2 の実数（15×15, n=100, max_steps=250）：

| 手法 | 成功率 | 平均ステップ |
|---|---|---|
| Random Walk | 75% | 105.8 |
| **Greedy DFS** | **99%** | **82.1** |
| geDIG | 98% | 89.6 |

**成功率・ステップとも geDIG は Greedy DFS を上回らない**（25×25 では 75% vs 95% と明確に劣る）。
売りは成功率ではなく、**候補エッジの 98–99% を棄却しながら DFS 同等の成功率を保つ**こと（= ドメインルールなしの創発的制御＋メモリ圧縮）。
質問者が「DFS に負けてるじゃん」と言ったら「**その通りです。本実験の主張は剪定とイベント駆動制御です**」と即答する。
...
反直感。ただし現データは**方向を確定できる強度がない**ので、仮説として答える：

```
SP版 (n=1):   Baseline 88.07% / Positive 87.16% / Negative 89.45%
              ※ ただし baseline 自身が epoch1-2 で 89.45%（過学習前ピークと同値）
β₁版 (n=1):   Baseline 88.5% / Negative 85.5% ← negative も baseline 以下（不成立）
統一版 (n=1): Baseline 82.5% / Positive 84.5% / Negative 86.0% ← positive も改善
別対照:       geDIG-F 正則化 66.5% < random 値正則化 69.5%
```

仮説：F 最大化 = DG（位相的多様性）を保持する正則化として機能（attention の冗長サイクルを壊さない方が良い）。
ただし **n=1・β₁版不成立・random 対照敗北**のため未確立。
注意：JSAI 論文 §4 は F **最小化**で +0.33pp（α=0.001, 補正後は非有意）であり、正則化強度が 100 倍違う（α=0.001 vs β=0.1）。
**「いつ最小化し、いつ保存するか」は理論側の未解決問題**として正直に答える（`docs/audits/f_sign_audit.md`）。
...
| 学習との関係は？ | 論文 §4 (F 正則化) では α=0.001 で +0.33pp（n=3, 多重比較補正後は非有意）。デモは推論時 (観察) の話で別。後続の exp4 に F 最大化で +1.4pp という**単一 run の予備結果**があるが、β₁版では不成立で**方向は未確定**（詳細は次の Q&A）。 |
...
| **exp4** (`thermodynamic_gedig.py`) | 学習時に F **最大化** 正則化 (CE − βF) | SP版 +1.4pp（**n=1**; baseline の早期ピークと同値）。**β₁版は baseline 比 −3.0pp で不成立**、統一版は positive も改善。方向は**未確定** | ❌ (符号決定実験後に判断) |
...
| **介入実験は?** | やりました 3 段階。Phase 3 (推論時介入) は 50% 崩壊で F 特有性示せず。Phase 5 (F 最小化学習) は +0.33pp の小さい効果（補正後は非有意・random 対照に敗北）。後続の exp4 で方向を逆転 (F 最大化) させたら SP 版で +1.4pp が出たが、**n=1 で β₁ 版は不成立、方向の結論は保留**。「いつ最小化し、いつ保存すべきか」を符号決定実験（n≥10 seeds・事前登録）で決めるのが次の課題です。 |
| **論文採用と未採用の境界は?** | **再現性 + 統計的有意性**。Pattern C は 2304 サンプル、p<10⁻⁸⁰。Phase 5 は 3 シードで p=0.038。Pattern A/B/Phase 3 は判定基準未達のため "今後の課題" 扱い。 |
| **exp4 を v7 に入れる予定?** | 符号決定実験（n≥10 seeds・両符号×強度グリッド・random 対照・事前登録）の結果次第です。現状の "negative_better" は n=1 かつ β₁ 版不成立のため、理論基盤にはまだできません。確定したら v7 で正式に位置付けます。 |
