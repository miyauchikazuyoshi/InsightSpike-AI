...
| 質問 | 即答 |
|---|---|
| F は小さい方が良いのでは？増えていくのは何？ | **正しい指摘**。迷路 / RAG では「小さい F = 良い統合判定」。デモは推論軌跡の**観察**で、判断ではない。F 累積の上下方向に良し悪しはなく、見るべきは形と文ごとの違い。 |
| 各層の F が常に正なのはなぜ？ | hidden state は層を進むごとに移動するので ΔEPC ≥ 0。情報利得が EPC を上回ればその層は F < 0。**「ΔH + γΔSP がコスト ΔEPC を上回るか」**が判定材料。 |
