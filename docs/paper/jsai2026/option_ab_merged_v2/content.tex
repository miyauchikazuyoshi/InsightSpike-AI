...
\begin{itemize}
\item $\gednorm$：正規化EPC (Edit Path Cost)．グラフ編集距離（GED）の局所近似であり，MDLにおけるモデル記述長 $L(M)$ の増分に相当する．$\Delta$は比較対象間の差分（迷路：更新前後$G_{t-1},G_t$，Transformer：ランダムベースラインとの差）を表す．
...
なお，推論時にAttention重みを直接編集する介入実験では，軽微なノイズ（scale=0.05）でも精度が約50\%低下した．
これはF正則化が有効なのは学習時にモデル全体が適応できるためであり，推論時の直接操作には適さないことを示す．
