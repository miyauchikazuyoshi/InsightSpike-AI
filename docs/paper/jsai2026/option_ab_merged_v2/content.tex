...
\caption{F正則化強度$\alpha$とSST-2精度の関係（DistilBERT, $n=3$ seeds, エラーバーは標準偏差）．微弱な正則化（$\alpha=0.001$）で精度が86.00\%$\to$86.33\%に改善（$p=0.038$）．過剰な正則化（$\alpha \geq 0.1$）は構造を固定化しすぎ性能を悪化させる．この逆U字特性は適切な構造化圧の存在を示す．}
\label{fig:f_reg}
\end{figure}
