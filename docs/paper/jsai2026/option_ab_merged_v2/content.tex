...
まず，物理的な「構造探索」の典型例として，部分観測迷路（15$\times$15）を用いた検証を行った．
...
パラメータは $\lambda=1.0, \gamma=1.0, \theta_{\mathrm{AG}}=0.4, \theta_{\mathrm{DG}}=0.0$，最大ステップ数は250とした．
...
\begin{table}[t]
\centering
\caption{迷路ナビゲーション結果（15$\times$15, $n=100$ seeds, max\_steps=250）}
...
\multicolumn{4}{l}{\scriptsize 圧縮率は候補エッジに対するコミットエッジの棄却率（edge-compression: $1-|E_{\mathrm{commit}}|/|E_{\mathrm{cand}}|$）} \\
\multicolumn{4}{l}{\scriptsize geDIG vs Random Walk: 同一seedでの成功/失敗をペアにしたMcNemar検定（$p < 10^{-5}$）} \\
\end{tabular}
\end{table}

geDIGはRandom Walkを成功率・効率の両面で上回り，Greedy DFSと同等の成功率を維持しながら，候補エッジの大半を棄却する高い圧縮率（98\%）を達成した（図\ref{fig:maze_process}）．
