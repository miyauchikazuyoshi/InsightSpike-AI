...
\caption{geDIGによる二段階構造化．AG（内側軌道，青）は0-hop評価で自由エネルギーが高い状態（$g_0>\theta_{\mathrm{AG}}$）を検知し探索を起動する．探索により候補分布が広がると$\Delta H>0$（伸展利得）が得られ，$\F$が低下する．DG（外側軌道，赤）はMulti-hop評価で経路短縮（$\Delta \mathrm{SP} > 0$）を発見し，$g_{\min}<\theta_{\mathrm{DG}}$のときエッジを確定する．}
