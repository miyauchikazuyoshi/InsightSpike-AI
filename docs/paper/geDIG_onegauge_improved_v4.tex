...
\section{はじめに}
本稿は、洞察（insight）を「離散的エピソードが瞬時に連結される現象」とする作業仮説を採る。
本仮説は海馬リプレイ（覚醒・睡眠期の順行・逆行再生）に関する先行研究と整合的である\,\cite{Buzsaki2015,Carr2011,Pfeiffer2013}。
言い換えると、「ひらめき」の瞬間を、ばらばらだった経験や記憶が一気につながり直し「あ、こういうことか」と全体像が見える現象として捉える。
この直観を\,\textbf{ナレッジグラフの更新アルゴリズムに適用できる}と仮説し、知識グラフ上の\,\textbf{構造統合}（EPC の抑制）と\,\textbf{情報整理}（エントロピーの減少）の\,\emph{同時発生}として操作化する。
その上で、単一スカラー geDIG により\,\emph{検出と制御}を行う。
ここで geDIG（ジーディグ）は \textit{graph edit Distance and Information Gain} の略であり、既存グラフに対する\,\textbf{編集経路コストの変化}と\,\textbf{情報利得（秩序化＋経路短縮）}を一つのゲージ $\F$ に束ね、静的/動的RAGの双方で「いつ受容するか（When）」を判定する枠組みである。
...
\noindent\textbf{代表挙動（25×25; 120steps）}: $\theta_{\rm AG}{=}{-}0.15$ で 0\,\mbox{-}hop の改善が大きい場面は AG により multi\,\mbox{-}hop を節約（$g_0$ が負域に集中）。\,一方、分岐が濃い区画では forced 候補（赤破線）が保持され、DG 閾値を下げると\,\textit{想起\,\mbox{-}\,確証\,\mbox{-}\,コミット}の連鎖が確認できる。
