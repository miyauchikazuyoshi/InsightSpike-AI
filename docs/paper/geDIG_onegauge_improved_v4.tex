...
\begin{abstract}
本稿は、動的知識グラフにおける\textbf{受容のタイミング（When）}を単一ゲージ\,\textbf{$\F$}（定義は §3.2）で定式化し、\textbf{0‑hop} の曖昧検知（AG）と \textbf{multi‑hop} の洞察確証（DG）によるイベント駆動制御（geDIG）を提案する。\;\textbf{静的RAG}では $\F$ を\,\emph{再ランキングの連続弱化}として用い、\,equal‑resources 下で \textbf{EM/F1 と根拠整合}の向上を確認した。\;\textbf{動的RAG}では $\F$ を\,\emph{更新ゲート}として用い、\,\textbf{受容（Acc）・誤統合（FMR）・遅延（P50）}の三者について \textbf{PSZ}（Acc$\ge$95\%, FMR$\le$2\%, P50$\le$200ms）を運用目標とした曲線を提示し、\,\textit{s\textsubscript{PSZ}} の改善と\,AG/DG 決定ログの\,\emph{可観測化}を示した（到達自体は未達）。\;\textbf{迷路PoCとRAG}の双方で、構造コストと情報利得の差分で\,\emph{短絡}を検出できることを示し、\,FEP–MDL への\,\textbf{操作的ブリッジ}を与える。Phase~2（オフライン再配線）は\,\emph{設計提示のみ}とし、数学的厳密化と大規模検証は今後の課題とする。
\end{abstract}
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{静的$\to$動的の統一プロトコル}: \;静的RAG（更新なし）と動的RAG（更新あり）を\,\textbf{共通条件}（§\ref{sec:common_eval}, §\ref{sec:common_setup}）で評価可能にし、\,\textbf{PSZ/FMR/P50/ZSR} を横断指標として定義。
...
静的/動的の比較は、以降の各章で差分のみを述べ、共通条件はここに集約する（詳細は \S\ref{sec:common_setup}）。
\begin{itemize}[leftmargin=1.6em]
...
\section{理論ブリッジ（FEP–MDL：操作的命題）}\label{sec:fep_mdl_bridge}
