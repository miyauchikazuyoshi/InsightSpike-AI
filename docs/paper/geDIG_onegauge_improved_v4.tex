...
\noindent\textbf{短式}：$\displaystyle \F\;=\;\gednorm\; -\; \lambda\,\bigl(\Delta H_{\mathrm{norm}} + \gamma\,\Delta\mathrm{SP}_{\mathrm{rel}}\bigr)$。\\
\noindent\textbf{符号規約}：$\Delta H_{\rm norm}{=}(H_{\rm after}{-}H_{\rm before})/\log K$（秩序化=低下で \emph{負}）、$\;\Delta\mathrm{SP}_{\rm rel}{=}(L_{\rm before}{-}L_{\rm after})/\max\{L_{\rm before},\varepsilon\}$（短縮で \emph{正}）。$\F$ は\,\textbf{小さいほど良い}（構造コストが低く、情報整理が進むほど小）。
...
\paragraph{PSZ/SLO の定義と集計} 本稿の SLO として \textbf{PSZ（Perfect Scaling Zone）} を用いる。\\
\noindent 目標は3指標の同時充足である：
\begin{equation}
