...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=7mm,>=Stealth,rounded corners,scale=0.8,transform shape]
    % Static pipeline
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    node distance=42mm,
    box/.style={rectangle,draw,rounded corners,align=center,minimum width=40mm,minimum height=10mm,font=\small},
    note/.style={align=center,font=\scriptsize,text width=0.7\linewidth},
    >=Latex,
    scale=0.8,
    transform shape]
...
\subsection*{Summary (results-first)}

The goal of this PoC is to quantify \textbf{exploration efficiency in unknown mazes} under partial observability.
In representative settings (15/25/50$\times$15 mazes), geDIG maintains high success rates (roughly 95--100\% on easier configurations) while reducing the number of steps by about 20--30\% compared to baselines, and pruning roughly 95\% of candidate edges.
Classical planners such as Dijkstra and A* serve as \emph{ideal upper bounds} for reference (Regret, SPL, frontier order), not as direct competitors.

For a 25$\times$25 maze (L3-only, max steps $=250$), typical numbers include:
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Exploration ratio} (unique / total): $\approx 0.26$ (average over seeds).
  \item \textbf{Revisit ratio} (steps / unique): $\approx 1.3$.
  \item \textbf{Average backtrack length}: around 1.0 step (as an approximation of dead-end run length).
  \item \textbf{Dead-end detection delay}: near 0 (effectively immediate).
  \item \textbf{Success rate}: high on 15$\times$15 and 25$\times$25 with generous budgets; lower on harder 25$\times$25 configurations with tight budgets.
  \item \textbf{Reference proximity}: Regret and SPL remain close to Dijkstra/A* bounds, and the frontier ranking induced by $-\F$ correlates well with Dijkstra priority.
\end{itemize}

Design-wise, we verify that:
(i) \textbf{Unified-gauge control} improves combined metrics (success, steps, compression) relative to EPC-only / IG-only controls;
(ii) \textbf{Two-stage gating} behaves as intended (division of labor between 0-hop and multi-hop, stable AG/DG rates and DG/AG ratio);
and (iii) \textbf{Query-centric multi-hop evaluation} with small $H\in\{1,2,3\}$ balances shortcut detection and computational cost.
Together, these observations support the view that \emph{$g_0$ detects stagnation early}, and \emph{DG steers the agent back to promising branches}, thereby reducing unnecessary wandering.

\begin{figure}[H]
  \centering
  \includegraphics[width=.74\linewidth]{fig5_maze_scaling.pdf}
  \caption{Maze scaling (L3; max steps 250/500/1000).
  Success rate (left axis) and average steps (right axis) as a function of maze size.
  For 15$\times$15 (max steps 250) and 25$\times$25 (max steps 500), geDIG maintains high success while keeping steps low; for 51$\times$51, larger budgets (max steps 1000) recover success.}
  \label{fig:maze_scaling}
\end{figure}

In this chapter we treat the partial-observation maze as a small, fully controllable world-model and ask whether a dynamic KG can, under a minimal configuration (8-dimensional state vectors, partial observability, incremental wiring), use the \textbf{single gauge} $\F$ and \textbf{two-stage gate} (AG/DG) to simultaneously govern:
\begin{enumerate}
  \item selection among new episodes (accept / hold / reject), and
  \item exploitation of the episodic memory graph (explore / backtrack / reuse).
\end{enumerate}
The key design elements are:
\begin{enumerate}
  \item \textbf{Unified-gauge control}:
  a single scalar $\F=\gednorm-\lambda\ignorm$ that drives event-based decisions over candidate edges and exploration branches.
  \item \textbf{Two-stage gate (AG/DG)}:
  0-hop (novelty / ambiguity) detects stagnation; multi-hop evaluation confirms shortcuts and triggers backtracking / integration.
  \item \textbf{Query-centric multi-hop evaluation}:
  $k$-hop subgraphs centered on the current episode, providing scalable EPC/IG evaluation with local computations.
\end{enumerate}

\subsection{Environment and episodes}

We use grid mazes of sizes $15{\times}15$, $25{\times}25$, and $50{\times}50$ with partial observations: the agent sees only a local $3{\times}3$ window around its current position.
The underlying maze layout (walls, goal position) is unknown; the agent must discover it by moving.
States, actions, and outcomes are embedded into an 8-dimensional episode vector (one of the simplest nontrivial encodings that satisfies the embedding requirements in \cref{sec:phi_requirements}).
An episodic graph $G_t$ is built online: visited positions become nodes, transitions become edges, and episode vectors are stored as node or edge attributes.

At each time step $t$ the agent:
\begin{itemize}[leftmargin=1.6em]
  \item observes a local patch around the current position $p_t$,
  \item constructs an episode vector $v_t\in\mathbb{R}^8$ (encoding normalized position, wall configuration, visit counts, etc.),
  \item tentatively integrates $v_t$ into the episodic graph,
  \item evaluates candidate connections and updates via the gauge $\F$ and gates AG/DG.
\end{itemize}
...
\subsection{Gauge-based control in the maze}

We use the same gauge as in the RAG setting:
\[
  \F = \Delta\mathrm{EPC}_{\mathrm{norm}} - \lambda \Delta\mathrm{IG}_{\mathrm{norm}},
  \quad
  \Delta\mathrm{IG}_{\mathrm{norm}} = \Delta H_{\mathrm{norm}} + \gamma\Delta\mathrm{SP}_{\mathrm{rel}}.
\]
Here $\Delta\mathrm{EPC}_{\mathrm{norm}}$ is the normalized edit-path cost of the episode edits actually applied in the local graph, and $\Delta\mathrm{IG}_{\mathrm{norm}}$ is the normalized information gain (entropy decrease plus path shortening) on a query-centric induced subgraph.
We interpret negative $\F$ as beneficial (strong consolidation with modest structural cost).

At runtime we separate:
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{0-hop evaluation $g_0(t)$}: local structural and entropy change immediately after tentative wiring; high $g_0$ indicates ambiguous or unhelpful edits.
  \item \textbf{Multi-hop evaluation $g^{(h)}(t)$, $g_{\min}$}: evaluation over $h$-hop induced subgraphs, detecting structural shortcuts (shorter paths, better connectivity).
\end{itemize}
AG is triggered when $g_0>\theta_{\mathrm{AG}}$ (stagnation / ambiguity), and DG is triggered when $\min\{g_0,g_{\min}\}\le\theta_{\mathrm{DG}}$ (confirmed gain).
Percentile-based thresholds $\theta_{\mathrm{AG}},\theta_{\mathrm{DG}}$ are calibrated on burn-in trajectories.

\subsection{Experimental design}

We use grid mazes with random wall patterns and a single start/goal pair.
The agent starts at a fixed or random start cell and aims to reach the goal within a maximum number of steps (step budget scaled with maze size).
The PoC uses simple policies on top of the gauge:
\begin{itemize}[leftmargin=1.6em]
  \item actions are sampled from a softmax over distances to known episodes (greedy toward promising frontier states),
  \item backtracking is handled via BFS to the last branching point when DG confirms a shortcut,
  \item eviction (if enabled) uses low-confidence and old episodes as eviction candidates.
\end{itemize}
We do not perform policy learning or value-function optimization: the goal is to test \emph{operational consistency} of recall and integration under the gauge, not to optimize a general RL policy.

\subsection{Metrics and success criteria}\label{sec:maze_metrics_success}

Primary metrics focus on exploration efficiency:
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{Exploration ratio}: unique visited cells / total steps.
  \item \textbf{Revisit ratio}: steps / unique cells.
  \item \textbf{Average backtrack length}: average length of backtrack segments (AG$\to$DG).
  \item \textbf{Dead-end detection delay}: steps between entering a dead-end and AG firing.
  \item \textbf{Success rate}: fraction of episodes reaching the goal within the step budget.
\end{itemize}
Secondary metrics quantify path quality:
Regret $=$ steps $-L^*$ (difference to shortest path), SPL $=L^*/\max\{L^*,\text{steps}\}$ (closer to $1$ is better).
Diagnostics include runtime P50/P95, AG/DG firing rates, frontier rank correlation between $-\F$ and Dijkstra priority, and path overlap (Jaccard).
...
Greedy Novelty (least-visited-first), $\varepsilon$-greedy (e.g., 90\% unvisited / 10\% random), UCB1-like novelty scores, partially observed A* (A* on the known region plus exploration toward the unknown boundary), and diagnostic oracles (Dijkstra/A* with full map).
We also run ablations of geDIG: EPC-only (sending $\lambda\to\infty$), IG-only (ignoring EPC), no AG/DG (always accept / always reject), and 0-hop-only ($H{=}0$).
...
\begin{table}[H]
  \centering
  \small
  \caption{Static vs dynamic RAG (lite 500-query run; equal-resources). PSZ shortfall $s_{PSZ}$ summarizes how far each method is from the PSZ band (Acc$\ge0.95$, FMR$\le0.02$, P50$\le200$\,ms).}
...
Table~\ref{tab:key_metrics_direction} summarizes the desired directions on each metric axis; we follow the same conventions throughout.
...
\subsection{Setup (events, updates, and time)}

We follow a Dynamic GRAG-style setting with incremental updates and temporal consistency:
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{Data stream.}
  Time-stamped documents (e.g., news, specs, change logs) arrive as episodes with timestamps $t$ and optional durations $\Delta t$.
  \item \textbf{Updates.}
  Each episode is tentatively integrated: a local subgraph is built, merged into the main KG via incremental wiring (high-gain edges strengthened, low-gain edges weakened or pruned), and logged with its gauge trajectory.
  \item \textbf{Retrieval.}
  Time-consistent retrieval follows DyG-RAG / temporal-RAG practices: queries are matched against time-aware neighborhoods, and geDIG-soft applies continuous weakening via $\sigma(\tau\F)$ on top of the base retriever.
  \item \textbf{Generation.}
  We use temporally aware prompts (Temporal CoT-style) that encourage the model to surface time-resolved evidence paths and to always return citations.
  \item \textbf{Sleep phase (offline).}
  In Phase~2, we replay logs of $\F$ and AG/DG decisions over batches to further down-weight persistently low-$\F$ regions and to promote consistently high-gain chains; this is sketched as an outlook rather than fully implemented.
\end{itemize}

\subsection{Dynamic metrics and success criteria}

Beyond EM/F1 and faithfulness, dynamic runs track:
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{Acceptance rate (Acc).}
  Fraction of updates (pending/confirmed) that are ultimately judged correct; PSZ targets Acc$\ge95\%$.
  \item \textbf{False Merge Rate (FMR).}
  Among accepted episodes, the fraction later judged erroneous merges; PSZ targets FMR$\le2\%$.
  \item \textbf{Additional P50 latency.}
  Median extra latency relative to static RAG; PSZ targets P50$_{\Delta\mathrm{lat}}\le200$\,ms.
  \item \textbf{PSZ shortfall $s_{\mathrm{PSZ}}$.}
  Aggregated deficit from the PSZ band as defined in \cref{eq:psz_deficit}.
  \item \textbf{ZSR (Zero-Search Rate).}
  Fraction of queries answered at 0-hop (no AG firing), indicating how often the current KG suffices without additional retrieval.
  \item \textbf{Temporal Consistency and update lag.}
  Stability of answers and evidence as the KG evolves, and delay from ingest to availability.
  \item \textbf{AG/DG firing rates.}
  Frequency of AG and DG events over the query stream, used as diagnostics for gating behavior.
\end{itemize}
As a practical success criterion we aim either to enter the PSZ band or to \emph{consistently reduce} $s_{\mathrm{PSZ}}$ while keeping answer quality and temporal consistency acceptable.

\subsection{Time-series analysis and operating curves}

Following the evaluation protocol in \cref{sec:eval_protocol,sec:common_setup}, we analyze AG/DG behavior over query indices and inspect operating curves:
\begin{itemize}[leftmargin=1.6em]
  \item \textbf{AG/DG time-series.}
  We plot AG and DG firing rates as functions of query index; under scale-up, AG tends to increase (more ambiguity in larger KGs) while DG remains in a moderate band (roughly a few percent), consistent with the intended division of labor.
  \item \textbf{Operating curves.}
  By sweeping $(\theta_{\mathrm{AG}},\theta_{\mathrm{DG}})$ we obtain 3D operating curves over (Acc, FMR, additional P50).
  Regions with low $s_{\mathrm{PSZ}}$ correspond to operating points that are close to the PSZ band, even if full attainment is not yet realized.
\end{itemize}
These analyses, together with the logs of $\Delta\mathrm{EPC}$, $\Delta H$, $\Delta\mathrm{SP}$, and $\F$, give an operational view of \emph{when} the system chose to explore, backtrack, or commit, and how AG/DG behavior changes as the KG grows.
...
\begin{table}[H]
  \centering
  \small
  \caption{Alignment summary (lite 500-query run).
  Support vs random subgraphs show a positive $\Delta s$ with large effect size and statistically significant sign tests.}
  \label{tab:alignment_summary_en}
...
\begin{table}[H]
  \centering
  \small
  \caption{Ablation summary (lite 500-query dynamic RAG).
  Variants that remove EPC, IG, $\Delta\mathrm{SP}$, or gates show degraded acceptance profiles or latency compared to the base configuration.}
...
\noindent\textit{Scope.}
This section provides an \emph{operational} bridge to existing theory; the empirical results and main claims of this paper do not depend on it, and readers may safely skip it on a first pass.
We treat the Free Energy Principle (FEP) and Minimum Description Length (MDL) as sources of design intuition rather than as frameworks to which we claim formal equivalence.

\paragraph{Background: FEP and MDL}
FEP (in the sense of Friston and others~\cite{Friston2010}) frames adaptive behavior as minimization of a variational free-energy bound on surprise; MDL frames learning as minimization of the sum of model and data code lengths.
In both views, there is a trade-off between \emph{complexity} (model / structure cost) and \emph{accuracy} (fit to data).
Our gauge $\F=\gednorm-\lambda\ignorm$ can be read as an operational instantiation of this trade-off at the level of a dynamic knowledge graph, where:
\begin{itemize}[leftmargin=*]
  \item $\gednorm$ plays the role of a structural complexity term (edit-path cost of the applied updates),
  \item $\ignorm=\Delta H_{\mathrm{norm}}+\gamma\Delta\mathrm{SP}_{\mathrm{rel}}$ plays the role of a data/accuracy improvement term (entropy decrease plus path shortening),
  \item $\lambda$ acts as an information temperature-like coefficient that balances the two.
\end{itemize}
We do \emph{not} claim that $\F$ is a variational bound in the FEP sense or that it equals a particular MDL objective; instead, we use these analogies to reason about design choices and to interpret experimental observations.

\paragraph{Operational correspondence: definition}
We call a relation an \emph{operational correspondence} if it satisfies three conditions:
\begin{enumerate}[leftmargin=1.6em]
  \item it is proportional rather than exact (e.g., up to a positive scale factor),
  \item residual terms are bounded, e.g., $O(1/N)$ under explicit assumptions, and
  \item it yields concrete, testable predictions (e.g., about ablations or metric behavior).
\end{enumerate}
In our case we consider a correspondence of the form
\[
  \F \;\propto\; \Delta\mathrm{MDL} + O(1/N),
\]
under assumptions about normalization, bounded horizons, decomposable edits, stable entropy estimation, and embedding conditions as in \cref{sec:phi_requirements}.
The proportionality constant is absorbed into $\lambda$ (interpreted as a scale-anchoring, temperature-like parameter).

\paragraph{Reading guide}
This section is best read as a proof sketch tying together:
\begin{itemize}[leftmargin=*]
  \item the \textbf{definition} of the gauge $\F=\gednorm-\lambda\ignorm$ (\cref{sec:gauge_theory}),
  \item the \textbf{duality intuition} between 0-hop (error / ambiguity; FEP-side) and multi-hop (compression / shortcuts; MDL-side),
  \item the \textbf{gate mechanism} (AG/DG) as an engineering realization of this split (\cref{sec:ag_dg_gate}),
  \item the \textbf{embedding assumptions} (A1--A3) in \cref{sec:phi_requirements}.
\end{itemize}
The main question is: under what assumptions can we interpret $\F$ as proportional to a change in an MDL-style objective, and what does this imply for using a single control signal to govern both structure and information?

\paragraph{Implications}
If the correspondence $\F\propto\Delta\mathrm{MDL}{+}O(1/N)$ holds under these assumptions, it suggests:
\begin{itemize}[leftmargin=*]
  \item \textbf{Justification for a single control signal.}
  It is reasonable to use one scalar $\F$ to jointly control structure edits (EPC) and information consolidation ($\Delta H$, $\Delta\mathrm{SP}$) rather than separate, potentially conflicting signals.
  \item \textbf{Interpretation of $\lambda$.}
  The coefficient $\lambda$ can be read as an information temperature, analogous to the ratio between data and model code-length scales in MDL.
  \item \textbf{Orthogonality and double-counting.}
  By keeping $\Delta$EPC on the structural side and $\Delta\mathrm{IG}$ on the information side, we avoid double-counting structural effects in the information term, which improves interpretability.
  \item \textbf{Alignment with ablations.}
  Ablations in \cref{sec:ablation_analysis} (removing $\Delta H$ or $\Delta\mathrm{SP}$) show behavior consistent with the idea that these terms contribute to an MDL-like compression objective.
\end{itemize}
At the same time, this is explicitly an \emph{operational} proposition: we do not claim that $\F$ is the unique or correct MDL/FEP objective, only that it behaves in a way that is consistent with these principles under the given assumptions.

\paragraph{Assumptions (B1--B4) and their intent}
To reason about $\F$ as proportional to $\Delta\mathrm{MDL}$ we rely on the following assumptions, adapted from the Japanese manuscript:
\begin{description}[leftmargin=1.8em]
  \item[(B1) Local boundedness] Edit costs are bounded by $C_{\max}$ and the multi-hop horizon $H$ is finite; this anchors normalization and prevents unbounded growth of $\gednorm$.
  \item[(B2) Edit decomposability] Substitution operations can be upper-bounded by delete+insert pairs, so GED can be approximated as a sum of additive edit costs.
  \item[(B3) Entropy-estimation stability] The variance of local entropy-difference estimates scales as $\sigma_H^2 = O(1/|\mathcal{N}|)$ for neighborhood size $|\mathcal{N}|$; we use estimators with uniform convergence.
  \item[(B4) Normalization stability] The normalization bases $C_{\max}(G)$ and $\log K(G)$ vary slowly over time so that their fluctuations can be absorbed into a scalar factor (and treated as $O(1/N)$ residuals).
\end{description}
These assumptions correspond to the embedding requirements (A1)--(A3) in \cref{sec:phi_requirements}: semantic gradient preservation (A1) supports stable probability distributions for $\Delta H$, scale normalization (A2) stabilizes $C_{\max}$ and $\log K$, and local smoothness (A3) keeps edits and neighborhood changes well behaved.

\begin{table}[H]
  \centering
  \small
  \caption{Correspondence between assumptions (B1--B4) and embedding requirements (A1--A3), summarized at a high level.}
  \label{tab:ba_mapping_en}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{@{}l l l@{}}
    \toprule
...
\paragraph{Proposition (summary) and proof sketch}
Under assumptions (B1)--(B4) and the normalization conventions in \cref{sec:gauge_theory}, we can summarize the main proposition as:
\[
  \F \;=\; \gednorm - \lambda\ignorm \;\propto\; \Delta\mathrm{MDL} + O(1/N).
\]
MDL decomposes into a model term $L(M)$ and a data term $L(D\mid M)$.
Using (B2) and (B1), the model term can be upper-bounded as $c_{\mathrm{ged}}\gednorm + O(1/N)$ for some constant $c_{\mathrm{ged}}$.
Using (B3) and (B4), the data term can be approximated as $-c_{\mathrm{ig}}\ignorm + O(1/N)$ with $c_{\mathrm{ig}}$ capturing the scale of entropy / path contributions.
Choosing $\lambda \approx c_{\mathrm{ig}}/c_{\mathrm{ged}}$ aligns scales so that $\F$ becomes proportional to $\Delta\mathrm{MDL}$ up to an $O(1/N)$ residual.
This is not a formal theorem but an operational statement: it justifies reading $\F$ as a proxy for MDL-style change under the stated assumptions, and it matches the behavior observed in ablations.

\paragraph{Thermodynamic reading (metaphor)}
Following the Japanese manuscript, we can also offer a free-energy-style reading as a metaphor.
We define
\begin{equation}
  U := \Delta\mathrm{EPC}_{\mathrm{norm}} - \lambda\gamma\,\Delta\mathrm{SP}_{\mathrm{rel}},\quad
  S := \Delta H_{\mathrm{norm}},\quad
  F := U - \lambda S,
  \label{eq:free_energy_en}
\end{equation}
so that $F$ is isomorphic to $\F$ by rearrangement.
Here $U$ is read as a structural energy term (edit cost minus path-efficiency gains), $S$ as an entropy term (ordering), and $F$ as a free-energy-like objective.
This mapping is purely heuristic and is not required for using geDIG; it simply offers an additional lens for interpreting $\lambda$ as $kT$ and for thinking about phase-transition-style behavior (e.g., under $\lambda$-scans).

\paragraph{Open theoretical questions}
A full, rigorous treatment of the MDL correspondence and of the thermodynamic analogy is beyond the scope of this paper.
Open questions include generalizing assumptions (e.g., to directed or labeled graphs), tightening bounds, connecting to information thermodynamics, and deriving sharper predictions about phase transitions in graph structure as $\lambda$ varies.
We present this section as an invitation for theoretical collaboration rather than as a completed theory.
...
\begin{table}[H]
  \centering
  \small
  \caption{High-level comparison with related approaches (performance / control / theory layers).}
  \label{tab:related_comparison_en}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lcccccc}
    \toprule
    Method & Dynamic KG & Structure detection & Single gauge & Joint learn/use & Insight events & Theory basis \\
    \midrule
    GraphRAG & \checkmark & $\triangle$ & $\times$ & $\times$ & $\times$ & -- \\
    DyG-RAG & \checkmark & $\triangle$ & $\times$ & $\triangle$ & $\times$ & -- \\
    KEDKG & \checkmark & $\times$ & $\times$ & $\triangle$ & $\times$ & -- \\
    FEP / Active Inf. & $\times$ & $\times$ & \checkmark & \checkmark & $\triangle$ & FEP \\
    MDL / IB & $\times$ & $\triangle$ & \checkmark & $\times$ & $\triangle$ & MDL \\
    \textbf{geDIG} & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & FEP--MDL (operational) \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\textbf{Internal validity} is limited by the choice of grading model, embedder (Sentence-BERT), and prompts; although we use equal-resources comparisons and deterministic scripts, changes in these components could affect absolute numbers.
\textbf{External validity} is limited by domain coverage, vocabulary, and scale: the maze PoC uses synthetic environments, and the RAG experiments use a lite 500-query suite with a single family of LLMs / encoders, so results may not transfer directly to other domains or larger graphs.
\textbf{Construct validity} is constrained by our metric choices: Acc/EM/PER, FMR, PSZ, and ZSR capture important aspects of operation but do not exhaustively describe user-centric utility or long-term safety.
Finally, \textbf{implementation biases} (e.g., particular approximate shortest-path algorithms, embedding approximations) may influence measured $\Delta\mathrm{SP}$ and thereby $\F$; we mitigate this via ablations and by keeping the code and scripts open for inspection and reproduction.
...
\subsection{Why PSZ is not yet fully attained}

Although geDIG reduces PSZ shortfall $s_{\mathrm{PSZ}}$ compared to baselines, our current dynamic experiments do not fully enter the PSZ band.
We see three main contributing factors.

First, \textbf{hyperparameter tuning is deliberately conservative}.
We calibrate $\lambda$ once on a lite development suite and then keep it fixed across runs, and we use simple percentile rules for $(\theta_{\mathrm{AG}},\theta_{\mathrm{DG}})$ to avoid overfitting.
This avoids overly optimistic operating points at the cost of leaving some room for improvement in Acc/FMR trade-offs.
Systematic $\lambda$- and $\gamma$-sweeps under stricter train/val/test splits are left for future work.

Second, \textbf{experimental scale and diversity are limited}.
The dynamic RAG experiments use a 500-query lite suite with a single family of encoders and LLMs; under such constraints, PSZ is a relatively strict target.
Larger and more diverse query sets, and more aggressive exploration of DG thresholds under equal-resources constraints, may identify regions that enter PSZ more fully.

Third, \textbf{gate thresholds and time-series behavior are tuned for stability rather than PSZ optimality}.
In our current design, AG/DG rates are kept in moderate bands to avoid oscillatory or brittle behavior; this sometimes leaves FMR slightly above the PSZ target or P50 slightly above 200\,ms.
We view PSZ as an SLO-style goal rather than a hard requirement; future work will explore adaptive thresholding and more nuanced per-query-type gating policies to move operating points closer to the PSZ band without sacrificing robustness.

\subsection{Phase~2 outlook: offline rewiring}\label{sec:phase2_offline}
...
\paragraph{Edge feature expansions and Phase~2 use}
In Phase~2 we envision using not only scalar edge weights but a richer \emph{edge feature vector} to support multi-faceted optimization.
For each edge $e\in E$ we attach a feature vector
\[
  \mathbf{f}(e)\in\mathbb{R}^d
\]
that summarizes how the edge behaves under queries and updates.
Examples include:
\begin{itemize}[leftmargin=*]
  \item \textbf{Co-occurrence / frequency features}: co-occurrence counts, per-query joint activation frequencies, temporal co-activation patterns (edges that tend to be active in the same time windows).
  \item \textbf{Embedding / geometric features}: similarity between node embeddings, contribution to $\Delta\mathrm{SP}_{\mathrm{rel}}$ when the edge is added or removed, changes in local clustering coefficients.
  \item \textbf{Model / attention features}: LLM attention weights or gradient-based saliency, number of times the edge was actually cited, frequency of appearance in chains-of-thought.
  \item \textbf{Operational log features}: AG/DG passage counts, histories of pending$\to$confirmed / rollback, contributions to FMR, flags for long-unused edges, etc.
  \end{itemize}

In Phase~1 we primarily \emph{log} $\mathbf{f}(e)$ while using $\F$ and AG/DG for real-time decisions.
Phase~2 then uses $\mathbf{f}(e)$ as input to:
\begin{itemize}[leftmargin=*]
  \item \textbf{Edge scoring}: learn a simple scorer $s(e)=\mathbf{w}^\top\mathbf{f}(e)$ (or a shallow model) from acceptance / rejection labels and FMR logs, and rank edges as ``keep'', ``prune'', or ``rewire candidate''.
  \item \textbf{Multi-objective optimization}: augment the Phase~2 objective with terms that capture patterns in $\mathbf{f}(e)$, such as ``frequently used but contamination-prone edges'' or ``rarely used but structurally important bridge edges''.
  \item \textbf{Edge-type-specific policies}: design differentiated policies, e.g.\ keep temporally consistent edges more aggressively than pure co-occurrence edges, or treat edges with stable high attention weights as bridge candidates with lower effective cost.
\end{itemize}
Thus Phase~1 performs local edits in real time based on $\F$ and AG/DG, while Phase~2 uses multi-channel edge features $\mathbf{f}(e)$ to perform \emph{global compression, rewiring, and weight updates}.
Both phases share the same gauge $\F$ and logs (AG/DG histories), but operate at different time scales and with different objectives (online operation vs offline optimization).

\paragraph{Roadmap and collaboration}
Looking ahead, we see several concrete directions.
First, we plan to \textbf{scale Phase~1} to larger and more diverse query suites (5k--10k+ queries across many domains) and systematically explore $\lambda$, $\gamma$, and gate thresholds under stricter train/validation/test splits.
Second, we aim to \textbf{operationalize Phase~2} by implementing offline rewiring pipelines that replay logs at scale, tune edge-feature-based objectives, and stress-test PSZ behavior under long-running workloads.
Third, we would like to \textbf{add transformer-based validation experiments}, including alternative retrievers / LLMs and controlled perturbation studies, to probe how robust the gauge and gates are to model changes.
Finally, we intend to \textbf{extend geDIG to other domains} (e.g., software engineering corpora, scientific literature, enterprise knowledge graphs) and to more varied forms of ``insight'' events.
These directions are intentionally left open for collaboration: we welcome joint work and co-authorship on both theoretical tightening (FEP--MDL, optimality) and large-scale empirical validation.
