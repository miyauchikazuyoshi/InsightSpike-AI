...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{RQ2 (Discrimination)}: Can AG/DG discriminate between \emph{true insights} (bridges/shortcuts) and \emph{pseudo-insights} (misdirection/marginal improvement), thereby reducing contamination rate ($\downarrow$) and increasing the pending$\to$confirmed rate ($\uparrow$)?
...
\textbf{Objective}\; Operate $\theta_{\mathrm{AG}},\theta_{\mathrm{DG}},\lambda,\,H,\,k$ \emph{stably and reproducibly} without relying on excessive manual tuning.
\begin{description}[leftmargin=1.6em]
  ...
  \item[Determination Procedure for $\lambda$] \; In practice, determined by: (1) Measure $\sigma_{\mathrm{EPC}}{=}\mathrm{Std}[\gednorm], \sigma_{\mathrm{IG}}{=}\mathrm{Std}[\ignorm]$ with pilot $N{\approx}100$, (2) Set initial value $\lambda_0{=}\sigma_{\mathrm{EPC}}/\max\{\sigma_{\mathrm{IG}},\varepsilon\}$ ($\varepsilon{=}10^{-6}$), (3) Compare grid $\{\tfrac{1}{2}\lambda_0,\lambda_0,2\lambda_0\}$, (4) Select $\lambda$ maximizing PSZ compliance (Acc/FMR/P50), (5) Fix for all subsequent experiments (prevent drift). This interpretation corresponds to discrete system approximation of information thermodynamic temperature ($\beta{=}1/k_BT$).
...
\noindent\textit{Setting parameters $\lambda,\gamma$}\; As scale alignment of structure and information terms, $\lambda$ is first determined by initial value $\lambda_0:=\mathrm{Std}[\gednorm]/\max\{\mathrm{Std}[\ignorm],\varepsilon\}$ via variance balancing as stated in "Note: IG approximation and $\lambda$ interpretation", and PSZ achievement rate and FMR/latency are observed with small grid $\{\tfrac{1}{2}\lambda_0,\lambda_0,2\lambda_0\}$.
In actual operation, once decided $\lambda$ is fixed for the entire system and not tuned frequently per environment (see Fig.\ref{fig:psz_operating_curves_lite_ja} and $(\tau,\lambda)$ Operating Curves in Appendix). $\gamma$ is a coefficient determining balance of contribution between $\Delta H$ and $\Delta\mathrm{SP}_{\rm rel}$, confirmed to have low sensitivity around range $\gamma{\approx}0.5$ in maze PoC.
\\
\noindent\textit{Similarity calculation and Linkset construction}\; In implementation, $S_{\mathrm{link}}$ is constructed by Top-$k$ acquisition based on weighted cosine similarity for episode embedding $\phi(e)\in\mathbb{R}^d$ (weights assigned by task-specific weight vector management; code example: \texttt{src/insightspike/search/similarity\_search.py}).
...
\subsection{Qualitative Analysis: How $\F$ Behaves}
...
\end{enumerate}
This justifies proceeding to the RAG experiment, mapping ``Dead-end'' to ``Unknown/Hallucination'' and ``Shortcut'' to ``Insight/Logical Connection''.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Justification of Single Control}: Learning (structure edit) and Inference (search/compression) are theoretically unified as minimizing $\F$.
...
% \bibliography{references} % Already included in main file structure usually
...
\begin{figure}[H]
  \centering
  \missingfigure[figcolor=white]{Operating Curves (Please insert `exp23_paper_20251104_052644_psz_curve.png`)}
...
\begin{figure}[H]
  \centering
  \missingfigure[figcolor=white]{Latency vs Accept (Please insert `exp23_paper_20251104_052644_latency_curve.png`)}
...
\begin{figure}[H]
  \centering
  \missingfigure[figcolor=white]{Gating Time-series (Please insert `exp23_paper_20251104_052644_gating_timeseries.png`)}
